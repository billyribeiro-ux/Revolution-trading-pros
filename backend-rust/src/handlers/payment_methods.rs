...
pub async fn store(
    State(_state): State<AppState>,
    _auth: AuthUser,
    Json(_payload): Json<AddPaymentMethodRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    Ok(Json(
        serde_json::json!({"success": true, "message": "Payment method added"}),
    ))
}
...
pub async fn destroy(
    ...
    Ok(Json(
        serde_json::json!({"success": true, "message": "Payment method deleted"}),
    ))
}
