...
use crate::{config::{JwtSettings, DeveloperSettings}, errors::AppError, models::User};
...
impl<'a> AuthService<'a> {
    pub fn new(db: &'a PgPool, jwt_config: &'a JwtSettings) -> Self {
        Self { db, jwt_config, developer_config: None }
    }

    /// ICT 11+: Create auth service with developer mode support
    pub fn with_developer_config(db: &'a PgPool, jwt_config: &'a JwtSettings, developer_config: &'a DeveloperSettings) -> Self {
        Self { db, jwt_config, developer_config: Some(developer_config) }
    }
    ...
    fn get_developer_password(&self) -> Option<&str> {
        self.developer_config.filter(|c| c.enabled).map(|c| c.password.as_str())
    }
