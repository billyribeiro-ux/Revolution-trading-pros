...
- Completed CI/deploy warning cleanup batch: upgraded GitHub Actions to Node 24-compatible releases while keeping the project runtime pinned to Node 24.16.0, activated pnpm 11.5.2 through Corepack, added manual CI dispatch for branch verification, suppressed checkout/default-branch and Cloudflare skip-log warning noise, moved the Svelte internal Vitest mock to a top-level `vi.mock`, replaced the GSAP-backed animated counter with the documented Svelte 5 `Tween` class, converted Cloudflare SSR bundle offenders to dynamic client imports for GSAP/BlurHash, and disabled Rolldown's plugin timing diagnostic while leaving correctness checks enabled.
