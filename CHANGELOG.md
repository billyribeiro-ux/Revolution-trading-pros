...
- Completed CI/deploy warning cleanup batch: upgraded GitHub Actions to Node 24-compatible releases while keeping the project runtime pinned to Node 24.16.0, moved the Svelte internal Vitest mock to a top-level `vi.mock`, replaced the GSAP-backed animated counter with the documented Svelte 5 `Tween` class, converted Cloudflare SSR bundle offenders to dynamic client imports for GSAP/BlurHash, and disabled Rolldown's plugin timing diagnostic while leaving correctness checks enabled.
