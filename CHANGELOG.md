...
- Completed mixed UI batch (`FloatingTocWidget.svelte`, `SocialShare.svelte`, `AddToCartButton.svelte`, `EnterpriseChart.svelte`, and `PricingSelector.svelte`): converted tracked class/style bindings and pricing declaration tags, added social copy timeout cleanup, replaced chart container `bind:this` with an attachment, and removed a placeholder chart update effect flagged by Svelte MCP.
