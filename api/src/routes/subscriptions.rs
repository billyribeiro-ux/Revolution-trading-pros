...
async fn get_plan(
    ...
    let plan: MembershipPlanRow = sqlx::query_as(
        "SELECT * FROM membership_plans WHERE slug = $1"
    )
    .bind(&slug)
...
async fn create_subscription(
    ...
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?
    .ok_or_else(|| (StatusCode::NOT_FOUND, Json(json!({"error": "Plan not found or inactive"}))))?;

    // Check if user already has active subscription
    let existing: Option<(i64,)> = sqlx::query_as(
        "SELECT id FROM user_memberships WHERE user_id = $1 AND status = 'active'"
    )
    .bind(user.id)
    .fetch_optional(&state.db.pool)
    .await
    ...
    if existing.is_some() {
        return Err((StatusCode::CONFLICT, Json(json!({"error": "User already has an active subscription"}))));
    }
...
async fn cancel_subscription(
    ...
    if subscription.status != "active" {
        return Err((StatusCode::BAD_REQUEST, Json(json!({"error": "Subscription is not active"}))));
    }
...
async fn get_metrics(
    ...
    let active: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM user_memberships WHERE status = 'active'")
        .fetch_one(&state.db.pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;

    let cancelled: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM user_memberships WHERE status = 'cancelled'")
        .fetch_one(&state.db.pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;

    let expired: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM user_memberships WHERE status = 'expired'")
        .fetch_one(&state.db.pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;
