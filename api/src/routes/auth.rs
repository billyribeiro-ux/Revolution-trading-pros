...
async fn register(
    ...
    let existing: Option<User> = sqlx::query_as(
        "SELECT id, email, password as password_hash, name, role, email_verified_at, avatar_url, mfa_enabled, created_at, updated_at FROM users WHERE email = $1"
    )
        .bind(&input.email)
...
async fn verify_email(
    ...
    let user: User = sqlx::query_as(
        "SELECT id, email, password as password_hash, name, role, email_verified_at, avatar_url, mfa_enabled, created_at, updated_at FROM users WHERE id = $1"
    )
        .bind(user_id)
        .fetch_one(&state.db.pool)
        .await
