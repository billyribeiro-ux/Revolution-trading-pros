...
//   - next_renewal        : same as above, then for each active sub on the
//                           old price, PATCH it to the new price with
//                           proration_behavior=none and billing_cycle_anchor
//                           pinned (no proration, renewal date intact).
//   - immediate_proration : same as above, but proration_behavior=
//                           create_prorations (Stripe issues prorated
...
async fn list_plan_price_history(
    State(state): State<AppState>,
...
pub fn plans_router() -> Router<AppState> {
    Router::new()
        ...
        .route("/:id/price", post(change_plan_price))
        .route("/:id/price-history", get(list_plan_price_history))
}
