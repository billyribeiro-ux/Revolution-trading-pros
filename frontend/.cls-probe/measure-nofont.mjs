...
const init = () => {
	...
	new PerformanceObserver((l) => {
		for (const e of l.getEntries()) {
			if (!e.hadRecentInput) {
				window.__c += e.value;
				window.__t.push(Math.round(e.startTime));
			}
		}
...
if (BLOCK) {
	await ctx.route('**/*', (r) => {
		...
		if (/\.(woff2?|ttf|otf|eot)(\?|$)/i.test(u) || r.request().resourceType() === 'font')
			return r.abort();
		return r.continue();
	});
}
...
console.log(
	'fontsBlocked=' + BLOCK,
	'CLS=' + r.c.toFixed(4),
	'shiftTimes(ms)=' + JSON.stringify(r.t)
);
await b.close();
