...
const probe = async () => p.evaluate(() => {
	...
	return {
		...
		htmlPaddingRight: html.style.paddingRight || '0px', htmlOverflow: html.style.overflow || '',
		scrollbarW: window.innerWidth - html.clientWidth, scrollY: window.scrollY,
...
if (!ham) { console.log(JSON.stringify({ error: 'no hamburger at this width', before }, null, 2)); await b.close(); process.exit(0); }
await ham.click(); await p.waitForTimeout(500);
const opened = await probe();
const closeBtn = await p.$('.mobile-close') || ham;
await closeBtn.click(); await p.waitForTimeout(600);
const closed = await probe();
console.log(JSON.stringify({
	url: URL, viewport: `${W}x${H}`, before, opened, closed,
