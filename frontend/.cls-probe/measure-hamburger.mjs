...
const probe = async () =>
	p.evaluate(() => {
		...
		return {
			heroTag: hero
				? hero.tagName.toLowerCase() +
					'.' +
					(typeof hero.className === 'string'
						? hero.className.trim().split(/\s+/).slice(0, 2).join('.')
						: '')
				: null,
...
if (!ham) {
	console.log(JSON.stringify({ error: 'hamburger not found', before }));
	await b.close();
	process.exit(0);
}
	...
	JSON.stringify(
		{
			...
			closed,
			heroShift_open_x:
				opened.heroX !== null && before.heroX !== null
					? +(opened.heroX - before.heroX).toFixed(2)
					: null,
			heroShift_open_y:
				opened.heroY !== null && before.heroY !== null
					? +(opened.heroY - before.heroY).toFixed(2)
					: null,
			heroShift_afterClose_x:
				closed.heroX !== null && before.heroX !== null
					? +(closed.heroX - before.heroX).toFixed(2)
					: null,
			heroShift_afterClose_y:
				closed.heroY !== null && before.heroY !== null
					? +(closed.heroY - before.heroY).toFixed(2)
					: null
		},
		null,
		2
