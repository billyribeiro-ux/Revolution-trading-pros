...
const initScript = () => {
	...
	new PerformanceObserver((list) => {
		for (const e of list.getEntries()) {
			...
			window.__shifts.push({ value: +e.value.toFixed(4), t: Math.round(e.startTime), sources });
		}
...
await ctx.addCookies([
	{ name: 'rtp_access_token', value: 'mock', domain: 'localhost', path: '/' }
]);
...
console.log(JSON.stringify({
	label: LABEL, url: URL, viewport: `${W}x${H}`, status: resp?.status(), finalURL: p.url(),
	totalCLS: +r.cls.toFixed(4), shiftCount: r.shifts.length,
