...
const URL=process.argv[2], W=Number(process.argv[3]||390), H=Number(process.argv[4]||844), WAIT=Number(process.argv[5]||1000);
...
await p.goto(URL,{waitUntil:'networkidle',timeout:30000});await p.waitForTimeout(WAIT);
...
console.log('preWait='+WAIT+'ms  CLS_beforeScroll='+c0.toFixed(4)+'  CLS_afterScroll='+(await p.evaluate(()=>window.__c)).toFixed(4));
await b.close();
