...
const watchlist = {
	...
	video_title: 'Watchlist Rundown',
	thumbnail_url: 'https://placehold.co/640x360',
	description: 'This week’s setups',
	stocks: []
};
...
const favorites = [
	{
		...
		item_type: 'alert',
		item_id: 9001,
		title: 'SPX 5800c — runner trimmed',
		excerpt: 'Took partials into the 11:30 push, trailing the rest on the 5-min.',
		href: '/dashboard/explosive-swings/alerts/9001',
		thumbnail_url: 'https://placehold.co/320x240',
	...
	},
	{
		...
		item_type: 'video',
		item_id: 9002,
		title: 'Weekly game plan walkthrough',
		excerpt: null,
		href: '/dashboard/explosive-swings/videos/9002',
		thumbnail_url: 'https://placehold.co/320x240',
		created_at: '2026-05-27T09:00:00Z'
	},
	{
		id: 103,
		item_type: 'trade_plan',
		item_id: 9003,
		title: 'NVDA earnings playbook',
		excerpt: 'Three scenarios mapped with invalidation levels and sizing notes for each.',
		href: null,
		thumbnail_url: null,
...
let seo404 = [
	{
		...
		url: '/old-landing',
		hits: 42,
		last_hit_at: '2026-05-30T10:00:00Z',
		referer: 'google.com',
		is_resolved: false,
		is_ignored: false
	},
	{
		...
		url: '/discontinued',
		hits: 12,
		last_hit_at: '2026-05-29T10:00:00Z',
		referer: null,
		is_resolved: false,
		is_ignored: false
	},
	{
		...
		url: '/typo-url',
		hits: 5,
		last_hit_at: '2026-05-28T10:00:00Z',
		referer: 'twitter.com',
		is_resolved: false,
		is_ignored: false
	}
...
createServer(async (req, res) => {
	...
	if (url.includes('/seo/redirects')) {
		...
		if (toggleMatch) {
			...
			seoRedirects = seoRedirects.map((r) =>
	...
	if (url.includes('memberships')) {
		await delay(NETWORK_DELAY_MS);
		return send(res, 200, { success: true, data: { memberships } });
	}
	if (url.includes('watchlist')) {
		await delay(NETWORK_DELAY_MS);
		return send(res, 200, { success: true, data: watchlist });
	}
	if (url.includes('weekly-video')) {
		await delay(NETWORK_DELAY_MS);
		return send(res, 200, { success: true, data: watchlist });
	}
	...
	if (url.includes('/media/analytics/')) {
		...
		if (url.includes('/overview'))
			return send(res, 200, {
				totalOriginal: 5_000_000_000,
				totalOptimized: 2_100_000_000,
				totalSavings: 2_900_000_000,
				savingsPercent: 58,
				totalImages: 12400,
				optimizedImages: 11900,
				avgCompressionRatio: 2.4,
		...
		if (url.includes('/bandwidth'))
			return send(res, 200, [
				{
					date: '2026-05-01',
					original: 800_000_000,
					optimized: 340_000_000,
					savings: 460_000_000,
					requests: 9000
				},
				{
					date: '2026-05-15',
					original: 900_000_000,
					optimized: 380_000_000,
					savings: 520_000_000,
					requests: 9800
				},
				{
					date: '2026-05-31',
					original: 950_000_000,
					optimized: 400_000_000,
					savings: 550_000_000,
					requests: 10200
				}
			]);
			...
			return send(res, 200, [
				{
					format: 'webp',
					count: 7200,
					originalSize: 3_000_000_000,
					optimizedSize: 1_200_000_000,
					savings: 1_800_000_000
				},
				{
					format: 'avif',
					count: 4700,
					originalSize: 2_000_000_000,
					optimizedSize: 900_000_000,
					savings: 1_100_000_000
				}
			]);
		return send(res, 200, []);
	}
	...
	if (orderDetailMatch) {
		...
		return send(res, 200, {
			data: {
				...
				billing_name: 'Measure User',
				billing_email: 'measure@rtp.test',
				items: [
					{ name: 'Explosive Swings (monthly)', quantity: 1, unit_price: 149, total: 149 }
				]
			}
		});
	}
	...
	if (url.includes('/admin/orders')) {
		...
		const mk = (n) => ({
			id: n,
			order_number: `RTP-${1000 + n}`,
			status: n % 3 === 0 ? 'pending' : 'completed',
			total: 99 + n,
			currency: 'USD',
			user_email: `u${n}@rtp.test`,
			user_name: `User ${n}`,
		...
		return send(res, 200, {
			...
			stats: {
				...
				refunded_orders: 3,
				total_revenue: 12000,
				revenue_this_month: 3400,
				average_order_value: 164
			},
