		...
		if ( doing_action( 'woocommerce_add_cart_item_data' ) ) {
			...
			if ( $product ) {
				...
				if ( isset( $_REQUEST['customer_id'] ) ) {
					$user_id = sanitize_text_field( wp_unslash( $_REQUEST['customer_id'] ) );
				} elseif ( isset( $_REQUEST['form'] ) ) {
					parse_str( wp_unslash( $_REQUEST['form'] ), $query ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					$user_id = isset($query['customer_id']) ? intval($query['customer_id']) : null;
				}
