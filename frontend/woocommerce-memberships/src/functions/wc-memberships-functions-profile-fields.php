...
function wc_memberships_profile_field_form_field( \SkyVerge\WooCommerce\Memberships\Profile_Fields\Profile_Field $profile_field, array $args = [] ) {
	...
	if (
		...
		if ( \SkyVerge\WooCommerce\Memberships\Profile_Fields::TYPE_FILE === $field_args['type'] ) :
			...
			?>
			<div class="form-row <?php echo esc_attr( implode( ' ', $field_args['class'] ) ); ?> " id="<?php echo esc_attr( $field_key ); ?>_field">
				<span class="woocommerce-input-wrapper">

					<?php if ( $field_args['label'] ) : ?>
						<label for="<?php echo esc_attr( $field_key ); ?>" class="<?php echo esc_attr( implode( ' ', $field_args['label_class'] ) ); ?>"><?php echo wp_kses_post( $field_args['label'] . $required ); ?></label>
					<?php endif; ?>

					<div class="wc-memberships-profile-field-input-file-plupload <?php echo esc_attr( implode( ' ', $field_args['input_class'] ) ); ?>">
					...
					<?php if ( isset( $field_args['description'] ) && '' !== $field_args['description'] ) : ?>
						<span class="description" id="<?php echo esc_attr( $field_key . '-description' ); ?>"><?php echo wp_kses_post( $field_args['description'] ); ?></span>
					<?php endif; ?>
		...
		elseif ( \SkyVerge\WooCommerce\Memberships\Profile_Fields::TYPE_MULTICHECKBOX === $field_args['type'] || \SkyVerge\WooCommerce\Memberships\Profile_Fields::TYPE_RADIO === $field_args['type'] ) :
			...
			if ( ! empty( $field_args['options'] ) ) :
				...
				?>
				<p class="form-row <?php echo esc_attr( implode( ' ', $field_args['class'] ) ); ?>" id="<?php echo esc_attr( $field_key ); ?>_field">
					<span class="woocommerce-input-wrapper">
						...
						<?php if ( $field_args['label'] ) : ?>
							<label for="<?php echo esc_attr( $field_key ); ?>" class="<?php echo esc_attr( implode( ' ', $field_args['label_class'] ) ); ?>"><?php echo esc_html( $field_args['label'] ) . $required; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></label>
						<?php endif; ?>

						<?php foreach ( $field_args['options'] as $option_key => $option_text ) : ?>

							<label for="<?php echo esc_attr( $field_key ) . '_' . esc_attr( $option_key ); ?>" class="checkbox <?php echo esc_attr( implode( ' ', $field_args['label_class'] ) ); ?>">
								<input
				...
				<p class="form-row <?php echo esc_attr( implode( ' ', $field_args['class'] ) ); ?>" id="<?php echo esc_attr( $field_key );?>_field">
					<span class="woocommerce-input-wrapper">
						...
						<select
							...
							class="select select2-search__field <?php echo esc_attr( implode( ' ', $field_args['input_class'] ) ); ?>"
							multiple="multiple"
							<?php echo ! empty( $custom_attributes ) ? implode( ' ', $custom_attributes ) : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
