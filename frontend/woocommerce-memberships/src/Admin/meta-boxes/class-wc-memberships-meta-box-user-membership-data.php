...
class WC_Memberships_Meta_Box_User_Membership_Data extends \WC_Memberships_Meta_Box {
			...
			<li class="wide">
				...
				if ( ! empty( $user_membership_actions ) ) :
					...
					foreach( $user_membership_actions as $id => $action ) :
						...
						?>
						<div id="<?php echo esc_attr( $id ); ?>" class="user-membership-action">
							<?php
							...
							?>
							<a href="<?php echo esc_url( $action['link'] ); ?>" class="<?php echo esc_attr( $action['class'] ); ?>" <?php echo $custom_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
				...
				if ( $paused_date = $user_membership->get_local_paused_date( 'timestamp' ) ) :
					...
					<p class="form-field">
						<span class="description"><?php
							...
							printf( esc_html__( 'Paused since %s', 'woocommerce-memberships' ),
								esc_html( date_i18n( wc_date_format(), $paused_date ) . ' ' . date_i18n( wc_time_format(), $paused_date ) )
							); ?></span>
					</p>
					<?php
	...
	public function update_data( $post_id, \WP_Post $post ) {
		...
		if ( $user_membership ) {
			...
			$raw_start_date    = ! empty( $_POST['_start_date'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['_start_date'] ) ) ) : null;
			$raw_end_date      = ! empty( $_POST['_end_date'] )   ? trim( sanitize_text_field( wp_unslash( $_POST['_end_date'] ) ) )   : null;
