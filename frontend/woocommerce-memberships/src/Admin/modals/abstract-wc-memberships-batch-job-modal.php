...
abstract class WC_Memberships_Batch_Job_Modal extends \WC_Memberships_Modal {
	...
	protected function get_job_progress() {
		...
		<strong><?php /* translators: Placeholder: %s - percentage completed */
			printf( esc_html__( 'Current progress: %s', 'woocommerce-memberships' ), '{{data.job.percentage}}%' ); ?></strong>
		<?php
		...
		<article>
			...
			<# if ( data.job ) { #>
				...
				<div class="wc-memberships-background-process-modal-progress">
					...
					<# if ( 'completed' == data.job.status || 100 == data.job.percentage ) { #>
						<?php echo $this->get_job_completed(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<# } else { #>
						<# if ( data.error ) { #>
							<?php echo $this->get_job_error(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						<# } else { #>
							<?php echo $this->get_job_progress(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						<# } #>
					<# } #>
			...
			<# } else { #>
				...
				<div class="wc-memberships-background-process-modal-description">
					<?php echo $this->get_job_description(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</div>
		...
		<footer>
			<div class="inner">
				...
				<# if ( data.error ) { #>
					...
					<?php echo $this->get_close_button(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				...
				<# } else if ( data.job ) { #>
					...
					<# if ( 'completed' == data.job.status ) { #>
						<?php echo $this->get_close_button(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<# } else { #>
						<?php echo $this->get_stop_button(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<# } #>

				<# } else { #>

					<?php echo $this->get_cancel_button(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<?php echo $this->get_start_button(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
