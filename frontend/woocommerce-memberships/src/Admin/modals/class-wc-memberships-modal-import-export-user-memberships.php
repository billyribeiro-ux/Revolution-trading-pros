	...
	protected function get_job_progress() {
		...
		<# if ( 'import' == data.action.id ) { #>
			<p><?php /* translators: Placeholders: %1$s - opening <em> tag, %2$s - opening <strong> tag, %3$s - closing </strong> tag, %4$s - closing </em> tag */
				printf( esc_html__( '%1$s%2$sImportant!%3$s Do not leave this window until the import completes or it will be cancelled. Imported memberships will not be deleted automatically if the import is interrupted.%4$s', 'woocommerce-memberships' ), '<em>', '<strong>', '</strong>', '</em>' ); ?></p>
		<# } #>

		<# if ( 'export' == data.action.id ) { #>
			<p><?php /* translators: Placeholders: %1$s - opening <em> tag, %2$s - opening <strong> tag, %3$s - closing </strong> tag, %4$s - closing </em> tag */
				printf( esc_html__( '%1$s%2$sImportant!%3$s Do not leave this window until the export has completed or it will be stopped.%4$s', 'woocommerce-memberships' ), '<em>', '<strong>', '</strong>', '</em>' ); ?></p>
		<# } #>
		...
		<# if ( 'import' == data.action.id ) { #>
			...
			<small>
				<# if ( data.job.total == 1 ) { #>
					...
					printf( esc_html__( 'Processed %s out of 1 row.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?>
				<# } else { #>
					<?php /* translators: Placeholders: %1$s - current progress (number), %2$s - total amount of rows processed during an import */
					printf( esc_html__( 'Processed %1$s out of %2$s rows.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?>
				<# } #>
			</small>
		...
		<# if ( 'export' == data.action.id ) { #>
			...
			<small>
				<# if ( data.job.total == 1 ) { #>
					...
					printf( esc_html__( 'Processed %s out of 1 membership.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?>
				<# } else { #>
					<?php /* translators: Placeholders: %1$s - current progress (number), %2$s - total amount of user memberships to process (number) */
					printf( esc_html__( 'Processed %1$s out of %2$s memberships.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?>
				<# } #>
			</small>
