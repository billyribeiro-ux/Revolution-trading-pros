...
class List_Table extends \WP_List_Table {
	...
	private function get_profile_field_definitions() {
		...
		foreach ( $profile_field_definitions as $profile_field_slug => $profile_field_definition ) {
			...
			if ( ! empty( $filter_by_type ) && ! $profile_field_definition->is_type( $_GET['type'] ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function no_items() {
		...
		if ( ! Profile_Fields::is_using_profile_fields() ) {
			...
			printf(
				...
				esc_html__( 'You haven\'t created any profile fields yet &mdash; %1$sClick Here%2$s to create your first one!', 'woocommerce-memberships' ),
			...
			printf(
				...
				esc_html__( 'No profile fields found &mdash; %1$sClick Here%2$s to create one!', 'woocommerce-checkout-add-ons' ),
				'<a href="' . esc_url( wc_memberships()->get_admin_instance()->get_profile_fields_instance()->get_new_profile_field_definition_screen_url() ) . '">',
				'</a>'
			);
	...
	private function get_profile_field_definitions_type_filter() {
		...
		if ( null === $this->field_type_filter ) {
			$this->field_type_filter = isset( $_GET['type'] ) ? sanitize_text_field( wp_unslash( $_GET['type'] ) ) : '';
		}
	...
	private function get_profile_field_definitions_editable_by_filter() {
		...
		if ( null === $this->editable_by_filter ) {
			$this->editable_by_filter = isset( $_GET['editable_by'] ) ? sanitize_text_field( wp_unslash( $_GET['editable_by'] ) ) : '';
		}
