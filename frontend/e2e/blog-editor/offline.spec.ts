...
test.describe('Offline Mode', () => {
	...
	test.describe('Network State Detection', () => {
		test('should detect when going offline', async ({ adminPage, context }) => {
			...
			const offlineIndicator = adminPage.locator(
				'.offline-indicator, [data-testid="offline-status"], .connection-status'
			);
		...
		test('should detect when coming back online', async ({ adminPage, context }) => {
			...
			const offlineIndicator = adminPage.locator(
				'.offline-indicator, [data-testid="offline-status"]'
			);
		...
		test('should show sync pending indicator when offline changes exist', async ({
			adminPage,
			context,
			blockHelper
		}) => {
			...
			const syncIndicator = adminPage.locator(
				'.sync-pending, [data-testid="sync-pending"], .unsaved-indicator'
			);
	...
	test.describe('Offline Editing', () => {
		...
		test('should allow adding all block types while offline', async ({
			adminPage,
			context,
			blockHelper
		}) => {
		...
		test('should allow deleting blocks while offline', async ({
			adminPage,
			context,
			blockHelper
		}) => {
		...
		test('should allow reordering blocks while offline', async ({
			adminPage,
			context,
			blockHelper
		}) => {
		...
		test('should maintain undo/redo while offline', async ({
			adminPage,
			context,
			blockHelper,
			editorHelper
	...
	test.describe('Local Storage Persistence', () => {
		test('should save changes to local storage while offline', async ({
			adminPage,
			context,
			blockHelper
		}) => {
			...
			const storedData = await adminPage.evaluate(() => {
				const keys = Object.keys(localStorage).filter(
					(k) => k.includes('draft') || k.includes('editor') || k.includes('post')
				);
		...
		test('should restore from local storage on page reload', async ({
			adminPage,
			context,
			blockHelper,
			editorHelper
		...
		test('should clear local storage after successful save', async ({
			adminPage,
			context,
			blockHelper,
			editorHelper
	...
	test.describe('Sync and Recovery', () => {
		...
		test('should recover unsaved work after crash', async ({
			adminPage,
			context,
			blockHelper,
			editorHelper
	...
	test.describe('Autosave While Offline', () => {
		test('should disable server autosave when offline', async ({
			adminPage,
			context,
			blockHelper
		}) => {
		...
		test('should autosave to local storage when offline', async ({
			adminPage,
			context,
			blockHelper
		}) => {
			...
			const hasLocalSave = await adminPage.evaluate(() => {
				return Object.keys(localStorage).some(
					(k) => k.includes('draft') || k.includes('autosave') || k.includes('editor')
				);
			});
	...
	test.describe('Media Handling While Offline', () => {
		...
		test('should process queued uploads when back online', async ({
			adminPage,
			context,
			blockHelper
		}) => {
	...
	test.describe('Error Handling', () => {
		...
		test('should not lose data during connection issues', async ({
			adminPage,
			context,
			blockHelper
		}) => {
		...
		test('should show appropriate error messages', async ({
			...
			const errorMessage = adminPage.locator(
				'.save-error, [data-testid="save-failed"], .offline-warning'
			);
	...
	test.describe('Performance', () => {
		test('should maintain performance while offline', async ({
			adminPage,
			context,
			blockHelper,
			perfHelper
	...
	test.describe('Accessibility', () => {
		test('should announce offline status to screen readers', async ({
			adminPage,
			context,
			a11yHelper
		}) => {
		...
		test('should have proper ARIA labels for offline indicators', async ({
			adminPage,
			context,
			a11yHelper
		}) => {
