...
test.describe('Drag and Drop Operations', () => {
  ...
  test.describe('Single Block Drag', () => {
    test('should drag block from position 0 to position 2', async ({ adminPage, blockHelper }) => {
      ...
      if (firstBox && lastBox) {
        // Find drag handle
        const dragHandle = firstBlock.locator('[data-testid="drag-handle"], .drag-handle, [class*="grip"]').first();
    ...
    test('should show visual feedback during drag', async ({ adminPage, blockHelper }) => {
      ...
      if (box) {
        ...
        const dropIndicator = adminPage.locator('.drop-indicator, [data-drop-indicator], .drag-preview');
        const isDragging = adminPage.locator('.is-dragging, [data-dragging="true"]');

        // Either should be visible during drag
        const hasVisualFeedback = await dropIndicator.isVisible().catch(() => false) ||
    ...
    test('should show drop indicator at correct position', async ({ adminPage, blockHelper }) => {
      ...
      if (firstBox && secondBox) {
        ...
        await adminPage.mouse.move(secondBox.x + secondBox.width / 2, secondBox.y + secondBox.height / 2);
    ...
    test('should not allow drag in read-only mode', async ({ adminPage, blockHelper }) => {
      ...
      const isDisabled = await dragHandle.isDisabled().catch(() => false) ||
  ...
  test.describe('Auto-Scroll During Drag', () => {
    test('should auto-scroll up when dragging near top edge', async ({ adminPage, blockHelper }) => {
    ...
    test('should auto-scroll down when dragging near bottom edge', async ({ adminPage, blockHelper }) => {
      // Add many blocks
      for (let i = 0; i < 15; i++) {
        await blockHelper.addBlock('paragraph');
      }
  ...
  test.describe('Keyboard-Based Drag', () => {
    ...
    test('should announce position changes for screen readers', async ({ adminPage, blockHelper, a11yHelper }) => {
      await blockHelper.addBlock('paragraph');
      await blockHelper.addBlock('heading');
      await blockHelper.addBlock('quote');
  ...
  test.describe('Touch-Based Drag', () => {
    ...
    test('should provide haptic feedback on supported devices', async ({ adminPage, blockHelper }) => {
  ...
  test.describe('Drag Constraints', () => {
    ...
    test('should prevent dropping block into nested children', async ({ adminPage, blockHelper }) => {
    ...
    test('should respect locked blocks (no drag)', async ({ adminPage, blockHelper }) => {
      ...
      const lockButton = adminPage.locator('[data-testid="lock-block"], button[aria-label="Lock block"]').first();
  ...
  test.describe('Drag Performance', () => {
    test('should maintain 60fps during drag with many blocks', async ({ adminPage, blockHelper }) => {
      // Add many blocks
      for (let i = 0; i < 30; i++) {
        await blockHelper.addBlock('paragraph');
      }
      ...
      if (box) {
        ...
        if (measuredRates.length > 0) {
          const avgFps = measuredRates.reduce((a: number, b: number) => a + b, 0) / measuredRates.length;
    ...
    test('should complete drag operation under 100ms', async ({ adminPage, blockHelper, perfHelper }) => {
      await blockHelper.addBlock('paragraph');
      await blockHelper.addBlock('heading');
      await blockHelper.addBlock('quote');
