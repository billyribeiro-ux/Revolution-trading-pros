...
test.describe('Keyboard Shortcuts', () => {
	...
	test.describe('Text Formatting', () => {
		...
		test('should apply strikethrough with Ctrl+Shift+S', async ({ adminPage, blockHelper }) => {
			...
			const html = await editableContent.innerHTML();
			const hasStrike =
				html.includes('<s>') || html.includes('<del>') || html.includes('line-through');
		...
		test('should create link with Ctrl+K', async ({ adminPage, blockHelper }) => {
			...
			const linkDialog = adminPage.locator(
				'.link-dialog, [data-testid="link-dialog"], input[placeholder*="URL"]'
			);
	...
	test.describe('Block Operations', () => {
		...
		test('should select all blocks with Ctrl+A (when block focused)', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
	...
	test.describe('Navigation', () => {
		...
		test('should open block inserter with /', async ({ adminPage, blockHelper }) => {
			...
			const slashMenu = adminPage.locator(
				'.slash-menu, .block-inserter, [data-testid="slash-menu"]'
			);
		...
		test('should search blocks with /block-name', async ({ adminPage, blockHelper }) => {
			...
			const headingOption = adminPage.locator(
				'[data-block-type="heading"], .block-item:has-text("Heading")'
			);
	...
	test.describe('Save and Publish', () => {
		...
		test('should prevent accidental navigation with unsaved changes', async ({
			adminPage,
			blockHelper,
			editorHelper
		}) => {
	...
	test.describe('View Mode', () => {
		...
		test('should toggle fullscreen with F11 or Ctrl+Shift+F', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
		...
		test('should switch to desktop preview with Ctrl+Shift+D', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
		...
		test('should switch to tablet preview with Ctrl+Shift+T', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
		...
		test('should switch to mobile preview with Ctrl+Shift+M', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
	...
	test.describe('Help and Settings', () => {
		test('should show keyboard shortcuts help with Ctrl+/', async ({ adminPage, blockHelper }) => {
			...
			const helpModal = adminPage.locator(
				'.keyboard-shortcuts-modal, [data-testid="keyboard-help"]'
			);
	...
	test.describe('Heading Levels', () => {
		...
		test('should convert paragraph to heading with Ctrl+Alt+1', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
	...
	test.describe('List Operations', () => {
		...
		test('should convert to numbered list with Ctrl+Shift+O', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
	...
	test.describe('Accessibility', () => {
		test('should announce shortcut actions for screen readers', async ({
			adminPage,
			blockHelper,
			a11yHelper
		}) => {
		...
		test('should support focus visible on keyboard navigation', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
	...
	test.describe('Custom Shortcuts', () => {
		...
		test('should prevent browser default for registered shortcuts', async ({
			adminPage,
			blockHelper
		}) => {
			await blockHelper.addBlock('paragraph');
	...
	test.describe('Shortcut Performance', () => {
		test('should respond to shortcuts under 50ms', async ({
			adminPage,
			blockHelper,
			perfHelper
		}) => {
