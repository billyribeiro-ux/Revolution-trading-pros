...
test.describe('Collaboration Features', () => {
  ...
  test.describe('Multi-User Editing', () => {
    test('should show presence indicator when another user joins', async ({ browser, mockApi }) => {
      ...
      try {
        ...
        const presenceIndicator = page1.locator('.presence-indicator, [data-testid="user-presence"], .collaborator-avatar');
  ...
  test.describe('Block Locking', () => {
    test('should lock block when user is editing', async ({ adminPage, mockApi, editorHelper, blockHelper }) => {
      ...
      const lockIndicator = adminPage.locator('.block-locked, [data-locked="true"], .editing-indicator');
    ...
    test('should release lock when user leaves block', async ({ adminPage, mockApi, editorHelper, blockHelper }) => {
      await mockApi.setupBlogMocks();
      await editorHelper.navigateToCreate();
      ...
      const isStillLocked = await firstBlock.locator('.editing-indicator, [data-editing="true"]').isVisible().catch(() => false);
  ...
  test.describe('Comments and Annotations', () => {
    test('should add comment to block', async ({ adminPage, mockApi, editorHelper, blockHelper }) => {
      await mockApi.setupBlogMocks();
      await editorHelper.navigateToCreate();
      ...
      // Open comment panel
      const commentButton = adminPage.locator('[data-testid="add-comment"], button[aria-label="Add comment"]').first();
      if (await commentButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        await commentButton.click();

        const commentInput = adminPage.locator('textarea[placeholder*="comment"], [data-testid="comment-input"]').first();
        if (await commentInput.isVisible()) {
          await commentInput.fill('This is a test comment');

          const submitComment = adminPage.locator('button:has-text("Submit"), [data-testid="submit-comment"]').first();
          await submitComment.click();

          await adminPage.waitForTimeout(500);

          // Check for comment indicator
          const commentIndicator = adminPage.locator('.comment-indicator, [data-has-comments="true"]');
    ...
    test('should show comment thread', async ({ adminPage, mockApi, editorHelper, blockHelper }) => {
      await mockApi.setupBlogMocks();
            ...
            body: JSON.stringify({
              data: {
                ...
                blocks: [{
                  ...
                  metadata: {
                    comments: [
                      { id: '1', userId: 'u1', userName: 'User 1', content: 'First comment', createdAt: new Date().toISOString() },
                      { id: '2', userId: 'u2', userName: 'User 2', content: 'Reply', createdAt: new Date().toISOString() }
                    ]
                  }
                }]
              }
      ...
      const commentIndicator = adminPage.locator('.comment-indicator, [data-testid="comment-badge"]').first();
    ...
    test('should resolve comment', async ({ adminPage, mockApi, editorHelper }) => {
            ...
            body: JSON.stringify({
              data: {
                ...
                blocks: [{
                  ...
                  metadata: {
                    comments: [
                      { id: '1', content: 'Comment to resolve', resolved: false }
                    ]
                  }
                }]
              }
      ...
      const resolveButton = adminPage.locator('button:has-text("Resolve"), [data-testid="resolve-comment"]').first();
    ...
    test('should delete own comment', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      const deleteButton = adminPage.locator('[data-testid="delete-comment"], button[aria-label="Delete comment"]').first();
  ...
  test.describe('Conflict Resolution', () => {
    test('should detect editing conflicts', async ({ browser }) => {
      ...
      try {
        ...
        if (await editableContent1.isVisible() && await editableContent2.isVisible()) {
          ...
          const conflictIndicator = page1.locator('.conflict-indicator, [data-testid="conflict-warning"]');
          const hasConflict = await conflictIndicator.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    test('should allow keeping local changes', async ({ adminPage, mockApi, editorHelper, blockHelper }) => {
      await mockApi.setupBlogMocks();
      await editorHelper.navigateToCreate();

      // If conflict dialog appears
      const keepLocalButton = adminPage.locator('button:has-text("Keep Mine"), [data-testid="keep-local"]').first();
    ...
    test('should allow accepting server changes', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      const acceptServerButton = adminPage.locator('button:has-text("Accept Server"), [data-testid="accept-server"]').first();
    ...
    test('should allow merging changes', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      const mergeButton = adminPage.locator('button:has-text("Merge"), [data-testid="merge-changes"]').first();
  ...
  test.describe('Revision History', () => {
    ...
    test('should list previous revisions', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      await adminPage.route('**/api/admin/posts/*/revisions', async (route) => {
        await route.fulfill({
          ...
          body: JSON.stringify({
            data: [
              { id: '1', createdAt: new Date().toISOString(), createdByName: 'User 1', type: 'manual' },
              { id: '2', createdAt: new Date().toISOString(), createdByName: 'User 2', type: 'auto' }
            ]
          })
        });
      });
    ...
    test('should preview revision', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      const previewButton = adminPage.locator('button:has-text("Preview"), [data-testid="preview-revision"]').first();
      if (await previewButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        await previewButton.click();

        const revisionPreview = adminPage.locator('.revision-preview, [data-testid="revision-preview"]');
    ...
    test('should restore revision', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      await adminPage.route('**/api/admin/posts/*/revisions', async (route) => {
        await route.fulfill({
          ...
          body: JSON.stringify({
            data: [
              { id: '1', createdAt: new Date().toISOString(), createdByName: 'User 1', blocks: [{ type: 'paragraph', content: { text: 'Old content' } }] }
            ]
          })
        });
      });
      ...
      const restoreButton = adminPage.locator('button:has-text("Restore"), [data-testid="restore-revision"]').first();
    ...
    test('should compare revisions', async ({ adminPage, mockApi, editorHelper }) => {
      ...
      const compareButton = adminPage.locator('button:has-text("Compare"), [data-testid="compare-revisions"]').first();
  ...
  test.describe('Real-Time Updates', () => {
    ...
    test('should handle disconnection gracefully', async ({ adminPage, mockApi, editorHelper, blockHelper }) => {
      await mockApi.setupBlogMocks();
      await editorHelper.navigateToCreate();
      ...
      const offlineIndicator = adminPage.locator('.offline-indicator, [data-testid="offline-warning"]');
  ...
  test.describe('Accessibility', () => {
    test('should announce collaboration events to screen readers', async ({ adminPage, mockApi, editorHelper, a11yHelper }) => {
      await mockApi.setupBlogMocks();
      await editorHelper.navigateToCreate();
    ...
    test('should have proper ARIA labels for collaboration features', async ({ adminPage, mockApi, editorHelper, a11yHelper }) => {
      await mockApi.setupBlogMocks();
      await editorHelper.navigateToCreate();

      const results = await a11yHelper.analyze('.presence-indicator, .comment-panel, .revision-history');
...
async function loginAndNavigate(page: Page) {
  ...
  await page.waitForLoadState('domcontentloaded');
  await page.waitForSelector('.block-editor, [data-testid="block-editor"]', { timeout: 10000 }).catch(() => {});
}
