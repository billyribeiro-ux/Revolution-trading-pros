...
import { test as base, expect, Page, BrowserContext, Locator, APIRequestContext } from '@playwright/test';
...
export class BlockHelper {
  ...
  async addBlock(blockType: keyof typeof BLOCK_TYPES): Promise<Locator> {
    ...
    const searchInput = this.page.locator('input[placeholder*="Search"], .block-search-input').first();
    ...
    const blockOption = this.page.locator([
  ...
  async duplicateSelectedBlock(): Promise<Locator> {
    ...
    const duplicateButton = this.page.locator([
    ...
    await expect(this.page.locator('.block-wrapper, [data-testid="block"]')).toHaveCount(blockCount + 1);
  ...
  async editBlockContent(blockIndex: number, content: string): Promise<void> {
    ...
    const editableArea = block.locator([
...
export class EditorHelper {
  ...
  async waitForEditorReady(): Promise<void> {
    ...
    await this.page.waitForSelector('.loading, .skeleton, [data-loading="true"]', {
      state: 'hidden',
      timeout: 10000
  ...
  async setExcerpt(excerpt: string): Promise<void> {
    const excerptInput = this.page.locator([
      '#excerpt',
  ...
  async saveDraft(): Promise<void> {
    ...
    await this.page.waitForSelector('.save-success, [data-testid="save-success"]', {
      timeout: 10000
    }).catch(() => {});
  }
  ...
  async publish(): Promise<void> {
    ...
    await this.page.waitForSelector('.save-success, [data-testid="publish-success"]', {
      timeout: 10000
    }).catch(() => {});
  }
  ...
  async switchDevice(device: 'desktop' | 'tablet' | 'mobile'): Promise<void> {
    const deviceButton = this.page.locator(`[data-device="${device}"], button[aria-label="${device}"]`).first();
  ...
  async openAIAssistant(): Promise<void> {
    const aiButton = this.page.locator([
  ...
  async openSEOPanel(): Promise<void> {
    const seoButton = this.page.locator([
  ...
  async openRevisionHistory(): Promise<void> {
    const historyButton = this.page.locator([
      ...
      'button[aria-label="Revision history"]'
    ].join(', ')).first();
...
export class PerformanceHelper {
   ...
   */
  async measureBlockCreation(blockHelper: BlockHelper, blockType: keyof typeof BLOCK_TYPES): Promise<number> {
    const { duration } = await this.measureOperation(async () => {
...
export async function createTestPost(page: Page, options: {
  title: string;
  slug?: string;
