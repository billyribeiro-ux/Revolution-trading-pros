...
class WC_Report_Carts_By_Product extends WC_Admin_Report {
	...
	public function current_filters() {
		...
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'product_ids' ) ) . '">' . esc_html__(
			'Reset',
	...
	public function products_widget() {
		$range      = ! empty( $_GET['range'] ) ? sanitize_text_field( wp_unslash( $_GET['range'] ) ) : '';
		...
		$report     = ! empty( $_GET['report'] ) ? sanitize_text_field( wp_unslash( $_GET['report'] ) ) : '';
		?>
	...
	public function get_export_button() {
		...
		<a
			href="#"
			download="<?php echo esc_attr( sanitize_file_name($report_name) ); ?>"
			class="export_csv"
			data-export="chart"
			data-xaxes="<?php esc_attr_e( 'Date', 'woocommerce_cart_reports' ); ?>"
			data-groupby="<?php echo esc_attr( $this->chart_groupby ); ?>"
		>
			<?php esc_html_e( 'Export CSV', 'woocommerce_cart_reports' ); ?>
		</a>
		<?php
	}
						...
						var series = [
							{
								...
								color: '<?php echo esc_js( $this->chart_colours['item_counts'] ); ?>',
