...
class WC_Report_Carts_By_Date extends WC_Admin_Report {
	...
	public function get_main_chart() {
		...
		wp_enqueue_script(
			'date-chart',
			plugins_url( 'assets/js/date-chart.js', WC_CART_REPORTS_PLUGIN_FILE ),
			[ 'jquery' ],
			null,
		);
		...
		wp_localize_script( 'date-chart', 'date_chart_data', [
			'series'       => [
				[
					'label'      => esc_js( __( 'Number of conversions', 'woocommerce_cart_reports' ) ),
					'data'       => array_values( $data['order_counts'] ),
					'color'      => $this->chart_colours['order_counts'],
					'bars'       => [
						'fillColor' => $this->chart_colours['order_counts'],
						'fill'      => true,
						'show'      => true,
						'lineWidth' => 0,
				...
				[
					'label'      => esc_js( __( 'Abandoned Carts', 'woocommerce_cart_reports' ) ),
					'data'       => array_values( $data['abandoned_cart_count'] ),
					'color'      => $this->chart_colours['abandoned_carts'],
					'points'     => [
						'show'      => true,
						'radius'    => 5,
			...
			'month_names'  => $wp_locale->month_abbrev,
			'cart_groupby' => $this->chart_groupby,
		] );
		?>
