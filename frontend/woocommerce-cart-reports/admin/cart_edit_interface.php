...
class AV8_Edit_Interface {
	...
	public function tooltip_scripts() {
		global $pagenow;
		if ( is_admin() ) {
			if ( $pagenow == 'post.php' && isset( $_GET['post'] ) && get_post_type( get_post( absint( $_GET['post'] ) ) ) == 'carts' ) {
	...
	public function cart_status_customer_metabox() {
		...
		if ( $author_id > 0 ) {
			...
			printf( '<a href="%s">%s</a>', esc_url( $user_edit_url ), esc_html( $full_name ) );
		} elseif ( $full_name != false ) {
			if ( WP_DEBUG == true ) {
				assert( $full_name != '' && $full_name != ' ' );
			}
			printf( '<p>%s</p>', esc_html( $full_name ) );
			//Print out actions
			...
			if ( version_compare( WC_VERSION, '3.0', '<' ) ) {
				echo
				'<p>' . esc_html( ucwords( $order->billing_first_name . ' ' . $order->billing_last_name ) ) . ' (' . esc_html__(
						'Guest',
						'woocommerce_cart_reports'
				) . ')</p>';
			} else {
				echo '<p>' . esc_html( $order->get_formatted_billing_full_name() ) . ' (' . esc_html__( 'Guest', 'woocommerce_cart_reports' ) . ')</p>';
			}
		} else {
			echo '<span style="color: gray">' . esc_html__( 'Name Not Available', 'woocommerce_cart_reports' ) . '</span>' . wp_kses_post( av8_tooltip(
					esc_html__(
	...
	public function cart_status_metabox() {
		...
		echo '<div><p><strong>' . esc_html__( 'Created:', 'woocommerce_cart_reports' ) . '</strong><br/>' . esc_html( date(
			'F j, Y \a\t g:i a',
		...
		echo '<div id="edit_status"><mark class="color-wrapper ' . esc_attr( strtolower( $show_custom_state ) ) . '_edit">' . esc_html__(
			$show_custom_state,
			'woocommerce_cart_reports'
		) . wp_kses_post( $tooltip ) . '</mark></div>';
	}
