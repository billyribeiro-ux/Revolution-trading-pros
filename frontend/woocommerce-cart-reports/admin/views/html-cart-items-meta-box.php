		...
		<table cellpadding="0" width="100%" cellspacing="0" class="woocommerce_cart_reports_items">
			...
			<tr>
				...
				<th class="sku" style="text-align:left">
					<?php esc_html_e( 'SKU', 'woocommerce_cart_reports' ); ?>
				</th>
				<th class="name" style="text-align:left">
					<?php esc_html_e( 'Name', 'woocommerce_cart_reports' ); ?>
				</th>
				<th class="price" style="text-align:right">
					<?php esc_html_e( 'Price', 'woocommerce_cart_reports' ); ?>
				</th>
				<th class="quantity" style="text-align:right">
					<?php esc_html_e( 'Qty', 'woocommerce_cart_reports' ); ?>
				</th>
			</tr>
			</thead>
			...
			<?php endif; ?>
				...
				<tr class="item <?php echo esc_attr( $table_color ); ?>" rel="<?php echo esc_attr( $loop ); ?>">
					<td class="thumb">
						<a
							href="<?php echo esc_url( admin_url( 'post.php?post=' . $_product->get_id() . '&action=edit' ) ); ?>"
					...
					<td class="sku">
						...
						?>
						<input
							type="hidden" class="item_id" name="item_id[<?php echo esc_attr( $loop ); ?>]"
							value="<?php echo esc_attr( $item['id'] ?? '' ); ?>"
						/>
						<input
							type="hidden" name="item_name[<?php echo esc_attr( $loop ); ?>]"
							value="<?php echo esc_attr( $item['id'] ?? '' ); ?>"
						/>
						<?php if ( isset( $item['variation_id'] ) ) : ?>
							<input
								type="hidden" name="item_variation[<?php echo esc_attr( $loop ); ?>]"
								value="<?php echo esc_attr( $item['variation_id'] ); ?>"
							/>
						<?php endif; ?>
					</td>
					<td class="name">
						<a href="<?php echo esc_url( admin_url( 'post.php?post=' . $item['product_id'] . '&action=edit' ) ); ?>"><strong><?php echo esc_html( $_product->get_title() ); ?></strong></a>
						...
						if (
							...
							echo '&nbsp;' . wp_kses_post( $variation_data );
						}
						?>
			...
			<?php if ( $subtotal ): ?>
				...
				<tfoot>
				<tr>
					<th style="text-align: left"><strong><?php esc_html_e( 'Subtotal', 'woocommerce' ); ?></strong>
					</th>
					<th colspan="2"></th>
					<th style="text-align: right"><?php echo wp_kses_post( wc_price( $subtotal ) ); ?></th>
					<th style="text-align: right"><?php echo esc_html( $number_items ); ?></th>
				</tr>
				</tfoot>
			<?php endif; ?>
		</table>
