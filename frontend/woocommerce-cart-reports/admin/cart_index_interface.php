...
class AV8_Cart_Index_Page {
	...
	*/
	...
	public function woocommerce_carts_search_custom_fields( $wp ) {
		...
		// Add ID
		$search_order_id = str_replace( 'Order #', '', $search_term );
	...
	public function av8_manage_cart_columns( $column, $post_id = '' ) {
		...
		switch ( $column ) {
			...
			case 'post__in':
				...
				echo '<div class="index_status"><mark class="' . esc_attr( strtolower( $show_custom_state ) ) . '_index">' . esc_html__(
					$show_custom_state,
			...
			case 'products':
				...
				if ( count( $order_items ) > 0 && $order_items != false ) {
					...
					echo "<span style='color:lightgray;'>" . esc_html__(
							'No Products',
							'woocommerce_cart_reports'
						) . '</span>';
				}
	...
	*/
	public function restrict_manage_posts() {
		...
		if ( ( $pagenow == 'edit.php' ) && isset( $_GET['post_type'] ) && ( $_GET['post_type'] == 'carts' ) ) {
			...
			<label for="from">
				<?php esc_html_e( 'From:', 'woocommerce_cart_reports' ); ?>
			</label>
			<input
			...
			/>
			<label for="to">
				<?php esc_html_e( 'To:', 'woocommerce_cart_reports' ); ?>
			</label>
			<input
			...
			<script type="text/javascript">
              jQuery(function () {
                var dates = jQuery('#posts-filter #from, #posts-filter #to').datepicker({
                  defaultDate: '', dateFormat: 'yy-mm-dd', numberOfMonths: 1, maxDate: '+0D', showButtonPanel: true, showOn: 'both', buttonImage: "<?php echo esc_url( $woocommerce->plugin_url() ); ?>/assets/images/calendar.png", buttonImageOnly: true, onSelect: function (selectedDate) {
			...
			<select name="mv">
				<option value="">
					<?php echo esc_html__( 'Show All Carts', 'woocommerce_cart_reports' ); ?>
				</option>
				<?php
				foreach ( $status_options as $key => $value ) {
					...
					<option
						...
						<?php
						if ( isset( $_GET['mv'] ) ) {
							selected( sanitize_text_field( wp_unslash( $_GET['mv'] ) ), $value );
						}
						?>
					>
						<?php echo esc_html__( $key, 'woocommerce_cart_reports' ); ?>
					</option>
					<?php
				}
				?>
