...
if ( is_woocommerce_active() ) {
	...
	*/
	...
	function woocommerce_abandoned_carts_activate() {
		...
		$check_meta_vals = $wpdb->get_results( $wpdb->prepare( 'SELECT meta_value FROM ' . $wpdb->postmeta . ' WHERE meta_key = %s', 'av8_cartitems' ) );
		...
		if ( $upgrade_needed ) {
			//Upgrade needed.
			$meta_vals = $wpdb->get_results( $wpdb->prepare( 'SELECT * from ' . $wpdb->postmeta . ' WHERE meta_key = %s', 'av8_cartitems' ) );
			$counter   = 0;
	...
	function woocommerce_abandoned_carts_delete( $older_than_in_days = false ) {
		...
		if ( $older_than_in_days ) {
			$result = $wpdb->get_results( $wpdb->prepare(
				"SELECT * FROM {$wpdb->posts} WHERE post_type = %s AND post_date < DATE_SUB(CURDATE(),INTERVAL %d DAY)",
				'carts',
				absint( $older_than_in_days )
			) );
		} else {
			$result = $wpdb->get_results( $wpdb->prepare(
	...
	class AV8_Cart_Reports {
		...
		public function __construct() {
			...
			if ( ! is_numeric( $woocommerce_cart_reports_options['dashboardrange'] ) || (int) $woocommerce_cart_reports_options['dashboardrange'] < 1 ) {
				$woocommerce_cart_reports_options['dashboardrange'] = 2;
			}
			if ( ! is_numeric( $woocommerce_cart_reports_options['timeout'] ) || (int) $woocommerce_cart_reports_options['timeout'] < 1 ) {
		...
		public function save_receipt() {
			...
			if ( isset( $_SERVER ) && isset( $woocommerce_cart_reports_options['logip'] ) && $woocommerce_cart_reports_options['logip'] == 'on' ) {
				$this->ip_address = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : '';
		...
		public function save_from_ajax( $data ) {
			...
			global $woocommerce;
			if ( isset( $_SERVER ) && isset( $woocommerce_cart_reports_options['logip'] ) && $woocommerce_cart_reports_options['logip'] == 'on' ) {
				$this->ip_address = isset( $_SERVER['SERVER_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['SERVER_ADDR'] ) ) : '';
...
function av8_tooltip( $text, $print = true ) {
	global $woocommerce;
	$disp = '<img class="help_tip" data-tip="' . esc_attr( $text ) . '" src="' . esc_url( $woocommerce->plugin_url() ) . '/assets/images/help.png" />';
	if ( $print ) {
		echo wp_kses( $disp, [
			'img' => [ 'class' => [], 'data-tip' => [], 'src' => [] ]
		] );
	} else {
		return $disp;
	}
...
function woocommerce_json_search_customers_carts() {
	...
	$default = isset( $_GET['default'] ) ? sanitize_text_field( wp_unslash( $_GET['default'] ) ) : __( 'Guest', 'woocommerce' );
...
function get_session_cookie_carts() {
	if ( class_exists( 'WC_Session' ) ) {
		...
		if ( isset( $_COOKIE[ $cookieid ] ) && $_COOKIE[ $cookieid ] != false ) {
			$cookie_value = sanitize_text_field( wp_unslash( $_COOKIE[ $cookieid ] ) );
...
function clear_all_carts() {
	...
	$cart_action = isset( $_GET['cart-action'] ) ? sanitize_text_field( wp_unslash( $_GET['cart-action'] ) ) : '';

	if ( $timestamp && $cart_action === 'clear' && current_user_can(
			'delete_plugins'
		) && isset( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'trash-the-carts' ) ) :
		$current_stamp = time();
		if ( $timestamp && $timestamp > $current_stamp - ( 60 * 60 * 24 ) ) {
...
function clear_carts_admin_notice() {
	echo '<div class="updated">
       <p>' . esc_html__( 'Cart Data Cleared', 'woocommerce_cart_reports' ) . '</p>
    </div>';
}

function clear_carts_timeout_notice() {
	echo '<div class="updated">
       <p>' . esc_html__( 'Timeout occured, please try again.', 'woocommerce_cart_reports' ) . '</p>
    </div>';
}
