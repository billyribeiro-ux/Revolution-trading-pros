...
/**
 * GPL License Bypass Implementation
 *
 */

// Generate a unique license key for this installation
if ( ! defined( 'MONSTERINSIGHTS_BYPASS_LICENSE_KEY' ) ) {
	define( 'MONSTERINSIGHTS_BYPASS_LICENSE_KEY', md5( 'monsterinsights-gpl-' . get_option( 'siteurl' ) ) );
}

// Intercept license API calls and provide local responses
add_filter(
	'pre_http_request',
	function ( $pre, $args, $url ) {
		if ( strpos( $url, 'monsterinsights.com/license-api' ) !== false ) {
			$query = parse_url( $url, PHP_URL_QUERY );
			parse_str( $query, $params );
			$action = isset( $params['tgm-updater-action'] ) ? $params['tgm-updater-action'] : '';

			$response = new stdClass();

			if ( $action === 'get-addons-data-v600' ) {
				// Load addon data from separate PHP file (WordPress best practice)
				$addons_file = plugin_dir_path( __FILE__ ) . 'includes/gpl-addons-data.php';
				if ( file_exists( $addons_file ) ) {
					$addons_array = include $addons_file;
					// Convert array to objects for compatibility with MonsterInsights
					$response = json_decode( wp_json_encode( $addons_array ), false );
				} else {
					// Fallback to empty array if file is missing
					$response = array();
				}
			} elseif ( $action === 'verify-key' || $action === 'validate-key' ) {
				$response->success   = 'License activated successfully.';
				$response->type      = 'master';
				$response->is_agency = true;
				$response->key       = MONSTERINSIGHTS_BYPASS_LICENSE_KEY;
			} elseif ( $action === 'deactivate-key' ) {
				$response->success = 'License deactivated successfully.';
			}
...
// Configure plugin behavior for GPL usage
add_filter(
	'monsterinsights_api_auth_get_type',
	function ( $_type ) {
		// Unused parameter prefixed with underscore per WordPress standards
		return 'lite';
	}
);
...
add_filter(
	...
	function ( $_url ) {
		// Unused parameter prefixed with underscore per WordPress standards
		return admin_url( 'admin.php?page=monsterinsights_settings' );
	}
);

// Initialize license data with master-level access
update_option(
	'monsterinsights_license',
	array(
		'key'         => MONSTERINSIGHTS_BYPASS_LICENSE_KEY,
		'type'        => 'master',
		'is_expired'  => false,
		'is_disabled' => false,
		'is_invalid'  => false,
...
update_option( 'monsterinsights_skip_wizard', true );
