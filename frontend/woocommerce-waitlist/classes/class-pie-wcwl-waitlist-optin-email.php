...
if ( ! class_exists( 'Pie_WCWL_Waitlist_Optin_Email' ) ) {
	...
	class Pie_WCWL_Waitlist_Optin_Email extends WC_Email {
		...
		/**
		 * Product IDs to pass to the email template
		 * 
		 * @var array
		 */
		...
		/**
		 * Trigger function for the optin class
		 *
		...
		public function trigger( $email, $product_id, $products, $lang ) {
			...
			if ( ! $product ) {
				/* translators: %s: email */
				wcwl_add_log( sprintf( __( 'Failed to send waitlist optin notification for %s: No valid product found.' ), $email ) );
			}
			if ( $woocommerce_wpml ) {
				$this->language = $lang;
			...
			if ( ! $this->get_recipient() ) {
				...
				wcwl_add_log( 'woocommerce_waitlist', sprintf( __( 'Failed to send waitlist optin notification for %s. Invalid email address found.' ), $product->get_id() ) );
			}
		...
		public function get_content_html() {
			...
			/**
			 * Filter the expiration time for the double opt-in key
			 * 
			 * @since 2.4.0
			 */
