	...
	class Pie_WCWL_Waitlist_Settings {
		...
		public function enqueue_settings_scripts( $hook ) {
			if ( 'woocommerce_page_wc-settings' == $hook && $this->is_waitlist_settings_page() ) {
				...
				wp_enqueue_script( 'wcwl_admin_settings', WCWL_ENQUEUE_PATH . '/includes/js/src/wcwl_admin_settings.min.js', array(), WCWL_VERSION ); // @phpstan-ignore constant.notFound
		...
		protected function get_script_data() {
			return array(
				...
				/* translators: %s link to WooCommerce support */
				...
				/* translators: %1$s <b> tag opens, %2$s <b> tag closes */
				...
				/* translators: %1$s <a> tag opens and links to WooCommerce exporter, %2$s <a> tag closes */
				'export_text'                    => sprintf( __( 'Download a comma-separated list of all waitlisted and archived emails for ALL products.  If you want to export waitlist data for importing we recommend you use the %1$sWooCommerce Product Exporter%2$s.', 'woocommerce-waitlist' ), '<a href="' . admin_url( 'edit.php?post_type=product&page=product_exporter' ) . '">', '</a>' ),
				/**
				 * Filter the CSV export header
				 * 
				 * @since 2.4.0
				 */
				...
				/* translators: %s HTML for update counts notice */
				...
				/* translators: %s HTML for update meta notice */
		...
		public function add_filter_for_new_account_email_description() {
			if ( ! isset( $_REQUEST['page'] ) || 'wc-settings' != $_REQUEST['page'] || ! isset( $_REQUEST['tab'] ) || 'email' != $_REQUEST['tab'] || ! isset( $_REQUEST['section'] ) || 'wc_email_customer_new_account' != $_REQUEST['section'] ) {
				...
				add_filter( 'gettext', array( $this, 'filter_new_account_email_description' ), 20 );
			}
		}
