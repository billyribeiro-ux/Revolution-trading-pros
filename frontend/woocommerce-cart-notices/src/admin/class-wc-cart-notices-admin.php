...
class WC_Cart_Notices_Admin {
	...
	/** @var array notice types to display name -- do not access this property directly; use get_notice_types() instead */
	...
	/**
	 * Magic getter to get the notice_types property.
	 * The notice_types property was previously public and has been made protected. This method exists to provide backwards compatibility.
	 *
	 * @param string $name
	 * @return array|null
	 */
	...
	private function validate_options() {
		...
		if ( 'cart_notice_new' === $_POST['action'] ) {
			...
			if ( ! $_POST['notice_type'] || ! isset( $notice_types[ $_POST['notice_type'] ] ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$this->message_handler->add_error( __( 'You must choose a Notice Type', 'woocommerce-cart-notices' ) );
			}
		}

		// notice name is required
		if ( ! $_POST['notice_name'] ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( $wpdb->get_var( $name_exists_query ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$this->message_handler->add_error( __( 'That name is already in use', 'woocommerce-cart-notices' ) );
		}

		// validate target amount, if set
		if ( $_POST['minimum_order_amount'] && ( ! is_numeric( $_POST['minimum_order_amount'] ) || (float) $_POST['minimum_order_amount'] < 0 ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( $_POST['threshold_order_amount'] && ( ! is_numeric( $_POST['threshold_order_amount'] ) || (float) $_POST['threshold_order_amount'] < 0 ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( $_POST['deadline_hour'] && ( ! is_numeric( $_POST['deadline_hour'] ) || (int) $_POST['deadline_hour'] < 1 || (int) $_POST['deadline_hour'] > 24 ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	private function handle_create_update_cart_notice( $action ) {
		...
		if ( $this->message_handler->error_count() > 0 ) {
			...
			return wp_safe_redirect( esc_url_raw( add_query_arg( $query_params, "admin.php" ) ) );
		}
		...
		if ( 'create' === $action ) {
			...
			return wp_safe_redirect( esc_url_raw( add_query_arg( array( "page" => wc_cart_notices()->id, 'tab' => 'list', "result" => "created" ), 'admin.php' ) ) );
			...
			return wp_safe_redirect( esc_url_raw( add_query_arg( array( "page" => wc_cart_notices()->id, 'id' => $id, 'tab' => 'edit', "result" => "updated" ), 'admin.php' ) ) );
		}
	}
	...
	public function toggle_cart_notice() {
		...
		if ( isset( $_GET['page'] ) && wc_cart_notices()->id === $_GET['page'] && current_user_can( 'manage_woocommerce' ) ) {
			...
			if ( 'enable' === $action ) {
				...
				wp_safe_redirect( esc_url_raw( add_query_arg( array( 'page' => wc_cart_notices()->id, 'result' => 'enabled' ), 'admin.php' ) ) );
				exit;
				...
				wp_safe_redirect( esc_url_raw( add_query_arg( array( 'page' => wc_cart_notices()->id, 'result' => 'disabled' ), 'admin.php' ) ) );
				exit;
				...
				wp_safe_redirect( esc_url_raw( add_query_arg( array( 'page' => wc_cart_notices()->id, 'result' => 'deleted' ), 'admin.php' ) ) );
				exit;
			}
		}
	}
	...
	public function wc_cart_notices_options() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'woocommerce-cart-notices' ) );
		}
		...
		if ( 'list' === $tab ) {
			...
			if ( ! $notice ) {
				wp_die( esc_html__( 'The requested data could not be found!', 'woocommerce-cart-notices' ) );
			}
	...
	private function load_notice_from_request() {
		...
		$notice = (object) array(
			...
			'data'       => array(
				...
				'deadline_days'        => is_string( $this->get_requested_value( 'deadline_days' ) ) ? unserialize( $this->get_requested_value( 'deadline_days' ) ) : $this->get_requested_value( 'deadline_days' ),
				'referer'              => $this->get_requested_value( 'referer' ),
				'product_ids'          => is_string( $this->get_requested_value( 'product_ids' ) ) ? unserialize( $this->get_requested_value( 'product_ids' ) ) : $this->get_requested_value( 'product_ids' ),
				'hide_product_ids'     => is_string( $this->get_requested_value( 'hide_product_ids' ) ) ? unserialize( $this->get_requested_value( 'hide_product_ids' ) ) : $this->get_requested_value( 'hide_product_ids' ),
				'shipping_countries'   => is_string( $this->get_requested_value( 'shipping_countries' ) ) ? unserialize( $this->get_requested_value( 'shipping_countries' ) ) : $this->get_requested_value( 'shipping_countries' ),
				'minimum_quantity'     => $this->get_requested_value( 'minimum_quantity' ),
				'maximum_quantity'     => $this->get_requested_value( 'maximum_quantity' ),
				'category_ids'         => is_string( $this->get_requested_value( 'category_ids' ) ) ? unserialize( $this->get_requested_value( 'category_ids' ) ) : $this->get_requested_value( 'category_ids' ),
				'hide_category_ids'    => is_string( $this->get_requested_value( 'hide_category_ids' ) ) ? unserialize( $this->get_requested_value( 'hide_category_ids' ) ) : $this->get_requested_value( 'hide_category_ids' ),
			),
		);
	...
	private function get_requested_value( $name ) {
		...
		if ( isset( $_REQUEST[ $name ] ) ) {
			...
			if ( is_string( $_REQUEST[ $name ] ) ) {
				return stripslashes( sanitize_text_field( $_REQUEST[ $name ] ) );
			} else {
				return wc_clean( $_REQUEST[ $name ] );
			}
		}
