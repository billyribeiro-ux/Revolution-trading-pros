...
<div class="wrap woocommerce">
	...
	<?php if ( isset( $_GET['result'] ) ) : /* show any action messages */ ?>
		...
		<div id="message" class="updated">
			<p><strong><?php echo esc_html__( 'Cart Notice', 'woocommerce-cart-notices' ) . ' ' . esc_html( sanitize_text_field( $_GET['result'] ) ); ?></strong></p>
		</div>
					...
					<?php foreach ( $notices as $notice ) : ?>
						...
						<tr scope="row" class="<?php echo $notice->enabled ? 'active' : 'inactive' ?>">
							<td class="post-title column-title">
								<strong>
									<a class="row-title" href="admin.php?page=<?php echo esc_attr( wc_cart_notices()->id ); ?>&amp;tab=edit&amp;id=<?php echo esc_attr( $notice->id ); ?>">
										<?php echo esc_html( stripslashes( $notice->name ) ); ?>
									</a>
								</strong>
								<div class="row-actions">
									<span class="edit">
										<a href="admin.php?page=<?php echo esc_attr( wc_cart_notices()->id ); ?>&amp;tab=edit&amp;id=<?php echo esc_attr( $notice->id ); ?>">
											<?php esc_html_e( 'Edit', 'woocommerce-cart-notices' ); ?>
										</a>
									</span>
									|
									<span class="enable">
										<a href="admin.php?page=<?php echo esc_attr( wc_cart_notices()->id ); ?>&amp;action=<?php echo esc_attr( $notice->enabled ? 'disable' : 'enable' ); ?>&amp;id=<?php echo esc_attr( $notice->id ); ?>">
											<?php echo $notice->enabled ? esc_html__( 'Disable', 'woocommerce-cart-notices' ) : esc_html__( 'Enable', 'woocommerce-cart-notices' ); ?>
										</a>
									</span>
									|
									<span class="trash">
										<a onclick="return confirm( 'Really delete this entry?' );" href="admin.php?page=<?php echo esc_attr( wc_cart_notices()->id ); ?>&amp;action=delete&amp;id=<?php echo esc_attr( $notice->id ); ?>">
							...
							<td>
								...
								switch ( $notice->type ) :
									...
									case 'minimum_amount':
										...
										echo sprintf( esc_html__( 'Target amount: %s', 'woocommerce-cart-notices' ), wc_cart_notices()->get_minimum_order_amount( $notice ) ? esc_html( get_woocommerce_currency_symbol() . wc_cart_notices()->get_minimum_order_amount( $notice ) ) : 'none configured' ) . '<br/>';
										/* translators: %s - formatted amount quantity */
										echo sprintf( esc_html__( 'Threshold amount: %s', 'woocommerce-cart-notices' ), isset( $notice->data['threshold_order_amount'] ) ? esc_html( get_woocommerce_currency_symbol() . $notice->data['threshold_order_amount'] ) : 'none configured' );

									break;

									case 'deadline':

										echo sprintf( esc_html__( 'Deadline Hour: %s', 'woocommerce-cart-notices' ), $notice->data['deadline_hour'] ? esc_html( $notice->data['deadline_hour'] ) : '<em>' . esc_html_x( 'none', 'No deadline hour', 'woocommerce-cart-notices' ) . '</em>' ) . '<br/>';
										echo sprintf( esc_html__( 'Active Days: %s', 'woocommerce-cart-notices' ), $notice->data['deadline_days_names'] ? esc_html( implode( ', ', $notice->data['deadline_days_names'] ) ) : '<em>' . esc_html_x( 'none', 'No active days', 'woocommerce-cart-notices' ) . '</em>' );
									...
									case 'referer':
										...
										echo sprintf( esc_html__( 'Referring Site: %s', 'woocommerce-cart-notices' ), $notice->data['referer'] ? esc_html( $notice->data['referer'] ) : '<em>' . esc_html_x( 'none', 'No referring site', 'woocommerce-cart-notices' ) . '</em>' );

									break;

									case 'products':

										echo sprintf( esc_html__( 'Products: %s', 'woocommerce-cart-notices' ), $notice->data['products'] ? esc_html( implode( ', ', $notice->data['products'] ) ) : '<em>' . esc_html_x( 'none', 'No products', 'woocommerce-cart-notices' ) . '</em>' );

										if ( isset( $notice->data['minimum_quantity'] ) && '' !== $notice->data['minimum_quantity'] ) {
											echo '<br/>' . sprintf( esc_html__( 'Minimum quantity: %s', 'woocommerce-cart-notices' ), esc_html( $notice->data['minimum_quantity'] ) );
										}

										if ( isset( $notice->data['maximum_quantity'] ) && '' !== $notice->data['maximum_quantity'] ) {
											echo '<br/>' . sprintf( esc_html__( 'Maximum quantity: %s', 'woocommerce-cart-notices' ), esc_html( $notice->data['maximum_quantity'] ) );
										}

									break;

									case 'categories':
										echo sprintf( esc_html__( 'Categories: %s', 'woocommerce-cart-notices' ), $notice->data['categories'] ? esc_html( implode( ', ', $notice->data['categories'] ) ) : '<em>' . esc_html_x( 'none', 'No categories', 'woocommerce-cart-notices' ) . '</em>' );
									break;
	...
	<?php elseif ( 'new' === $tab || 'edit' === $tab ) : ?>
			...
			<table class="form-table">
				<tbody>
					...
					<tr valign="top">
							...
							<span class="description">
								...
								<ul>
									...
									foreach( $shortcodes as $shortcode => $help_tip ) {
										...
										echo '<li class="shortcode"><strong>' . esc_html( $shortcode ) . '</strong> ' . wc_help_tip( $help_tip ) . '</li>';
									}
						...
						<td>
							<textarea name="notice_message" id="notice_message" rows="12" cols="80"><?php echo is_string( $notice->message ) ? esc_textarea( $notice->message ) : ''; ?></textarea>
						</td>
					</tr>
						...
						<td>
							...
							<span class="description">
								...
								echo sprintf( esc_html__( 'Target cart total for the notice - the customer\'s cart total must be %1$sless than%2$s the target to display the notice. Leave blank to use the "Minimum order amount" from %3$san enabled Free shipping method%4$s instead.', 'woocommerce-cart-notices' ),
									'<strong>', '</strong>',
									'<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=shipping' ) ) . '">',
									'</a>'
								); ?>
							</span>
						</td>
					...
					<tr valign="top" class="deadline_notice_data notice_data" style="<?php echo 'deadline' !== $notice->type ? 'display:none;' : ''; ?>">
						...
						<td>
							...
							foreach ( $days as $key => $name ) :
								...
								echo '<input id="' . esc_attr( $input_name ) . '" name="deadline_days[' . esc_attr( $key ) . ']" type="checkbox" value="1" ' . checked( $value, 1, false ) . ' />';
					...
					<tr valign="top" class="categories_notice_data notice_data" style="<?php echo 'categories' !== $notice->type ? 'display:none;' : ''; ?>">
						...
						<td>
							...
							<select
								...
								data-nonce="<?php echo esc_attr( wp_create_nonce( 'search-product-categories' ) ); ?>"
		...
		<?php if ( 'edit' === $tab ) : ?>
			...
			<?php if ( isset( $example_notice ) ) : ?>
				...
				<h3><?php esc_html_e( 'Example Notice', 'woocommerce-cart-notices' ); ?></h3>
					...
					if ( 'minimum_amount' === $notice->type ) {
						...
						if ( is_numeric( $minimum_order_amount ) && ! is_numeric( $threshold_order_amount ) ) {
							...
							printf(
								...
								esc_html__( 'With the current configuration your cart notice will display when the order total is less than %s and will resemble:', 'woocommerce-cart-notices' ),
								esc_html( get_woocommerce_currency_symbol() . $minimum_order_amount )
							);
							...
							printf(
								...
								esc_html__( 'With the current configuration your cart notice will display when the order total is greater than or equal to %s and will resemble:', 'woocommerce-cart-notices' ),
								'<strong>' . esc_html( get_woocommerce_currency_symbol() . $threshold_order_amount ) . '</strong>'
							);
							...
							printf(
								...
								esc_html__( 'With the current configuration your cart notice will display when the order total is between %1$s and %2$s and will resemble:', 'woocommerce-cart-notices' ),
								'<strong>' . esc_html( get_woocommerce_currency_symbol() . $threshold_order_amount ) . '</strong>',
								'<strong>' . esc_html( get_woocommerce_currency_symbol() . $minimum_order_amount ) . '</strong>'
							);
				...
				if ( $example_notice ) {
					echo $example_notice; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				} else {
		...
		<script type="text/javascript">
			...
			var default_messages = {
				'minimum_amount' : '<?php echo wp_kses_post( __( 'Add <strong>{amount_under}</strong> to your cart in order to receive free shipping!', 'woocommerce-cart-notices' ) ); ?>',
				'deadline' : '<?php echo wp_kses_post( __( 'Order within the next <strong>{time}</strong> and your order ships today!', 'woocommerce-cart-notices' ) ); ?>'
			};
