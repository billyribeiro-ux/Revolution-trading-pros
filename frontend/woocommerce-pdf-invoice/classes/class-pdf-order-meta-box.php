<?php
	...
	class WC_pdf_order_meta_box {
		...
		function invoice_details_admin_init( $post_type, $post ) {
			...
			add_meta_box( 'woocommerce-invoice-details', __('Invoice Details', 'woocommerce-pdf-invoice'), array( $this,'woocommerce_invoice_details_meta_box'), array('shop_order' ), 'side', 'high');

			add_meta_box( 'woocommerce-invoice-details', __('Invoice Details', 'woocommerce-pdf-invoice'), array( $this,'woocommerce_invoice_details_meta_box_hpos'), array( 'woocommerce_page_wc-orders' ), 'side', 'high');
		...
		function woocommerce_invoice_details_meta_box( $post ) {
			...
			<div class="invoice_details_group">
				<ul>
					...
					<li class="left"><p>
						<?php esc_html_e( 'Invoice Number:', 'woocommerce-pdf-invoice' ); ?>
						<?php if ( isset( $invoice_meta['invoice_number_display'] ) ) {
								echo wp_kses_post( $invoice_meta['invoice_number_display'] ); 
							  } elseif( $order->get_meta( '_invoice_number_display', TRUE ) ) {
							  	echo wp_kses_post( $order->get_meta( '_invoice_number_display', TRUE ) ); 
							  }
						?>
					</p></li>
		
					<li class="left"><p>
						<?php esc_html_e( 'Invoice Date:', 'woocommerce-pdf-invoice' ); ?>
						<?php
						if ( isset( $invoice_meta['invoice_date'] ) ) {
							echo wp_kses_post( $invoice_meta['invoice_date'] );
						} elseif( $order->get_meta( '_invoice_date', TRUE ) ) {
							echo wp_kses_post( $order->get_meta( '_invoice_date', TRUE ) );
						}
	                ...
	                <li>
						<p><a class="pdf_invoice_metabox_download_invoice" href="<?php echo wp_kses_post( $pdf_invoice_download_link ); ?>"><?php esc_html_e( 'Download Invoice', 'woocommerce-pdf-invoice' ); ?></a></p>
					</li>

					<li>
						<p><a class="pdf_invoice_metabox_send_invoice" href="<?php echo wp_kses_post( $pdf_invoice_email_link ); ?>"><?php esc_html_e( 'Email Invoice', 'woocommerce-pdf-invoice' ); ?></a></p>
					</li>
	    ...
	    function pdf_invoice_woocommerce_order_actions( $orderactions, $order ) {
			...
			if( in_array( $allowed_user_role, $current_user->roles ) ) {
				...
				// HPOS
				if( isset( $_GET['page'] ) && wc_clean( $_GET['page'] ) == 'wc-orders' ) {
