<?php
    ...
    class WC_send_pdf {
        ...
        function init() {
			...
			add_filter( 'woocommerce_email_attachments' , array( $this, 'pdf_attachment' ), 9999, 3 );
        }
         ...
         */
	 	...
	 	public static function pdf_download_link( $order, $sent_to_admin = false, $plain_text = false, $email = '' ) {
	 		...
	 		$placeholder_replacement = apply_filters( 'pdf_invoice_download_invoice_link', $placeholder_replacement, $order, $order_id );

	 		echo wp_kses_post( $placeholder_replacement );
		...
		public static function get_woocommerce_pdf_invoice( $order = NULL, $email_id = NULL, $stream = NULL ) {
			...
			// Add PDF extension, required to correct errors made when using filename filter 
			 ...
			 */
	  		if( isset( $settings["pdf_debug"] ) && $settings["pdf_debug"] == "true" ) {
	  			...
	  			// Additional Logging, check if invoice is an attachment or download
	  			if( is_null( $email_id ) ) {
	  				$log_note = 'download';
	  			} else {
	  				$log_note = $email_id;
	  			...
	  			WC_pdf_debug::pdf_debug( $messagetext, 'WC_PDF_Invoice_' . $log_note, __('PDF Invoice Body : ', 'woocommerce-pdf-invoice'), TRUE );
	  			
	  			// Log file size
	  			self::log_pdf( "PDF content size : " . strlen( $messagetext ) );

	  			// Additional logging for 0 $messagetext length
	  			if( strlen( $messagetext ) == 0 ) {
	  				self::log_pdf( 'Order_id : ' . $order_id . ' Email_id : ' . $email_id );
	  			}
			...
			if( isset( $settings["pdf_generator"] ) && $settings["pdf_generator"] == 'MPDF' ) {
				...
				// Check if PDF already exists and method is attachment
				if( file_exists( $file ) && isset( $stream ) && $stream === FALSE ) {
					return $file;
				}
				...
				// Check if PDF already exists and method is attachment
				if( file_exists( $pdftemp . '/' . $filename ) ) {
					self::log_pdf( "PDF already in temp folder, let's reuse it." );
					return $pdftemp . '/' . $filename;
				} else {
					self::log_pdf( "PDF not in temp folder, let's create it" );
				}
		...
		private static function create_filename( $order_id ) {
			...
			if ( $filename == '' ) {
				...
				$filename	= str_replace( '{{month}}',	gmdate( 'F', strtotime( $invoice_date ) ) , $filename );
				$filename	= str_replace( '{{mon}}',	gmdate( 'M', strtotime( $invoice_date ) ) , $filename );
				$filename	= str_replace( '{{year}}',	gmdate( 'Y', strtotime( $invoice_date ) ) , $filename );
		...
		function get_woocommerce_invoice_content( $order_id, $email_id = NULL ) {
			...
			// Additional logging, place in the process
			self::log_pdf( 'get_woocommerce_invoice_content 01 - Order_id : ' . $order_id . ' Email_id : ' . $email_id );

			// Buffer
			ob_start();

			// load_template( $pdftemplate, false );
			load_template( WC_send_pdf::get_pdf_template( 'template', $order_id, $email_id ) ); // phpcs:ignore audit.php.lang.security.file.inclusion-arg
			...
			// Additional Logging - content before str_replace
			WC_pdf_debug::pdf_debug( $content, 'WC_PDF_Invoice_' . time() . ' ' . $email_id, __('Invoice Template : ', 'woocommerce-pdf-invoice'), TRUE );
			...
			// Additional logging, place in the process
			self::log_pdf( 'get_woocommerce_invoice_content 02 - ' . strlen( $content ) );
			...
			// Additional logging, place in the process
			self::log_pdf( 'get_woocommerce_invoice_content 03 - ' . strlen( mb_encode_numericentity( $content, [0x80, 0x10FFFF, 0, ~0], 'UTF-8' ) ) );
		...
		public static function get_invoice_companydetails( $order_id, $settings, $order ) {
			...
			if ( !isset( $pdf_company_detailspdf_company_details ) || $pdf_company_details == '' ) {
				...
				$pdf_company_details = nl2br( $pdf_company_details );
			}
				...
				foreach ( $order->get_items() as $item ) {
					...
					if ( $item['quantity'] ) {
						...
						foreach ( $item->get_formatted_meta_data() as $meta_key => $meta ) {
							$meta_display .= '<br /><small>(' . $meta->display_key . ':' . wp_kses_post( wp_strip_all_tags( $meta->display_value ) ) . ')</small>';
		 				}
		 				...
		 				if ( class_exists( 'WC_Booking_Data_Store' ) ) {
							...
							if ( $booking_ids ) {
								foreach ( $booking_ids as $booking_id ) {
									...
									$label    = $product && is_callable( array( $product, 'get_resource_label' ) ) && $product->get_resource_label() ? $product->get_resource_label() : __( 'Type', 'woocommerce-pdf-invoice' );
									...
									if ( $resource ) {
										/* translators: 1:Booking lable, 2:Booking Resource */
										$meta_display .= '<br /><small>' . esc_html( sprintf( __( '%1$s: %2$s', 'woocommerce-pdf-invoice' ), $label, $resource->get_name() ) ) . '</small>';
									}
										...
										if ( $product->has_person_types() ) {
											...
											if ( ! empty( $person_types ) && is_array( $person_types ) ) {
												foreach ( $person_types as $person_type ) {
													...
													/* translators: 1:Name, 2:Customer ID */
											...
											$meta_display .= '<br /><small>Persons : ' . esc_html( sprintf( __( '%d Persons', 'woocommerce-pdf-invoice' ), array_sum( $booking->get_person_counts() ) ) ) . '</small>';
		 				...
		 				if ( class_exists( 'WC_Booking_Data_Store' ) ) {
							...
							if ( $booking_ids ) {
								foreach ( $booking_ids as $booking_id ) {
									...
									if ( $resource ) {
										/* translators: 1:Booking label, 2:Name */
										$meta_display .= '<br /><small>' . esc_html( sprintf( __( '%1$s: %2$s', 'woocommerce-pdf-invoice' ), $label, $resource->get_name() ) ) . '</small>';
									}
		...
		public static function get_pdf_custom_heading( $order, $text ) {
			return apply_filters( 'woocommerce_pdf_invoice_' . strtolower( str_replace( ' ', '_', $text ) ) . '_heading', esc_html( $text ), $order );
		}
		...
		public static function get_woocommerce_invoice_terms( $page_id = 0, $order_id = 0 ) {
			...
			require( $termstemplate ); // nosemgrep audit.php.lang.security.file.inclusion-arg
