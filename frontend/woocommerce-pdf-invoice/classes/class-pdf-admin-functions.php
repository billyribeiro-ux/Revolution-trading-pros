...
class WC_pdf_admin_functions {
	...
	function invoice_number_admin_init( $column_id, $order = NULL ) {
		...
		if ( $column_id == 'pdf_invoice_num' ) {
			...
			if ( $order->get_meta( '_invoice_number_display', TRUE ) ) {
				...
				$output 		=  '<a href="' . $url . '&pdfid=' . $order->get_id() .'">' . $invoice_number . '<br />' . $invoice_date . '</a>';
				 ...
				 * $post->ID: $order_id
				 */
				echo apply_filters( 'pdf_invoice_number_order_screen_output', $output, $invoice_number, $invoice_date, $order->get_id() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    	...
    	if( in_array( $action, $supported_actions ) ) {
	    	...
	    	switch( $action ) {
	    		...
	    		case 'pdf_email_invoice':
	    			$redirect_args = $this->handle_email_invoice( $redirect_to, $action, $ids );
	    			break;
	...
	public function bulk_admin_notices() {
		...
		switch ( $bulk_action ) {
		    case 'handle_create_invoice':
		    	/* translators: formatted number of invoices created */
		    ...
		    case 'handle_create_email_invoice':
		    	/* translators: formatted number of orders queued for invoicing */
		    ...
		    case 'handle_email_invoice':
		    	/* translators: formatted number of invoices queued for emailing */
		    ...
		    case 'handle_delete_invoice':
		    	/* translators: formatted number of invoices queued for deleting */
		    ...
		    case 'handle_update_invoice_meta':
		    	/* translators: formatted number of invoices updated */
		    ...
		    case 'handle_bulk_export':
                ...
                if( isset( $zip_status ) && $zip_status == 0 ) {
                    ...
                    echo '<div class="updated"><p>' . wp_kses_post( sprintf( _n( '%s invoice added to zip file.', '%s invoices added to zip file.', $exported, 'woocommerce-pdf-invoice' ), number_format_i18n( $exported ) ) ) . '</p><p>' . wp_kses_post( sprintf( __( 'Download the zipfile from <a href="%1$s/%2$s.zip">%3$s.zip</a>', 'woocommerce-pdf-invoice' ), $upload_dir['baseurl'] . '/woocommerce_pdf_invoice', $zip_file, $zip_file ) ) . '</p></div>';
                            
                } else {
                    echo '<div class="error"><p>' . esc_html__('Zip file creation failed. Check the log in the WooCommerce System Status logs tab.', 'woocommerce-pdf-invoice') . '</p></div>';
                }
				break;
	...
	public static function restrict_manage_subscriptions() {
		...
		<select name='shop_order_subtype' id='dropdown_shop_order_subtype'>
			<option value=""><?php esc_html_e( 'All orders types', 'woocommerce-pdf-invoice' ); ?></option>
			<?php
			$order_types = apply_filters( 'woocommerce_subscriptions_order_type_dropdown', array(
				'original'    => _x( 'Original', 'An order type', 'woocommerce-pdf-invoice' ),
				'parent'      => _x( 'Subscription Parent', 'An order type', 'woocommerce-pdf-invoice' ),
				'renewal'     => _x( 'Subscription Renewal', 'An order type', 'woocommerce-pdf-invoice' ),
				'resubscribe' => _x( 'Subscription Resubscribe', 'An order type', 'woocommerce-pdf-invoice' ),
				'switch'      => _x( 'Subscription Switch', 'An order type', 'woocommerce-pdf-invoice' ),
				'regular'     => _x( 'Non-subscription', 'An order type', 'woocommerce-pdf-invoice' ),
			) );

			foreach ( $order_types as $order_type_key => $order_type_description ) {
				echo '<option value="' . esc_attr( $order_type_key ) . '"';

				if ( isset( $_GET['shop_order_subtype'] ) && wc_clean( wp_unslash( $_GET['shop_order_subtype'] ) ) ) {
					selected( $order_type_key, wc_clean( wp_unslash( $_GET['shop_order_subtype'] ) ) );
				}
	...
	public static function orders_by_type_query( $vars ) {
		...
		if ( 'shop_order' == $typenow && ! empty( $_GET['shop_order_subtype'] ) ) {
			...
			if ( 'original' == $_GET['shop_order_subtype'] || 'regular' == $_GET['shop_order_subtype'] ) {
				...
				$meta_key = apply_filters( 'woocommerce_subscriptions_admin_order_type_filter_meta_key', $meta_key, wc_clean( wp_unslash( $_GET['shop_order_subtype'] ) ) );
...
*/
    ...
    public static function debuger( $tolog = NULL, $id = NULL, $message = NULL, $start = FALSE ) {
        ...
        if( $start ) {
            ...
            $logger->log->add( $id, gmdate('d M Y, H:i:s') );
            $logger->log->add( $id, '' );
