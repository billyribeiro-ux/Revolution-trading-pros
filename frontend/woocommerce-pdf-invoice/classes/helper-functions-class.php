...
class WC_pdf_invoice_helper_functions {
	...
	public static function get_result_invoice_number_this_year( $start_date, $end_date ) {
		...
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
	        ...
	        $invoice = $wpdb->get_row( $wpdb->prepare( 
								...
								LIMIT 1;", 
							   	$meta_table_name,'_invoice_number', $meta_table_name,'_invoice_created_mysql', $start_date, $end_date 
							) 
						);
	...
	public static function get_result_invoice_number() {
        ...
        $invoice = $wpdb->get_row( $wpdb->prepare("SELECT * FROM %i 
	                                    WHERE meta_key = %s 
	                                    ORDER BY CAST(meta_value AS SIGNED) DESC
	                                    LIMIT 1;",
	                                    $meta_table_name,'_invoice_number'
	                                )
                                );
        return $invoice;
	...
	public static function get_result_invoice_count() {
        ...
        $invoice_count =   $wpdb->get_var( $wpdb->prepare("SELECT count(*) FROM %i
											WHERE meta_key = %s ",
	                                    	$meta_table_name,'_invoice_number'
	                                		)		
                                        );
	...
	public static function get_result_update_order_meta_invoice_date() {
		...
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			$results = $wpdb->get_var( $wpdb->prepare( 
						"SELECT COUNT(order_id) FROM %i 
						WHERE order_id IN (
							SELECT order_id FROM %i WHERE meta_key = %s
						) AND order_id NOT IN (
							SELECT order_id FROM %s WHERE meta_key = %s
						)
						GROUP BY (order_id)", 
					   	$meta_table_name, $meta_table_name, '_invoice_number', $meta_table_name, '_invoice_created' 
					) 
				);
