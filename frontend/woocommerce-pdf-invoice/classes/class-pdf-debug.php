<?php
    ...
    class WC_pdf_debug {
		...
		function woocommerce_invoice_meta_box( $post ) {
			...
			if ( is_array( $pdf_invoice_meta_items ) && !empty( $pdf_invoice_meta_items ) ) {
...
<?php 
				foreach( $pdf_invoice_meta_items as $key => $value ) {
					echo wp_kses_post( '<li><span>' . ucwords( str_replace( '_', ' ', $key) ) . ' : </span><input name="' . $key . '" type="text" value="' . $value . '" /></li>' );
				}
?>
					</ul>
					<p><?php esc_html_e('Please ensure you are aware of any potential legal issues before changing this information.<br />Changing the "Invoice Number" field IS NOT RECOMMENDED, changing this could cause duplicate invoice numbers.', 'woocommerce-pdf-invoice'); ?></p>
					<div class="clear"></div>
				</div><?php
...
<?php 
				foreach( $pdf_invoice_meta_items as $key => $value ) {
					...
					$field = '<li><span>' . ucwords( str_replace( '_', ' ', $key) ) . ' : </span><input name="' . $key . '" type="text" value="' . $value . '" /></li>';
