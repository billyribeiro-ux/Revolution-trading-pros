	...
	class WC_pdf_functions {
	    ...
	    public function __construct() {
	    	...
	    	if ( extension_loaded('iconv') && extension_loaded('mbstring') ) {					
				...
				// Create Invoice details at Payment Complete if necessary
        		add_action( 'woocommerce_payment_complete', array( $this,'pdf_invoice_payment_complete' ) ); 
		...
		public static function woocommerce_completed_order_create_invoice( $order_id ) {
			...
			if( apply_filters( 'pdf_invoice_no_pdf', $no_pdf, $order_id ) === true ) {
				...
				if( !$order->get_meta( '_invoice_number', TRUE ) || empty( $order->get_meta( '_invoice_number', TRUE ) ) ) {
		 ...
		 * @param [type] $order_id [description]
		 */
		public static function get_next_invoice_number( $order = NULL ) {
			global $woocommerce;
            ...
            if ( $annual_restart ) {
                ...
                $invoice = WC_pdf_invoice_helper_functions::get_result_invoice_number_this_year( $start_date, $end_date );
/*
...
*/
                ...
                // Check if we have created an invoice before this order
                $invoice = WC_pdf_invoice_helper_functions::get_result_invoice_number();
			...
			if ( !$current_invoice ) {
				...
				$invoice_count = WC_pdf_invoice_helper_functions::get_result_invoice_count();
			...
			if( is_object( $order ) ) {
				$next_invoice = apply_filters( 'woocommerce_pdf_invoices_set_invoice_number', $next_invoice, $order->get_id() );
			}
		...
		public static function set_invoice_number( $order_id ) {
			...
			// Invoice number exists, just get it and leave.
			if( $order->get_meta( '_invoice_number', TRUE ) && !empty( $order->get_meta( '_invoice_number', TRUE ) ) ) {
				return $order->get_meta( '_invoice_number', TRUE );
			}
			...
			if( !$order->get_meta( '_wc_pdf_invoice_created_date', TRUE ) ) {
				...
				if( isset( $invoice_meta['invoice_created'] ) && strlen( $invoice_meta['invoice_created'] != 0 ) ) {
					WC_pdf_functions::update_order_meta_data ( '_wc_pdf_invoice_created_date', $invoice_meta['invoice_created'], $order, $order_id );
				} else {
					$created_date = wc_string_to_datetime( current_time('mysql', true ) );
					WC_pdf_functions::update_order_meta_data ( '_wc_pdf_invoice_created_date', $created_date, $order, $order_id );
				}

				// Add additional _invoice_created_mysql for invoice number restart option
				WC_pdf_functions::add_order_meta_data( '_invoice_created_mysql', current_time('mysql', true ), $order, $order_id );
			...
			if ( !isset( $settings['sequential'] ) || $settings['sequential'] != 'true' ) {
				...
				$next_invoice = WC_pdf_functions::get_next_invoice_number( $order );
			}

			// Set an option for the current invoice and year to avoid querying the DB everytime
			update_option( 'woocommerce_pdf_invoice_current_year', gmdate('Y') );
			...
			WC_pdf_functions::update_order_meta_data ( '_wc_pdf_invoice_number', $next_invoice, $order, $order_id );
			WC_pdf_functions::update_order_meta_data ( '_invoice_number', $next_invoice, $order, $order_id );

			// Set display invoice number here
			$invoice_number_display = WC_pdf_functions::create_display_invoice_number( $order_id );
			WC_pdf_functions::update_order_meta_data ( '_invoice_number_display', $invoice_number_display, $order, $order_id );
			...
			// Save the order - DO NOT REMOVE THIS!
			$order->save();
		...
		public static function set_invoice_date( $order_id ) {
			...
			if ( $order->get_meta( '_invoice_date', TRUE ) && strlen( $order->get_meta( '_invoice_date', TRUE ) ) != 0 ) {
				...
				if ( $pdf_date == 'invoice' ) {
					$invoice_date = $order->get_meta( '_wc_pdf_invoice_created_date', TRUE ) ? $order->get_meta( '_wc_pdf_invoice_created_date', TRUE ) : $order->get_date_created();
		...
		public static function set_invoice_meta( $order_id ) {
			...
			// Backup Invoice Meta
			$invoice_meta = array(
				'invoice_created' 			=> $order->get_meta( '_wc_pdf_invoice_created_date', TRUE ),
			...
			foreach( $invoice_meta as $meta_key => $meta_value ) {
				WC_pdf_functions::update_order_meta_data ( '_'.$meta_key, $meta_value, $order, $order_id );
			}
		...
		static function create_display_invoice_number( $order_id ) { 
			...
			$invoice_number = $order->get_meta( '_wc_pdf_invoice_number', TRUE );
		...
		public static function send_invoice_icon_admin_init( $actions, $order ) {
			...
			if ( $order->get_meta( '_invoice_number', TRUE ) ) {
				...
				$actions['sendpdf'] = array(
					'url' 		=> wp_nonce_url( admin_url( 'admin-ajax.php?action=pdfinvoice-admin-send-pdf&order_id=' . $order->get_id() ), 'pdfinvoice-admin-send-pdf' ),
		...
		public static function download_invoice_icon_admin_init( $actions, $order ) {
			...
			if( isset( $settings['create_invoice'] ) && $settings['create_invoice'] == 'manual' && !$order->get_meta( '_invoice_number', TRUE ) ) {
				$show_download_link = false;
			}

			if( $show_download_link ) {
				$actions['downloadpdf'] = array(
					'url' 		=> ( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) . '&pdfid=' .$order->get_id() ),
		...
		function pdfinvoice_admin_send_pdf() {
			...
			if ( !is_admin() ) die;
			if ( !current_user_can('edit_posts') ) wp_die( esc_html__('You do not have sufficient permissions to access this page.', 'woocommerce-pdf-invoice') );
			if ( !check_admin_referer('pdfinvoice-admin-send-pdf')) wp_die( esc_html__('You have taken too long. Please go back and retry.', 'woocommerce-pdf-invoice') );
		 ...
		 function my_account_pdf( $actions = NULL, $order = NULL ) {
			...
			if ( $order->get_meta( '_invoice_number', TRUE ) ) {
				...
				$url = add_query_arg( array(
			 	...
			 	$actions['pdf'] = array(
					'url'  => $url,
					'name' => apply_filters('woocommerce_pdf_my_account_button_label', __( 'PDF Invoice', 'woocommerce-pdf-invoice' ) )
				);
		 ...
		 function pdf_url_check() {
			 ...
			 if ( isset( $_GET['pdfid'] ) && !is_admin() && isset( $_GET['key'] ) ) {
				...
				if ( $user_id == $current_user->ID && $order_key == $order_key_url ) {
					...
					echo WC_send_pdf::get_woocommerce_pdf_invoice( $order, NULL, 'false' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
	    ...
	    function trigger_email_action( $order_id ) {
		 ...
		 function invoice_link_thanks( $order_id ) {
		 	...
		 	if ( isset($settings['link_thanks']) && $settings['link_thanks'] == 'true' && $order->get_meta( '_invoice_number_display', TRUE ) ) {

		 		// Invoice number 
		 		$display_invoice_number = $order->get_meta( '_invoice_number_display', TRUE );

		 		// URL
		 		$download_url = add_query_arg( 'pdfid', $order_id, sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
		 		...
		 		if( $button_or_link === 'button' ) {
                    ...
                    /* translators: Download button output */
                    $invoice_link_thanks  = '<p class="pdf-download">' . sprintf( '%s', $button_url ) . '</p>';
                } else {
		 			$download_url 		 = '<a href="'. $download_url .'">' . $display_invoice_number . '</a>';
		 			$download_output 	 = str_replace( '[[INVOICENUMBER]]', $download_url, $output );
		 			/* translators: Download link output */
		 			$invoice_link_thanks = '<p class="pdf-download">' . sprintf( '%s', $download_output ) . '</p>';
		 		}

				echo wp_kses_post( apply_filters( 'pdf_invoice_invoice_link_thanks', $invoice_link_thanks, $order_id ) ); // nosemgrep scanner.php.wp.security.xss.query-arg.user-input 
		...
		function pdf_invoice_send_test() {
			 ...
			 if ( isset( $_POST['pdfemailtest'] ) && $_POST['pdfemailtest'] == '1' ) {
				...
				if ( !isset($_POST['pdf_test_nonce']) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['pdf_test_nonce'] ) ),'pdf_test_nonce_action') ) {
					die( 'Security check' );
				}
		...
		static function get_invoice_meta_for_deletion() {
		...
		function pdf_invoice_fix_dates() {
			...
			if( in_array( $allowed_user_role, $current_user->roles ) ) {
				...
				if ( isset( $_POST['pdffixdates'] ) && $_POST['pdffixdates'] == '1' && isset( $_POST['pdffix-dates-confirmation'] ) && $_POST['pdffix-dates-confirmation'] === "confirm" ) {
					...
					if ( !isset($_POST['pdf_fix_dates_nonce']) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['pdf_fix_dates_nonce'] ) ),'pdf_fix_dates_nonce_action') ) {
						die( 'Security check' );
					}
		...
		public static function action_scheduler_fix_dates() {
			...
			if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
				$post_table_name =  $wpdb->prefix.'wc_orders';
			}
			...
			$results = $wpdb->get_results( $wpdb->prepare( 
								"SELECT p.ID 
									...
									WHERE p.post_type = %s 
									AND pm.meta_key = %s
			                      	ORDER BY p.id ASC LIMIT 100 OFFSET %s", 
							   	$post_table_name, $meta_table_name, '_invoice_number', '_invoice_created', $offset 
							) 
						);
			...
			if( isset($results) && !empty( $results ) ) {
				...
				foreach ( $results as $result ) {
					...
					$current_date = $order->get_meta( '_invoice_date', TRUE );
					...
					// Update Invoice meta
					WC_pdf_functions::update_order_meta_data ( '_invoice_date', $new_date, $order, $order_id );
		...
		public static function pdf_invoice_delete_invoices() {
			...
			if( in_array( $allowed_user_role, $current_user->roles ) ) {
				...
				if ( isset( $_POST['pdfdelete'] ) && $_POST['pdfdelete'] == '1' && isset( $_POST['pdfdelete-confirmation'] ) && $_POST['pdfdelete-confirmation'] === "confirm" ) {
					...
					if ( !isset($_POST['pdf_delete_nonce']) || !wp_verify_nonce(sanitize_text_field( wp_unslash( $_POST['pdf_delete_nonce'] ) ),'pdf_delete_nonce_action') ) {
						die( 'Security check' );
					}

					$wc_orders_meta_table = $wpdb->prefix.'wc_orders_meta';

					$invoice_meta = self::get_invoice_meta_for_deletion();
					...
					$results = $wpdb->get_results( $wpdb->prepare( 
								"DELETE FROM %i WHERE meta_key = %s", 
							   	$wc_orders_meta_table, '_invoice_created_mysql'
							) 
						);
					...
					$results = $wpdb->get_results( $wpdb->prepare( 
								"DELETE FROM %i WHERE meta_key = %s", 
							   	$wc_orders_meta_table, '_wc_pdf_invoice_created_date'
							) 
						);
		...
		public static function pdf_invoice_past_orders() {
			...
			if( in_array( $allowed_user_role, $current_user->roles ) ) {
				...
				if ( (isset( $_POST['pdf_past_orders'] ) && $_POST['pdf_past_orders'] == '1' && isset( $_POST['pdf_past_orders-confirmation'] ) && $_POST['pdf_past_orders-confirmation'] === "confirm") ) {
					WC()->queue()->schedule_single( time()+10, 'woocommerce_pdf_invoice_update_past_orders' );
	            }
		...
		public static function action_scheduler_update_past_orders( $args = NULL, $group = '' ) {
			...
			if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
				...
				$results = $wpdb->get_results( $wpdb->prepare( 
						"SELECT COUNT(*)
	                      FROM %i AS p
	                      WHERE p.type = %s AND p.status = %s AND p.id NOT IN (
	                            SELECT p.ID FROM %i AS p INNER JOIN %i pm on p.id = pm.order_id WHERE p.type = %s AND p.status = %s AND pm.meta_key = %s
	                      )
	                      ORDER BY p.id ASC", 
					   	$wpdb->prefix.'wc_orders', 'shop_order', 'wc-completed', $wpdb->prefix.'wc_orders', $wpdb->prefix.'wc_orders_meta', 'shop_order', 'wc-completed', '_invoice_number'
					) 
				);
				...
				$results = $wpdb->get_results( $wpdb->prepare( 
						"SELECT COUNT(*)
	                      FROM %i AS p
	                      WHERE p.post_type = %s AND p.post_status = %s AND p.id NOT IN (
	                            SELECT p.ID FROM %i AS p INNER JOIN %i pm on p.id = pm.order_id WHERE p.post_type = %s AND p.post_status = %s AND pm.meta_key = %s
	                      )
	                      ORDER BY p.id ASC", 
					   	$wpdb->prefix.'posts', 'shop_order', 'wc-completed', $wpdb->prefix.'posts', $wpdb->prefix.'postmeta', 'shop_order', 'wc-completed', '_invoice_number'
					) 
				);
            ...
            if( isset($results) && !empty( $results ) ) {
	            ...
	            if( isset( $res ) ) {
	            	...
	            	foreach ( $res as $result ) {
	                    ...
	                    if ( sanitize_title( $order_status ) == 'completed' && $order->get_meta( '_invoice_number', TRUE ) == '' ) {
		...
		/**
		 * [action_scheduler_update_email_past_orders description]
		 * Setup schedule to create invoices for orders placed before PDF Invoices was installed.
		 * @param  [type] $args  [description]
		 * @param  string $group [description]
		 * @return [type]        [description]
		...
		public static function action_scheduler_delete_files( $args = NULL, $group = '' ) {
	        ...
	        if ( file_exists( $upload_dir . '/index.html' ) && $delete_pdf === TRUE ) {
	            ...
	            foreach( $pdfs as $pdf ) {
	                if( is_file($pdf) ) {
	                    wp_delete_file( $pdf ); // delete file
	                }
	            }
	            ...
	            foreach( $zips as $zip ) {
	                if( is_file($zip) ) {
	                    wp_delete_file( $zip ); // delete file
	                }
	            }
		...
		public static function action_scheduler_delete_logs( $args = NULL, $group = '' ) {
			...
			$actionscheduler_actions_table = $wpdb->prefix.'actionscheduler_actions';

			$wpdb->query( $wpdb->prepare( 
								"DELETE FROM %i 
							   	WHERE hook = %s 
							   	AND status =%s", 
							   	$actionscheduler_actions_table, 'woocommerce_pdf_invoice_delete_files', 'complete' 
							) 
						);
			...
			$wpdb->query( $wpdb->prepare( 
								"DELETE FROM %i 
							   	WHERE hook = %s 
							   	AND status =%s", 
							   	$actionscheduler_actions_table, 'woocommerce_pdf_invoice_update_order_meta_invoice_date', 'complete' 
							) 
						);
			...
			$wpdb->query( $wpdb->prepare( 
								"DELETE FROM %i
							   	WHERE hook = %s 
							   	AND status = %s", 
							   	$actionscheduler_actions_table, 'woocommerce_pdf_invoice_update_email_past_orders', 'complete' 
							) 
						);
			...
			$wpdb->query( $wpdb->prepare( 
								"DELETE FROM %i
							   	WHERE hook = %s 
							   	AND status = %s", 
							   	$actionscheduler_actions_table, 'woocommerce_pdf_invoice_upgrade_order_meta_invoice_creation_date', 'complete' 
							) 
						);
		...
		function subscriptions_remove_renewal_order_meta( $order_meta_query, $original_order_id, $renewal_order_id, $new_order_role ) {
			$order_meta_query .= " AND meta_key NOT IN ( " . implode( "','", $this->get_invoice_meta_for_deletion() ) . " )";
		...
		function subscriptions_remove_subscription_order_meta( $order_meta, $to_order, $from_order ) {
			...
			if ( $to_order instanceof WC_Subscription && $from_order instanceof WC_Order ) {
				...
				foreach ( $order_meta as $index => $meta ) {
					...
					if ( in_array( $meta['meta_key'], $this->get_invoice_meta_for_deletion() ) ) {
		...
		public static function pdf_invoice_update_order_meta_invoice_date() {
			...
			include( 'helper-functions-class.php' );

            $results = WC_pdf_invoice_helper_functions::get_result_update_order_meta_invoice_date();
		...
		/**
		 * [action_scheduler_update_order_meta_invoice_date description]
		 * Setup schedule to create invoices for orders placed before PDF Invoices was installed.
		 * @param  [type] $args  [description]
		 * @param  string $group [description]
		 * @return [type]        [description]
		...
		public static function action_scheduler_update_order_meta_invoice_date( $args = NULL, $group = '' ) {
            ...
            $results = WC_pdf_invoice_helper_functions::get_result_action_scheduler_update_order_meta_invoice_date();
            ...
            if( $results ) {
            	...
            	foreach( $results AS $result ) {
            		...
            		if( is_array( $invoice_meta ) ) {
            			WC_pdf_functions::update_order_meta_data ( '_invoice_created', $invoice_meta['invoice_created'], $order, $result->post_id );
            		}
         ...
         * @return [type]           [description]
         */
		public static function update_order_meta_data ( $key, $value, $order, $order_id ) {
	    ...
	    /**
	     * [add_order_meta_data description]
	     * @param  [type] $key      [description]
	     * @param  [type] $value    [description]
	     * @param  [type] $order    [description]
	     * @param  [type] $order_id [description]
	     * @return [type]           [description]
	     */
	    public static function add_order_meta_data( $key, $value, $order, $order_id ) {
    	...
    	/**
    	 * [delete_order_meta_data description]
    	 * @param  [type] $key      [description]
    	 * @param  [type] $order    [description]
    	 * @param  [type] $order_id [description]
    	 * @return [type]           [description]
    	 */
		public static function delete_order_meta_data ( $key, $order, $order_id ) {
		...
		public static function action_scheduler_create_email_invoice( $args = NULL, $group = '' ) {
			...
			foreach ( $args['ids'] as $order_id ) {
	            ...
	            if( $send_email && in_array( 'PDF_Invoice_Customer_PDF_Invoice', $email_classes ) ) {
	            	$order = wc_get_order( $order_id );
					WC()->mailer()->emails['PDF_Invoice_Customer_PDF_Invoice']->trigger( $order_id, $order );
				}
