...
class TCPDF {
	...
	public function Error($msg) {
		...
		if (defined('K_TCPDF_THROW_EXCEPTION_ERROR') AND !K_TCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>TCPDF ERROR: </strong>'. wp_kses_post( $msg ) );
		} else {
			throw new Exception('TCPDF ERROR: '. wp_kses_post( $msg ) );
		}
	}
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (!empty($imgdata)) {
			...
			fwrite($fp, $imgdata); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
		...
		if ($newimage) {
			...
			if (($info === false) AND extension_loaded('imagick')) {
				try {
					...
					unlink($tempname); // phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink 
					$img->destroy();
				} catch(Exception $e) {
					$info = false;
				}
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			fwrite($f, $pdfdoc, $pdfdoc_length); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					echo wp_kses_post( $this->getBuffer() );
				}
				break;
			}
			case 'D': {
			...
			case 'FD': {
				...
				fwrite($f, $this->getBuffer(), $this->bufferlen); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
				fclose($f); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
				if ($dest == 'FI') {
			...
			if ($handle = @opendir(K_PATH_CACHE)) {
				while ( false !== ( $file_name = readdir( $handle ) ) ) {
					if (strpos($file_name, '__tcpdf_'.$this->file_id.'_') === 0) {
						unlink(K_PATH_CACHE.$file_name);  // phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink
					}
				}
				closedir($handle);
			}
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$parsedUrl = parse_url($pl['txt']); // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				fwrite($f, $envelope, $envelope_length); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
				fclose($f); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
	...
	protected function _putbookmarks() {
		...
		foreach ($this->outlines as $i => $o) {
			...
			$title = preg_replace("/[\n]+/si", "\n", $title);
			$title = strip_tags($title); // phpcs:ignore WordPress.WP.AlternativeFunctions.strip_tags_strip_tags
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					if (($imgsrc[0] === '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
						// fix image path
						$findroot = strpos($imgsrc, sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ));
						if (($findroot === false) OR ($findroot > 1)) {
							if (substr(sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ), -1) == '/') {
								$imgsrc = substr(sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ), 0, -1).$imgsrc;
							} else {
								$imgsrc = sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ).$imgsrc;
							}
						}
						$imgsrc = urldecode($imgsrc);
						$testscrtype = @parse_url($imgsrc);  // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (($img[0] == '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
							$findroot = strpos($img, sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ));
							if (($findroot === false) OR ($findroot > 1)) {
								if (substr(sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ), -1) == '/') {
									$img = substr(sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ), 0, -1).$img;
								} else {
									$img = sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ).$img;
								}
							}
						}
						$img = urldecode($img);
						$testscrtype = @parse_url($img); // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
						if (empty($testscrtype['query'])) {
							// convert URL to server path
