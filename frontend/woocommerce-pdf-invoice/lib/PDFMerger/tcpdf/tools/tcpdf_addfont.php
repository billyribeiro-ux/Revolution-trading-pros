...
if (!is_dir($options['outpath']) OR !is_writable($options['outpath'])) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_is_writable
	echo "ERROR: Can't write to ". esc_url( $options['outpath'] ) ."\n\n";
	exit(3);
}

echo "\n>>> Converting fonts for TCPDF:\n";

echo '*** Output dir set to '. esc_url( $options['outpath'] ) ."\n";
