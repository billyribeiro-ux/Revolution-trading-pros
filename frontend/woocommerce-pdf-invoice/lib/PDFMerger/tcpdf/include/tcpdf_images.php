...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function getImageFileType($imgfile, $iminfo=array()) {
		...
		if (empty($type)) {
            $type = strtolower(trim(pathinfo(parse_url($imgfile, PHP_URL_PATH), PATHINFO_EXTENSION))); // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
		}
		if ($type == 'jpg') {
			$type = 'jpeg';
		}
	...
	public static function _toPNG($image, $tempfile) {
		...
		unlink($tempfile); // phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink
		return $retvars;
	}
	...
	public static function _parsepng($file) {
		...
		//Check signature
		if (fread($f, 8) != chr(137).'PNG'.chr(13).chr(10).chr(26).chr(10)) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
			// Not a PNG file
			return false;
		}
		//Read header chunk
		fread($f, 4); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
		if (fread($f, 4) != 'IHDR') { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
			//Incorrect PNG file
			return false;
		}
		$w = TCPDF_STATIC::_freadint($f);
		$h = TCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1)); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
		$ct = ord(fread($f, 1)); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		} elseif ($ct == 2) {
			$colspace = 'DeviceRGB';
		...
		if (ord(fread($f, 1)) != 0) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
		...
		do {
			$type = fread($f, 4); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
			if ($type == 'PLTE') {
				// read palette
				$pal = TCPDF_STATIC::rfread($f, $n); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
				fread($f, 4); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
			} elseif ($type == 'tRNS') {
				...
				// skip profile name
				$len = 0;
				while ((ord(fread($f, 1)) != 0) AND ($len < 80)) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
					++$len;
				}
				...
				TCPDF_STATIC::rfread($f, $n + 4); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
			}
			$n = TCPDF_STATIC::_freadint($f);
		} while ($n);
