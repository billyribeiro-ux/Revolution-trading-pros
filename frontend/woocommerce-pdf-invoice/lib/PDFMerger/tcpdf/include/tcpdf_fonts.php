...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			fwrite($fp, gzcompress($data)); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
			...
			if ($fmetric['type'] != 'cidfont0') {
				if ($link) {
					...
					$fmetric['file'] .= '.z';
					$fp = TCPDF_STATIC::fopenLocal($outpath.$fmetric['file'], 'wb');
					fwrite($fp, gzcompress($font)); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
					fclose($fp); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
				}
			}
			$offset += 4;
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($fmetric['type'] == 'cidfont0') {
				...
				fwrite($fp, gzcompress($cidtogidmap)); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
				fclose($fp); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
			}
		}
		$pfile .= '$desc=array(';
		...
		fwrite($fp, $pfile); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
		fclose($fp); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
