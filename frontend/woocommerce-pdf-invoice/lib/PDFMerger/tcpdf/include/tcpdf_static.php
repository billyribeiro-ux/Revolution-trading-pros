...
class TCPDF_STATIC {
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getFormattedDate($time) {
		return substr_replace(date('YmdHisO', intval($time)), '\'', (0 - 2), 0).'\''; // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
	}
	...
	public static function getRandomSeed($seed='') {
		$rnd = uniqid(rand().microtime(true), true); // phpcs:ignore WordPress.WP.AlternativeFunctions.rand_rand
	...
	public static function url_exists($url) {
		$crs = curl_init(); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_init
		// encode query params in URL to get right response form the server
		$url = self::encodeUrlQuery($url);
		curl_setopt($crs, CURLOPT_URL, $url); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_NOBODY, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_FAILONERROR, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		if ((ini_get('open_basedir') == '') && (!ini_get('safe_mode'))) {
			curl_setopt($crs, CURLOPT_FOLLOWLOCATION, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		}
		curl_setopt($crs, CURLOPT_CONNECTTIMEOUT, 5); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_TIMEOUT, 30); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_SSL_VERIFYPEER, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_SSL_VERIFYHOST, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_USERAGENT, 'tc-lib-file'); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		curl_setopt($crs, CURLOPT_MAXREDIRS, 5); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		if (defined('CURLOPT_PROTOCOLS')) {
		    curl_setopt($crs, CURLOPT_PROTOCOLS, CURLPROTO_HTTPS | CURLPROTO_HTTP |  CURLPROTO_FTP | CURLPROTO_FTPS); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
		}
		curl_exec($crs); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_exec
		$code = curl_getinfo($crs, CURLINFO_HTTP_CODE); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_getinfo
		curl_close($crs); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_close
		return ($code == 200);
	}
	...
	public static function encodeUrlQuery($url) {
		$urlData = parse_url($url); // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
	...
	public static function fileGetContents($file) {
		...
		if ((strlen($file) > 1)
		    ...
		    $findroot = strpos($file, sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ) );
		    if (($findroot === false) || ($findroot > 1)) {
			$alt[] = htmlspecialchars_decode(urldecode(sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ).$file));
		    }
		}
		//
		$protocol = 'http';
		...
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				$host = $protocol.'://'.sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) );
				if (strpos($url, $host) === 0) {
				    // convert URL to full server path
				    $tmp = str_replace($host, sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ), $url);
		...
		if (isset($_SERVER['SCRIPT_URI'])
		    ...
		    $urldata = @parse_url(sanitize_text_field( wp_unslash( $_SERVER['SCRIPT_URI'] ) )); // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
		...
		foreach ($alt as $path) {
			...
			if (!ini_get('allow_url_fopen')
				...
				$crs = curl_init(); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_init
				curl_setopt($crs, CURLOPT_URL, $path); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
				curl_setopt($crs, CURLOPT_BINARYTRANSFER, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
				curl_setopt($crs, CURLOPT_FAILONERROR, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
				curl_setopt($crs, CURLOPT_RETURNTRANSFER, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
				if ((ini_get('open_basedir') == '') && (!ini_get('safe_mode'))) {
				    curl_setopt($crs, CURLOPT_FOLLOWLOCATION, true); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
				}
				curl_setopt($crs, CURLOPT_CONNECTTIMEOUT, 5); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
				...
				$ret = curl_exec($crs); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_exec
				curl_close($crs); // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_close
				if ($ret !== false) {
					return $ret;
				}
	...
	public static function _freadint($f) {
		$a = unpack('Ni', fread($f, 4)); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
		return $a['i'];
	}
