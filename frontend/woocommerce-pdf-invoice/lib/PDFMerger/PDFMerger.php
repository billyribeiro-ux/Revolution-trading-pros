...
class PDFMerger
{
	...
	public function addPDF($filepath, $pages = 'all')
	{
		...
		{
			throw new \exception("Could not locate PDF on " . esc_url($filepath) );
		}
    ...
    $fpdi->SetPrintFooter(false);
		...
		foreach($this->_files as $file)
		{
			...
			else
			{
				...
				{
					if(!$template = $fpdi->importPage($page)): throw new exception("Could not load page " . wp_kses_post($page) . " in PDF " . wp_kses_post($filename) . ". Check that the page exists."); endif;
					$size = $fpdi->getTemplateSize($template);
					$orientation = ($size['h'] > $size['w']) ? 'P' : 'L';
		...
		else
		{
			...
			{
				throw new exception("Error outputting PDF to " . esc_url($outputmode) . ".");
				return false;
			}
		}
	...
	private function _rewritepages($pages)
	{
		...
		foreach($part as $i)
		{
			...
			if(count($ind) == 2)
			{
				...
				if($x > $y): throw new exception("Starting page, " . wp_kses_post($x) . " is greater than ending page " . wp_kses_post($y) . "."); return false; endif;
