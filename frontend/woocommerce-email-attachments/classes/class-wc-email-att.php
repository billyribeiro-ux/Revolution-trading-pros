...
//		add_action( 'woocommerce_init', array( $this, 'handler_wc_init'), 100 );
		...
		if( ! in_array( $this->options['show_notes_location'], array( 'top', 'inside' ) ) )
		{
			add_action( 'woocommerce_email_footer', array( $this, 'handler_wc_email_attachment_footer' ), 900, 1 );
		}
		else
		{
			$priority = 'top' == $this->options['show_notes_location'] ? 1 : 900;
			add_action( 'woocommerce_email_header', array( $this, 'handler_wc_email_attachment_footer' ), $priority, 1 );
		}
	}
...
//		register_taxonomy_for_object_type('category', 'my_post_type');
	...
	public function handler_wc_email_attachment_footer( $email )
	{
		...
		foreach ( $footers as &$footer )
		{
			...
			/**
			 * Filter to allow unfiltered HTML in notification headline and text
			 *
			 * @since 3.1.2
			 * @param boolean $allow_unfiltered_notification_text
			 * @return mixed|true					true to allow unfiltered text
			 */
			if( true !== apply_filters( 'wc_email_attachments_unfiltered_notification_text', false ) )
			{
				$head = esc_html($head);
				$text = esc_html($text);
			}
...
//		$request = "SELECT * FROM {$wpdb->posts} WHERE post_type = %s ORDER BY ID ASC";


		$posts = $wpdb->get_results(   // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->prepare(
								"SELECT * FROM {$wpdb->posts} WHERE post_type = %s ORDER BY ID ASC",
								self::POST_TYPE
							)
					);
