...
function is_plugin_activated() {
	if ( ConsentMagic()->getOption( 'wc_am_client_consent_magic_pro_activated' ) == 'Activated'
...
function generate_settings_tabhead_mobile( $title_arr, $page = 'consent-magic', $default = 'cs-general' ) {
    ...
    <button class="cm-dropdown-toggle">
        <span class="cm-current-menu <?php echo esc_attr( $active ? 'active' : '' ); ?>"><?php echo esc_html(
				$out_arr[ $current_tab ] ?? $out_arr[ 'cs-general' ]
			); ?></span>
    ...
    <ul class="cm-dropdown-menu">
		...
		foreach ( $out_arr as $k => $v ) {
            ...
            <li class="<?php echo esc_attr( $class ); ?>"><a
                        href="<?php echo esc_url( $admin_url ); ?>"><?php echo esc_attr(
						$v
					); ?></a><?php echo wp_kses_post( $class == 'active' ? '<i class="icon-check"></i>' : '' ); ?>
            </li>
			<?php
		}
		?>
...
function get_custom_scripts_list( $searchByFromdate_sql = null, $searchByTodate_sql = null ) {
	...
	while ( $query->have_posts() ) {
		...
		$buttons            = '<div class="custom-scripts-actions table_right_aligned"><a href="' . esc_url(
				get_admin_url( null, "admin.php?page=consent-magic" ) . "&script_id=" . get_the_ID()
			) . '" class="button-link link-edit">' . __( 'Edit', 'consent-magic' ) . '</a>
					<a href="#" class="button-link link-delete delete_custom_post" data-cat="' . $option_select_name
		                      . '" data-id="' . get_the_ID() . '">' . __( 'Delete', 'consent-magic' ) . '</a></div>
				';
...
function get_scan_cookies_list() {
	...
	if ( $scan_cookies ) {
		foreach ( $scan_cookies as $cookie ) {
			...
			$disabled           = ( ( strpos( $cookie[ 'description' ], 'Default wordpress cookie' ) !== false
			                          && $cookie[ 'category' ] === 'necessary' )
			                        || $cookie[ 'type' ] === 'pys' ) ? 'disabled' : '';
			$options[]          = array(
				...
				'options_terms'      => $options_terms,
				'checked'            => checked(
					get_scan_cookie_data( $cookie[ 'id_cs_scan_cookies' ], 'cookie_enabled' ),
					true,
					false
				),
		...
		foreach ( $scan_scripts as $script ) {
			if ( $script[ 'script_body' ] !== 'custom' ) {
				$options[] = array(
					...
					'script_button' => '<a href="#" data-target="pre-defined-scripts-bar" class="open_target_tab link table_link">'
					                   . esc_html__( 'Pre-Defined Script', 'consent-magic' ) . '</a>'
				);
			} else {
				...
				$options[] = array(
					...
					'script_body'   => $script[ 'script_body' ],
					'description'   => $script[ 'description' ],
					'script_button' => '<a href="' . get_admin_url( null, 'admin.php?page=consent-magic' )
					                   . '&script_id=' . esc_attr( $script_id ) . '" class="link table_link">'
					                   . esc_html__( 'Custom Script', 'consent-magic' ) . '</a>'
				);
			}
		}
	}
...
function get_post_by_title( $page_title, $post_type = 'post', $output = OBJECT ) {
	global $wpdb;
	$post = $wpdb->get_var(
		$wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type=%s", $page_title, $post_type )
	);
	if ( $post ) {
...
function get_scan_cookie_data( $id, $key ) {
	...
	$sql           = $wpdb->prepare(
		'SELECT `%1$s` FROM %2$s WHERE `id_cs_scan_cookies` = %3$s ORDER BY id_cs_scan_cookies DESC LIMIT 1',
		$key,
		$cookies_table,
		$id
	);
...
function update_scan_cookie_data( $id, $data_arr ) {
	...
	if ( $wpdb->update( $cookies_table, $data_arr, array( 'id_cs_scan_cookies' => $id )
...
function update_scan_script_data( $id, $data_arr ) {
	...
	if ( $wpdb->update( $scripts_table, $data_arr, array( 'id_cs_scan_scripts' => $id )
	) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery,WordPress.DB.PreparedSQL.NotPrepared
		return true;
	} else {
		return false;
...
function get_meta_value_by_key( $id, $key ) {
	if ( $key == '_cs_type' ) {
		...
		switch ( get_post_meta( $id, $key, true ) ) {
			case 'bar_small':
				return '<p class="purple-label">' . __( 'bar', 'consent-magic' ) . '</p><p class="purple-label">' . __(
						'small',
						'consent-magic'
					) . '</p>';
				break;
			case 'bar_large':
				...
				break;
			case 'popup_small':
				return '<p class="purple-label">' . __( 'popup', 'consent-magic' ) . '</p><p class="purple-label">'
				       . __( 'small', 'consent-magic' ) . '</p>';
				break;
			case 'popup_large':
				return '<p class="purple-label">' . __( 'popup', 'consent-magic' ) . '</p><p class="purple-label">'
				       . esc_html__( 'large', 'consent-magic' ) . '</p>';
				break;
			default:
				return '';
		}
...
function db_cron_change_recurrence( $reshedule = false ) {
	if ( ConsentMagic()->getOption( 'cs_geolocation' ) && ConsentMagic()->getOption( 'cs_geo_activated' )
	     && ConsentMagic()->getOption( 'cs_often_update' ) != 'never' ) {
...
function db_cron_update_one_time( $first_time = false ) {
	if ( ( !wp_next_scheduled( 'cs_db_cron_update_hook_one_time' )
	       && wp_next_scheduled( 'cs_db_cron_update_hook' ) > wp_next_scheduled(
				'cs_db_cron_update_hook_one_time'
			) )
	     || $first_time ) {
...
function create_email_before_delete_consent() {
	...
	if ( !empty( $email ) ) {
		...
		$message                = '<p>' . sprintf(
				esc_html__(
					'ConsentMagic is configured to delete stored consent when it reaches %s entries. This email confirms consent was deleted on %s',
					'consent-magic'
				),
				$cs_proof_consent_count,
				current_time( 'mysql' )
			) . '</p>' . '<p>' . esc_html__( 'Consent entries are attached to this email.', 'consent-magic' ) . '</p>'
		                          . '<p>' . sprintf(
			                          esc_html__( 'You can change these settings %shere%s', 'consent-magic' ),
			                          "<a href='" . admin_url( 'admin.php?page=cs-proof-consent&tab=cs-settings' )
			                          . "' target='_blank'>",
			                          '</a>'
		                          ) . '</p>';
...
function replace_text_for_partners( $text, $lang, $count, $classes = '' ) {
	...
	if ( isset( $search[ $lang ] ) ) {
		...
		$text    = str_replace(
			$search[ $lang ][ 'search' ],
			"<a class='cs_open_vendors $classes'>$replace</a>",
			$text
		);
	}
