...
if ( !class_exists( 'CS_EDD_Updater' ) ) {
	return;
}
...
class CS_EDD_Updater {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( !empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			echo '<div class="update-message notice inline notice-warning notice-alt cm-fixed-notice">';
	...
	private function api_request( $_action, $_data ) {
		...
		if ( $data[ 'slug' ] != $this->slug || $this->api_url == trailingslashit( home_url() ) ) {
			return false;
		}
	...
	public function show_changelog() {
		...
		$cache_key    = hash( 'sha256', 'edd_plugin_' . sanitize_key( $_REQUEST[ 'plugin' ] ) . '_' . $beta . '_version_info' );
	...
	public function is_cache_expired( $cache_key = '' ) {
		...
		$cache = ConsentMagic()->getOption( $cache_key );
	...
	public function set_version_info_cache( $value = '', $cache_key = '' ) {
		...
		ConsentMagic()->updateOptions( array(  $cache_key => $data ) );
	}
