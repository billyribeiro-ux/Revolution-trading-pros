...
async function fetchFromBackend(endpoint: string, options: RequestInit = {}): Promise<any | null> {
	try {
		logger.info(`[Stats API] Fetching: ${BACKEND_URL}${endpoint}`);
		...
		if (!response.ok) {
			logger.error(`[Stats API] Backend error: ${response.status} ${response.statusText}`);
			return null;
		}

		const data = await response.json();
		logger.info(`[Stats API] Backend success`);
		return data;
	} catch (err) {
		logger.error('[Stats API] Backend fetch failed:', err);
		return null;
	}
}
...
export const GET: RequestHandler = async ({ params, request, cookies }) => {
	...
	logger.info(`[Stats API] Using mock data for ${slug}`);
