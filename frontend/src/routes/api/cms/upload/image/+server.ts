...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        if (!ALLOWED_TYPES.includes(file.type)) {
            return json(
                { success: false, error: `Invalid file type. Allowed: ${ALLOWED_TYPES.join(', ')}` } as UploadResponse,
                { status: 400 }
            );
        }
        ...
        if (file.size > MAX_FILE_SIZE) {
            return json(
                { success: false, error: `File too large. Maximum size: ${MAX_FILE_SIZE / 1024 / 1024}MB` } as UploadResponse,
                { status: 400 }
            );
        }
        ...
        return json(
            { success: false, error: 'Failed to upload file' } as UploadResponse,
            { status: 500 }
        );
    }
};
