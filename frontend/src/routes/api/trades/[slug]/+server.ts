...
async function fetchFromBackend(endpoint: string, options: RequestInit = {}): Promise<any | null> {
	try {
		logger.info(`[Trades API] Fetching: ${BACKEND_URL}${endpoint}`);
		...
		if (!response.ok) {
			logger.error(`[Trades API] Backend error: ${response.status} ${response.statusText}`);
			return null;
		}

		const data = await response.json();
		logger.info(`[Trades API] Backend success:`, data?.data?.length || 0, 'items');
		return data;
	} catch (err) {
		logger.error('[Trades API] Backend fetch failed:', err);
		return null;
	}
}
...
export const GET: RequestHandler = async ({ params, url, request, cookies }) => {
	...
	logger.info(`[Trades API] Using mock data for ${slug}`);
