/**
 * Admin Member Analytics — cohorts
...
export const GET: RequestHandler = async (event) => {
	...
	try {
		...
		const response = await fetch(`${API_URL}/api/admin/members/analytics/cohorts${qs}`, {
			method: 'GET',
			headers: {
		...
		console.error('[API Proxy] members/analytics/cohorts error:', err);
