<script lang="ts">
	/**
	 * Options Day Trading - Trader Store
	 * ═══════════════════════════════════════════════════════════════════════════
	 */
	...
	const products = [
		{
			...
			description: 'Complete options trading system from beginner to advanced.',
			category: 'Course'
		},
		{
			id: 2,
			name: 'Weekly Options Blueprint',
			trader: 'Expert',
			price: 297,
			originalPrice: null,
			rating: 4.8,
			...
			description: 'Master weekly options trading strategies.',
			category: 'Course'
		}
	];
</script>
...
<svelte:head>
	<title>Trader Store - Options Day Trading | Revolution Trading Pros</title>
</svelte:head>

<div class="trader-store">
	<a href="/dashboard/options-day-trading" class="back-link">
		<IconArrowLeft size={18} />
		Back to Options Day Trading
	</a>

	<header class="page-header">
		<div class="header-icon"><IconBuildingStore size={32} /></div>
		<div class="header-content">
			<h1 class="page-title">Trader Store</h1>
			<p class="page-subtitle">Options courses and resources</p>
		</div>
	</header>
		...
		{#each products as product}
			<article class="product-card">
				<div class="product-image" style="background-image: url({product.image})">
					...
					{#if product.originalPrice}<span class="product-sale">Sale</span>{/if}
				</div>
				<div class="product-info">
					<p class="product-trader">by {product.trader}</p>
					<h3 class="product-name">{product.name}</h3>
					<p class="product-description">{product.description}</p>
					<div class="product-rating">
						<IconStar size={16} fill="#F69532" color="#F69532" />
						<span class="rating-value">{product.rating}</span>
						<span class="rating-count">({product.reviews} reviews)</span>
					</div>
					<div class="product-pricing">
						<span class="current-price">${product.price}</span>
						{#if product.originalPrice}<span class="original-price">${product.originalPrice}</span>{/if}
					</div>
					<button class="add-to-cart-btn"><IconShoppingCart size={18} />Add to Cart</button>
				</div>
			</article>
		{/each}
	</div>
...
<style>
	.trader-store { padding: 30px; max-width: 1400px; margin: 0 auto; }
	.back-link { display: inline-flex; align-items: center; gap: 8px; color: #0984ae; text-decoration: none; font-size: 14px; font-weight: 500; margin-bottom: 24px; }
	...
	.product-trader { font-size: 12px; color: #0984ae; margin: 0 0 8px; font-weight: 600; }
	.product-name { font-size: 18px; font-weight: 700; color: #333; margin: 0 0 8px; line-height: 1.3; font-family: 'Open Sans', sans-serif; }
	.product-description { font-size: 14px; color: #666; line-height: 1.5; margin: 0 0 12px; }
	.product-rating { display: flex; align-items: center; gap: 6px; margin-bottom: 12px; }
	.rating-value { font-size: 14px; font-weight: 700; color: #333; }
