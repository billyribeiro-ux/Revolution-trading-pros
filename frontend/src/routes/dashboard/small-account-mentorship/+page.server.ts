...
export const load = (async ({ fetch }) => {
	...
	const [watchlist, tutorialRes, updatesRes, documentsRes] = await Promise.all([
		// Weekly watchlist
		getLatestWatchlist('small-account-mentorship', fetch),
		
		// Featured tutorial video
		fetch(`${baseUrl}/api/room-resources?room_id=${SMALL_ACCOUNT_MENTORSHIP_ID}&resource_type=video&content_type=tutorial&is_featured=true&per_page=1`)
			.then((r: Response) => r.ok ? r.json() : { data: [] })
			.catch(() => ({ data: [] })),
		
		// Latest daily videos
		fetch(`${baseUrl}/api/room-resources?room_id=${SMALL_ACCOUNT_MENTORSHIP_ID}&resource_type=video&content_type=daily_video&per_page=6`)
			.then((r: Response) => r.ok ? r.json() : { data: [] })
			.catch(() => ({ data: [] })),
		
		// PDFs and documents
		fetch(`${baseUrl}/api/room-resources?room_id=${SMALL_ACCOUNT_MENTORSHIP_ID}&resource_type=pdf&per_page=10`)
			.then((r: Response) => r.ok ? r.json() : { data: [] })
			.catch(() => ({ data: [] }))
	]);
