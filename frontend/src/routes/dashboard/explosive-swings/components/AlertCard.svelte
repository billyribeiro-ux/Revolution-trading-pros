<script lang="ts">
	...
	const alertConfig = $derived.by(() => {
		switch (alert.type) {
			...
			case 'EXIT':
				...
				return {
					cardClass: isWin ? 'alert-card--exit-win' : 'alert-card--exit-loss',
	...
	const alertCardClass = $derived({
		'alert-card': true,
		[alertConfig.cardClass]: true,
		'is-new': alert.isNew,
		'has-notes-open': isNotesExpanded
	});

	const typeBadgeClass = $derived(['type-badge', alertConfig.badgeClass]);

	const notesToggleClass = $derived({
		'notes-toggle': true,
		expanded: isNotesExpanded
	});

	const exitResultClass = $derived({
		'exit-result': true,
		positive: (alert.resultPercent ?? 0) >= 0,
		negative: (alert.resultPercent ?? 0) < 0
	});
...
>
	...
	<div class="actions-row">
		<button class={copyButtonClass} onclick={handleCopy}>
			{#if isCopied}
				<svg aria-hidden="true" viewBox="0 0 20 20" fill="currentColor">
					<path
						fill-rule="evenodd"
...
<style>
	...
	.alert-card--entry {
		border-left-color: var(--color-entry, #14b8a6);
	}

	.alert-card--entry.is-new {
		background: rgba(240, 253, 250, 0.6);
	}

	.alert-card--update {
		border-left-color: var(--color-watching, #f59e0b);
	}

	.alert-card--update.is-new {
		background: rgba(255, 251, 235, 0.6);
	}

	.alert-card--exit-win {
		border-left-color: var(--color-profit, #10b981);
		background: rgba(236, 253, 245, 0.4);
	}

	.alert-card--exit-loss {
	...
	.badge-entry {
		background: linear-gradient(135deg, var(--color-entry-bg) 0%, var(--color-entry-bg) 100%);
		color: var(--color-entry-dark);
	}

	.badge-update {
		background: linear-gradient(135deg, var(--color-watching-bg) 0%, var(--color-watching-bg) 100%);
	...
	.notes-content {
		padding: 18px 20px;
		background: linear-gradient(135deg, var(--color-bg-subtle) 0%, var(--color-bg-subtle) 100%);
	}
