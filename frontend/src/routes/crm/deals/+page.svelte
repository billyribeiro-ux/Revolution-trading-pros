<script lang="ts">
  ...
  import { deals, pipelines, selectedPipeline, dealsByStage, isLoading, error } from '$lib/stores/crm';
  ...
  let forecast: { commit: number; best_case: number; pipeline: number; worst_case: number } | null = null;
...
<div class="min-h-screen bg-slate-950/95 text-slate-50">
  <div class="mx-auto max-w-7xl px-6 py-8">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-slate-400">RevolutionCRM-L8-System · Forecast-ready sales pipeline</p>
        </div>
      </div>
      ...
      <div class="mb-6 grid gap-4 md:grid-cols-4">
        <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs">
          ...
          <p class="mt-2 flex items-center gap-1 text-lg font-semibold text-emerald-300">
            <IconCurrencyDollar size={18} /> {formatCurrency(forecast.commit)}
          </p>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs">
          ...
          <p class="mt-2 flex items-center gap-1 text-lg font-semibold text-sky-300">
            <IconCurrencyDollar size={18} /> {formatCurrency(forecast.best_case)}
          </p>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs">
          ...
          <p class="mt-2 flex items-center gap-1 text-lg font-semibold text-slate-100">
            <IconCurrencyDollar size={18} /> {formatCurrency(forecast.pipeline)}
          </p>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs">
          ...
          <p class="mt-2 flex items-center gap-1 text-lg font-semibold text-amber-300">
            <IconCurrencyDollar size={18} /> {formatCurrency(forecast.worst_case)}
          </p>
        </div>
      </div>
    {/if}
    ...
    <!-- Error -->
    {#if $error}
      <div class="mb-4 rounded-xl border border-rose-700/60 bg-rose-950/40 px-4 py-3 text-sm text-rose-200">
        {$error}
      </div>
    {/if}
    ...
    <div class="flex gap-3 overflow-x-auto pb-4">
      ...
      {:else}
        {#each $selectedPipeline.stages as stage (stage.id)}
          <div class="min-w-[260px] flex-1 rounded-2xl border border-slate-800/80 bg-slate-900/70 p-3">
            <div class="mb-3 flex items-center justify-between gap-2 text-xs">
              <div>
                <p class="font-medium text-slate-100">{stage.name}</p>
                <p class="text-[11px] text-slate-500">{stage.probability}% · {$dealsByStage[stage.id]?.length ?? 0} deals</p>
              </div>
