<script lang="ts">
  ...
  import { IconUser, IconSearch, IconFilter, IconTrendingUp, IconAlertTriangle } from '@tabler/icons-svelte';
  ...
  $: atRiskCustomers = $contacts.filter((c) => c.status === 'customer' && c.health_score < 50).length;
</script>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-slate-400">RevolutionCRM-L8-System · 360° view of every relationship</p>
        </div>
      </div>

      <div class="flex items-center gap-2 text-xs text-slate-400">
        <span class="rounded-full bg-emerald-500/10 px-3 py-1 text-emerald-300">Lead Scoring L8</span>
        <span class="rounded-full bg-sky-500/10 px-3 py-1 text-sky-300">Behavior Linked</span>
      </div>
    </div>

    <!-- Filters -->
    <div class="mb-6 flex flex-wrap items-center gap-3 rounded-2xl border border-slate-800/80 bg-slate-900/70 p-4">
      <div class="flex flex-1 items-center gap-2 rounded-xl border border-slate-800 bg-slate-900/80 px-3 py-2">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/70">
      <div class="max-h-[70vh] overflow-auto">
        <table class="min-w-full text-sm">
          <thead class="sticky top-0 bg-slate-900/95 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
