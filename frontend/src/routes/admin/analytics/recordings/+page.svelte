<script lang="ts">
	...
	async function loadRecordings() {
		...
		try {
			...
			const response = await fetch(`/api/admin/analytics/recordings?period=${selectedPeriod}&filter=${activeFilter}&page=${page}`);
			if (response.ok) {
	...
	function getDeviceIcon(device: string): string {
		switch (device) {
			case 'mobile': return 'M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z';
			case 'tablet': return 'M12 18h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z';
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		...
		<header class="flex items-center justify-between mb-8">
			<div class="flex items-center gap-4">
				<div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center text-2xl shadow-lg shadow-indigo-500/20">
		...
		{#if connectionLoading}
			<div class="flex items-center justify-center py-20">
				<div class="relative">
					...
					<div class="absolute top-0 left-0 w-12 h-12 border-4 border-indigo-500 rounded-full animate-spin border-t-transparent"></div>
				</div>
			</div>
		...
		{:else}
			...
			<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
				...
				<div class="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-5">
					<div class="text-3xl font-bold text-indigo-400 mb-1">{formatDuration(stats.avgDuration)}</div>
			...
			{#if loading}
				<div class="flex items-center justify-center py-20">
					<div class="relative">
						...
						<div class="absolute top-0 left-0 w-10 h-10 border-4 border-indigo-500 rounded-full animate-spin border-t-transparent"></div>
					</div>
				</div>
			{:else if error}
			...
			{:else if recordings.length === 0}
				<div class="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-12 text-center">
					...
					<p class="text-slate-400 mb-6">Session recordings will appear once enabled and visitors interact with your site</p>
					<a
					...
					<div class="overflow-x-auto">
						<table class="w-full text-sm">
							<thead class="bg-slate-800/50">
								<tr>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Session</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">User</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Device</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Duration</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Pages</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Flags</th>
									<th class="text-right py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Date</th>
										...
										<td class="py-4 px-5">
											<div class="flex items-center gap-2">
												<svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={getDeviceIcon(recording.device_type)} />
												</svg>
												<span class="text-slate-400 capitalize">{recording.device_type}</span>
											</div>
										</td>
										<td class="py-4 px-5">
											<span class="text-white font-medium">{formatDuration(recording.duration)}</span>
										</td>
										<td class="py-4 px-5">
										...
										<td class="py-4 px-5">
											<div class="flex items-center gap-2">
												{#if recording.has_errors}
													<span class="px-2 py-1 rounded-lg bg-red-500/20 text-red-400 text-xs font-medium">Error</span>
												{/if}
												{#if recording.has_rage_clicks}
													<span class="px-2 py-1 rounded-lg bg-amber-500/20 text-amber-400 text-xs font-medium">Rage</span>
												{/if}
...
{#if selectedRecording}
	<div class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
		<div class="bg-slate-900 rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden border border-white/10 shadow-2xl">
			...
			<div class="aspect-video bg-slate-800/50 flex items-center justify-center">
				<div class="text-center">
					<div class="w-20 h-20 mx-auto mb-4 rounded-2xl bg-indigo-500/10 flex items-center justify-center">
