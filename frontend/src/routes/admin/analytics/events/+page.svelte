	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		...
		<header class="mb-8">
			<div class="flex items-center gap-4 mb-2">
				<div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-2xl shadow-lg shadow-amber-500/20">
		...
		{#if connectionLoading}
			<div class="flex items-center justify-center py-20">
				<div class="relative">
					...
					<div class="absolute top-0 left-0 w-12 h-12 border-4 border-amber-500 rounded-full animate-spin border-t-transparent"></div>
				</div>
			</div>
						...
						{#each eventTypes.slice(0, 12) as type}
							<button
								...
								class="px-4 py-2 rounded-xl text-sm font-medium transition-all border
									...
									: eventColors[type.name] || 'bg-slate-700/50 text-slate-300 border-slate-600/50 hover:bg-slate-700'}"
							>
								{type.name}
				...
				{#if loading}
					<div class="flex items-center justify-center py-20">
						<div class="relative">
							...
							<div class="absolute top-0 left-0 w-10 h-10 border-4 border-amber-500 rounded-full animate-spin border-t-transparent"></div>
						</div>
					</div>
				{:else if error}
					...
					<div class="overflow-x-auto">
						<table class="w-full text-sm">
							<thead class="bg-slate-800/50">
								<tr>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Event</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">User</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Page</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Properties</th>
									<th class="text-left py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Source</th>
									<th class="text-right py-4 px-5 font-medium text-slate-400 uppercase text-xs tracking-wider">Time</th>
								</tr>
							</thead>
							<tbody class="divide-y divide-white/5">
								{#each events as event}
									<tr class="hover:bg-white/5 cursor-pointer transition-colors">
										<td class="py-4 px-5">
											<span
												class="px-3 py-1.5 rounded-lg text-xs font-medium border {eventColors[event.event_type] ||
