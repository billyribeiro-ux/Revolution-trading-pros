<script lang="ts">
	/**
	 * R27-E extraction (2026-05-20): create/edit modal for a blog Tag.
	 * Identical shape to BlogCategoryModal MINUS the description
	 * textarea (Tags have no description in the API schema). Kept as
	 * a separate component so the form-field set is statically
	 * type-safe per entity — TagFormData has no `description`
	 * property, so a parameterized single-modal would either need a
	 * runtime branch or an awkward `description?: never` type.
	 */
			...
			<div class="modal-body">
				...
				<div class="form-row">
					<div class="form-group">
						...
						<input id="tag-color" name="tag-color" type="color" bind:value={formData.color} />
					</div>
