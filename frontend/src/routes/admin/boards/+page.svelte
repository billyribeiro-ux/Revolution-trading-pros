		...
		<div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
			...
			<div class="lg:col-span-1 space-y-6">
				...
				<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
					...
					<div class="space-y-1">
						<button
							...
							class="w-full px-3 py-2 text-left rounded-lg flex items-center gap-2 {selectedFolder === null ? 'bg-[rgba(230,184,0,0.1)] dark:bg-[rgba(230,184,0,0.2)] text-[#E6B800] dark:text-[#FFD11A]' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}"
						>
							<IconLayoutKanban class="w-4 h-4" />
							All Boards
							<span class="ml-auto text-xs bg-gray-200 dark:bg-gray-700 px-2 py-0.5 rounded-full">{boards.filter(b => !b.is_archived).length}</span>
						</button>
						{#each folders as folder}
							<button
								onclick={() => selectedFolder = folder.id}
								class="w-full px-3 py-2 text-left rounded-lg flex items-center gap-2 {selectedFolder === folder.id ? 'bg-[rgba(230,184,0,0.1)] dark:bg-[rgba(230,184,0,0.2)] text-[#E6B800] dark:text-[#FFD11A]' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}"
							>
								<IconFolder class="w-4 h-4" style="color: {folder.color}" />
								{folder.title}
								<span class="ml-auto text-xs bg-gray-200 dark:bg-gray-700 px-2 py-0.5 rounded-full">{folder.board_count || 0}</span>
							</button>
						{/each}
					</div>
				</div>
						...
						<div class="space-y-2">
							{#each tasksDueToday.slice(0, 5) as task}
								...
								>
									...
									<div class="text-xs text-gray-500 dark:text-gray-400">{task.due_date ? formatDate(task.due_date) : ''}</div>
								</a>
							{/each}
						</div>
					</div>
				...
				{#if overdueTasks.length > 0}
					<div class="bg-white dark:bg-gray-800 rounded-xl border border-red-200 dark:border-red-900/50 p-4">
						...
						<div class="space-y-2">
							{#each overdueTasks.slice(0, 5) as task}
								...
								>
									<div class="text-sm font-medium text-gray-900 dark:text-white truncate">{task.title}</div>
									<div class="text-xs text-red-500">{task.due_date ? formatDate(task.due_date) : ''}</div>
								</a>
							{/each}
						</div>
					</div>
				...
				<div class="flex items-center gap-4 mb-6">
					...
					<div class="flex items-center gap-2">
						<button
							...
							class="p-2 rounded-lg {viewMode === 'grid' ? 'bg-[rgba(230,184,0,0.15)] dark:bg-[rgba(230,184,0,0.2)] text-[#E6B800]' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700'}"
						>
							<IconLayoutGrid class="w-5 h-5" />
						</button>
						<button
							onclick={() => viewMode = 'list'}
							class="p-2 rounded-lg {viewMode === 'list' ? 'bg-[rgba(230,184,0,0.15)] dark:bg-[rgba(230,184,0,0.2)] text-[#E6B800]' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700'}"
						>
						...
						<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
							{#each favoriteBoards as board}
								<a
									...
									class="group bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 hover:shadow-lg hover:border-[#E6B800] dark:hover:border-[#B38F00] transition-all"
								...
								>
									...
									{#if board.description}
										<p class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-3">{board.description}</p>
									{/if}
					...
					{:else if viewMode === 'grid'}
						<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
							{#each filteredBoards as board}
								<div class="group bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:shadow-lg hover:border-[#E6B800] dark:hover:border-[#B38F00] transition-all overflow-hidden">
									<a
										href="/admin/boards/{board.id}"
										class="block p-4"
										style="border-left: 4px solid {board.background_color || '#E6B800'}"
									...
									<div class="px-4 py-2 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
										<button
						...
						<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
							<table class="w-full">
								<thead class="bg-gray-50 dark:bg-gray-700">
									<tr>
										<th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Board</th>
										<th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Tasks</th>
										<th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Members</th>
										<th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Updated</th>
												...
												<a href="/admin/boards/{board.id}" class="flex items-center gap-3">
													...
													<div>
														<div class="font-medium text-gray-900 dark:text-white flex items-center gap-2">
															...
															{/if}
															{#if board.is_archived}
																<span class="text-xs bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-2 py-0.5 rounded">Archived</span>
															{/if}
														</div>
														{#if board.description}
															<div class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">{board.description}</div>
														{/if}
													</div>
												</a>
											</td>
