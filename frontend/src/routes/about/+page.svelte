...
<svelte:head>
    <style>
        ...
        .interactive-card::before {
            ...
            background: radial-gradient(800px circle at var(--card-x, 50%) var(--card-y, 50%), rgba(255,255,255,0.08), transparent 40%);
            z-index: 2;
            pointer-events: none;
            opacity: 0;
...
>
    <div class="fixed inset-0 pointer-events-none z-0">
        <div class="absolute inset-0 opacity-[0.04] mix-blend-overlay z-[2]" style="background-image: url('data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.8%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter)%22/%3E%3C/svg%3E');"></div>
        
        <div class="absolute inset-0 opacity-30 transition-transform duration-100 ease-out will-change-transform"
    ...
    <div class="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#020202]/90 backdrop-blur-md h-10 flex items-center shadow-2xl">
            ...
            <div class="ticker-move">
                {#each [...tickerItems, ...tickerItems, ...tickerItems, ...tickerItems] as item}
                    <span class="inline-block px-8 group cursor-default hover:text-white transition-colors">
                        <span class="{item.includes('+') ? 'text-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'text-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]'} inline-block w-1.5 h-1.5 rounded-full mr-2"></span> 
                        {item}
                    </span>
                {/each}
            </div>
    ...
    <main id="main-content" class="relative z-10 pt-32 pb-24 px-6 lg:px-8">
        ...
        <section class="max-w-[1600px] mx-auto mb-32 lg:mb-48 relative">
            <div class="absolute top-20 -left-[20%] -right-[20%] h-[500px] grid-floor pointer-events-none parallax-layer z-0"></div>

            {#if isVisible}
                <div in:heavySlide={{ delay: 0, duration: 1200 }} class="flex flex-col lg:flex-row gap-16 lg:items-end border-b border-white/10 pb-16 relative z-10">
                    <div class="absolute -top-4 -left-4 w-12 h-12 border-t-2 border-l-2 border-amber-600/30"></div>
                    <div class="absolute -bottom-4 -right-4 w-12 h-12 border-b-2 border-r-2 border-amber-600/30"></div>

                    <div class="lg:w-2/3 relative z-10">
                        <div class="inline-flex items-center gap-3 px-4 py-1.5 border border-emerald-900/30 bg-emerald-950/20 text-emerald-400 text-[10px] font-bold tracking-[0.3em] uppercase mb-8 rounded-full shadow-[0_0_20px_rgba(16,185,129,0.15)] backdrop-blur-sm">
                        ...
                        <h1 class="text-6xl sm:text-7xl lg:text-9xl font-serif text-white tracking-tighter leading-[0.9] mb-10 mix-blend-screen drop-shadow-2xl">
                            Trade Real <span class="text-transparent bg-clip-text bg-gradient-to-r from-slate-100 to-slate-600">Capital.</span><br />
                            <span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-700 drop-shadow-[0_0_30px_rgba(217,119,6,0.3)]">Real Mentors.</span><br />
                            <span class="text-slate-600">Real Time.</span>
                        </h1>
                        
                        <p class="text-xl text-slate-400 font-light leading-relaxed max-w-2xl border-l-2 border-amber-600/50 pl-6 mb-12 bg-gradient-to-r from-white/5 to-transparent py-4 backdrop-blur-sm rounded-r-lg">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            <a href="/join" class="group relative inline-flex justify-center items-center gap-3 px-10 py-5 bg-gradient-to-br from-amber-700 to-amber-900 text-white font-bold text-xs uppercase tracking-[0.2em] transition-all overflow-hidden border border-amber-600/50 hover:border-amber-400 hover:shadow-[0_0_40px_rgba(245,158,11,0.3)]">
                                <span class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-500 ease-in-out"></span>
                    ...
                    <div class="lg:w-1/3 grid grid-cols-2 gap-px bg-white/10 border border-white/10 backdrop-blur-md shadow-2xl rounded-sm overflow-hidden">
                        {#each stats as stat, i}
                            ...
                            <div class="bg-[#050505]/90 p-6 group hover:bg-[#0A0A0A] transition-colors relative overflow-hidden interactive-card">
                                ...
                                <div class="text-3xl font-serif text-white mb-1 tabular-nums tracking-tight group-hover:text-amber-500 transition-colors">{stat.value}</div>
        ...
        <section class="max-w-[1400px] mx-auto mb-32 lg:mb-48 grid lg:grid-cols-2 gap-24 items-center">
            {#if isVisible}
                <div in:heavySlide={{ delay: 200 }} class="order-2 lg:order-1 relative group perspective-1000">
                    <div class="relative w-full aspect-[4/3] bg-[#050505] border border-white/10 rounded-lg overflow-hidden shadow-2xl transform transition-transform duration-1000 group-hover:rotate-y-3 group-hover:rotate-x-2 preserve-3d">
                        
                        <div class="absolute inset-0 bg-[linear-gradient(0deg,rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:3rem_3rem]"></div>
                        
                        <svg class="absolute inset-0 w-full h-full text-amber-500 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]" viewBox="0 0 400 300" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path 
                            ...
                            />
                            <path d="M0 280 L50 250 L100 260 L150 180 L200 200 L250 100 L300 120 L350 50 L400 20 V 300 H 0 Z" fill="url(#grad)" opacity="0.1" in:fade={{ delay: 1000, duration: 2000 }} />
                            <defs>
                        ...
                        <div class="absolute top-0 bottom-0 w-[2px] bg-white/50 scanner-line shadow-[0_0_20px_rgba(255,255,255,0.8)] z-20"></div>
                        
                        <div class="absolute top-10 left-10 bg-[#080808]/90 border-l-4 border-red-500 p-6 shadow-2xl max-w-xs backdrop-blur-xl transform translate-z-10 group-hover:translate-z-20 transition-transform duration-500">
                        ...
                        <ul class="space-y-4 mt-8">
                            {#each ['No pump and dumps', 'No fake P&L screenshots', 'Real-time voice commentary'] as item}
                                <li class="flex items-center gap-4 text-sm text-amber-500 font-mono uppercase tracking-wider group cursor-default">
                                    <div class="p-1.5 rounded bg-amber-500/10 border border-amber-500/20 group-hover:bg-amber-500 group-hover:text-black transition-colors duration-300">
            ...
            {#if isVisible}
                <div in:heavySlide={{ delay: 400 }} class="border-t border-white/10 pt-20">
                    <div class="flex flex-col md:flex-row justify-between items-end mb-20 gap-6">
                        ...
                        <p class="text-slate-500 text-sm max-w-md md:text-right hidden md:block leading-relaxed border-r-2 border-amber-600/50 pr-6">
                    ...
                    <div class="grid md:grid-cols-3 gap-6">
                        {#each features as feat, i}
                            ...
                            <div class="group interactive-card bg-[#050505] border border-white/10 p-10 hover:border-amber-600/40 transition-all duration-500 relative overflow-hidden flex flex-col h-full rounded-xl">
                                
                                <div class="absolute -top-6 -right-6 text-white opacity-[0.02] group-hover:opacity-[0.05] transition-opacity duration-700 pointer-events-none transform group-hover:scale-110 group-hover:-rotate-12 transition-transform">
                                    <Icon size={200} />
                                </div>
                                
                                <div class="flex justify-between items-start mb-8 relative z-10">
                                    <div class="p-4 bg-white/5 border border-white/5 text-amber-600 group-hover:bg-amber-600 group-hover:text-black transition-all duration-300 rounded-lg shadow-lg group-hover:shadow-amber-600/20">
                                ...
                                <div class="w-12 h-[1px] bg-amber-600/50 mb-4 group-hover:w-full transition-all duration-700 ease-out"></div>
                ...
                <div in:heavySlide={{ delay: 500 }} class="border-t border-white/10 pt-20">
                    <div class="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
                        ...
                        <div class="hidden md:flex gap-2">
                             {#each team as _}<div class="w-1.5 h-1.5 rounded-full bg-emerald-500/50 animate-pulse"></div>{/each}
                        </div>
                    </div>

                    <div class="grid gap-px bg-white/5 border border-white/10 overflow-hidden rounded-lg">
                        {#each team as member}
                            <div class="group bg-[#050505] p-8 md:p-12 grid md:grid-cols-12 gap-8 items-center hover:bg-[#080808] transition-colors duration-300 relative overflow-hidden interactive-card">
                                <div class="absolute left-0 top-0 bottom-0 w-1.5 bg-amber-600 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300"></div>

                                <div class="md:col-span-2 flex items-center gap-6 relative z-10">
                                    <div class="w-20 h-20 rounded-full bg-gradient-to-br from-white/10 to-transparent flex items-center justify-center text-slate-400 border border-white/5 group-hover:border-amber-600/50 group-hover:text-amber-500 overflow-hidden shadow-2xl transition-all group-hover:scale-105">
                                ...
                                <div class="hidden md:block md:col-span-3 relative z-10">
                                    ...
                                    <div class="text-[10px] font-mono text-slate-500 uppercase tracking-widest mt-1 flex items-center gap-2">
                                        {member.role} <span class="text-white/10">|</span> <span class="text-emerald-500/50 font-bold">{member.id}</span>
                                    </div>
                                </div>

                                <div class="md:col-span-5 relative z-10">
                                    <p class="text-sm text-slate-400 leading-relaxed font-light border-l border-white/10 pl-4 group-hover:border-amber-600/30 transition-colors">{member.bio}</p>
                                </div>

                                <div class="md:col-span-2 flex flex-col items-end gap-3 relative z-10">
                                    {#each member.specialties as spec}
                                        <span class="px-3 py-1.5 bg-white/5 border border-white/5 text-[9px] text-slate-300 font-mono uppercase tracking-wider rounded-sm hover:border-amber-600/50 hover:text-amber-500 hover:bg-amber-900/10 transition-colors cursor-default whitespace-nowrap">
                                            {spec}
                                        </span>
                                    {/each}
                                </div>
        ...
        <section class="max-w-[1200px] mx-auto mb-32 lg:mb-48 px-4">
             {#if isVisible}
                <div in:heavySlide={{ delay: 600 }} class="text-center mb-20 relative">
                    <div class="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-96 h-96 bg-amber-600/10 blur-[120px] rounded-full pointer-events-none mix-blend-screen"></div>
                ...
                <div in:heavySlide={{ delay: 700 }} class="grid md:grid-cols-2 gap-8">
                    <div class="bg-[#080808]/80 backdrop-blur-xl p-10 border border-white/5 rounded-xl relative hover:border-amber-600/30 transition-colors duration-500 interactive-card">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="flex items-center gap-4 border-t border-white/5 pt-6">
                                <div class="w-10 h-10 bg-gradient-to-tr from-slate-800 to-slate-700 rounded-full flex items-center justify-center text-xs font-bold text-white border border-white/10">MK</div>
                                <div>
             ...
             {#if isVisible}
                <div in:heavySlide={{ delay: 800 }} class="border-t border-white/10 pt-20">
                    <div class="text-center mb-16">
                         ...
                         <div class="h-1 w-20 bg-amber-600 mx-auto rounded-full mt-6 shadow-[0_0_15px_rgba(217,119,6,0.5)]"></div>
                    </div>
                    
                    <div class="space-y-4">
                        {#each faqs as faq}
                            <details class="group bg-[#050505] border border-white/10 open:border-amber-600/30 transition-all duration-300 rounded-lg overflow-hidden">
        ...
        <section class="mt-32 border-t border-white/10 pt-32 pb-32 text-center relative overflow-hidden">
            ...
            <div class="relative z-10 px-4">
                <IconBuildingBank size={64} class="mx-auto text-amber-600 mb-8 opacity-80 drop-shadow-[0_0_20px_rgba(217,119,6,0.4)]" stroke={0.8} />
                <h2 class="text-5xl md:text-8xl font-serif text-white mb-8 tracking-tighter">Ready to <span class="italic text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-700">Turn Pro?</span></h2>
                ...
                <div class="mt-12 flex flex-wrap justify-center gap-6 text-[10px] text-slate-600 uppercase tracking-widest font-mono">
                    <span class="flex items-center gap-2 text-slate-500"><IconShieldLock size={12} /> Secure Payment</span>
