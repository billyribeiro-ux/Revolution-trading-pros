 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Generates Gravatar URLs from user email addresses.
...
export interface GravatarOptions {
	size?: number; // Image size (1-2048, default 80)
	default?: string; // Default image: 'mp', 'identicon', 'monsterid', 'wavatar', 'retro', 'robohash', 'blank', or URL
 ...
 *
 * @example
 * getGravatarUrl('user@example.com', { size: 32, default: 'mp' })
 * // Returns: https://secure.gravatar.com/avatar/abc123...?s=32&d=mp&r=g
 */
export function getGravatarUrl(
	...
	const {
		size = 32,
		default: defaultImage = 'mp', // 'mp' = mystery person (gender-neutral silhouette)
		rating = 'g'
	} = options;

	// Handle missing email
	if (!email) {
		return `https://secure.gravatar.com/avatar/?s=${size}&d=${defaultImage}&r=${rating}`;
	}
	...
	return `https://secure.gravatar.com/avatar/${hash}?s=${size}&d=${defaultImage}&r=${rating}`;
}

/**
 * Get user's avatar URL with Gravatar fallback
 * Prioritizes: avatar_url > avatar > gravatar
 */
export function getUserAvatarUrl(
	user: { email?: string; avatar?: string; avatar_url?: string } | null | undefined,
	options: GravatarOptions = {}
): string {
	if (!user) {
		return getGravatarUrl(null, options);
	}
