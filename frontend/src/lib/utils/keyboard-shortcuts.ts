 ...
 */
export function getAllShortcutDisplays(platform?: PlatformInfo): Record<
  ShortcutCategory,
  Array<{ action: ShortcutAction; display: string; description: string }>
> {
  const plat = platform || getPlatform();
  const result: Record<ShortcutCategory, Array<{ action: ShortcutAction; display: string; description: string }>> = {
    system: [],
    formatting: [],
    blocks: [],
    navigation: []
