<script lang="ts">
	...
	$effect(() => {
		if (tableEl && chain) {
			...
			gsap.fromTo(rows, { opacity: 0, y: 4 }, { opacity: 1, y: 0, duration: 0.2, stagger: 0.008, ease: 'power2.out' });
		}
	});
...
<div class="flex flex-col gap-3">
	...
	<div class="flex items-center justify-between flex-wrap gap-2">
		<div class="flex items-center gap-2">
			...
			{#if ticker}
				<span class="text-xs font-bold px-1.5 py-0.5 rounded" style="background: var(--calc-accent-glow); color: var(--calc-accent); font-family: var(--calc-font-mono);">
					{ticker}
				</span>
			{/if}
			{#if chain}
	...
	{:else if chain && strikes.length > 0}
		<!-- Chain Table -->
		<div bind:this={tableEl} class="overflow-x-auto rounded-xl" style="border: 1px solid var(--calc-border);">
			<table class="w-full text-[10px]" style="font-family: var(--calc-font-mono);">
				<thead>
					<tr style="background: var(--calc-surface);">
						<th colspan="6" class="text-center py-1.5 font-semibold border-b border-r" style="color: var(--calc-call); border-color: var(--calc-border);">CALLS</th>
						<th class="text-center py-1.5 font-semibold border-b px-3" style="color: var(--calc-text);">STRIKE</th>
						<th colspan="6" class="text-center py-1.5 font-semibold border-b border-l" style="color: var(--calc-put); border-color: var(--calc-border);">PUTS</th>
					</tr>
					<tr style="background: var(--calc-surface);">
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Bid</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Ask</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Last</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Vol</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">OI</th>
						<th class="text-right px-2 py-1 font-medium border-r" style="color: var(--calc-text-muted); border-color: var(--calc-border);">IV</th>
						<th class="text-center px-3 py-1 font-bold" style="color: var(--calc-text);">K</th>
						<th class="text-right px-2 py-1 font-medium border-l" style="color: var(--calc-text-muted); border-color: var(--calc-border);">Bid</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Ask</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Last</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">Vol</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">OI</th>
						<th class="text-right px-2 py-1 font-medium" style="color: var(--calc-text-muted);">IV</th>
					</tr>
				</thead>
				<tbody>
					{#each strikes as strike (strike)}
						...
						<tr
							...
							style="border-top: 1px solid var(--calc-border); {isATM ? 'background: var(--calc-accent-glow);' : ''}"
						>
							<!-- Call side -->
							{#if call}
								<td class="text-right px-2 py-1 cursor-pointer" style="background: {callBg}; {call.inTheMoney ? 'color: var(--calc-call);' : 'color: var(--calc-text-secondary);'}" onclick={() => handleSelectContract(call)}>{call.bid.toFixed(2)}</td>
								<td class="text-right px-2 py-1 cursor-pointer" style="background: {callBg}; {call.inTheMoney ? 'color: var(--calc-call);' : 'color: var(--calc-text-secondary);'}" onclick={() => handleSelectContract(call)}>{call.ask.toFixed(2)}</td>
								<td class="text-right px-2 py-1 cursor-pointer" style="background: {callBg}; color: var(--calc-text);" onclick={() => handleSelectContract(call)}>{call.lastPrice.toFixed(2)}</td>
								<td class="text-right px-2 py-1" style="color: var(--calc-text-muted);">{call.volume.toLocaleString()}</td>
								<td class="text-right px-2 py-1" style="color: var(--calc-text-muted);">{call.openInterest.toLocaleString()}</td>
								<td class="text-right px-2 py-1 border-r" style="color: var(--calc-accent); border-color: var(--calc-border);">
									{(call.impliedVolatility * 100).toFixed(1)}%
									{#if mispricingBadge(call)}
										<span class="ml-0.5 text-[8px]" style="color: {detectMispricing(call)?.direction === 'underpriced' ? 'var(--calc-call)' : 'var(--calc-put)'};">
							...
							<!-- Strike -->
							<td class="text-center px-3 py-1 font-bold" style="color: {isATM ? 'var(--calc-accent)' : 'var(--calc-text)'};">
								{strike.toFixed(strike % 1 === 0 ? 0 : 2)}
							</td>

							<!-- Put side -->
							{#if put}
								<td class="text-right px-2 py-1 border-l cursor-pointer" style="background: {putBg}; border-color: var(--calc-border); {put.inTheMoney ? 'color: var(--calc-put);' : 'color: var(--calc-text-secondary);'}" onclick={() => handleSelectContract(put)}>{put.bid.toFixed(2)}</td>
								<td class="text-right px-2 py-1 cursor-pointer" style="background: {putBg}; {put.inTheMoney ? 'color: var(--calc-put);' : 'color: var(--calc-text-secondary);'}" onclick={() => handleSelectContract(put)}>{put.ask.toFixed(2)}</td>
								<td class="text-right px-2 py-1 cursor-pointer" style="background: {putBg}; color: var(--calc-text);" onclick={() => handleSelectContract(put)}>{put.lastPrice.toFixed(2)}</td>
								<td class="text-right px-2 py-1" style="color: var(--calc-text-muted);">{put.volume.toLocaleString()}</td>
								<td class="text-right px-2 py-1" style="color: var(--calc-text-muted);">{put.openInterest.toLocaleString()}</td>
								<td class="text-right px-2 py-1" style="color: var(--calc-accent);">
									{(put.impliedVolatility * 100).toFixed(1)}%
									{#if mispricingBadge(put)}
										<span class="ml-0.5 text-[8px]" style="color: {detectMispricing(put)?.direction === 'underpriced' ? 'var(--calc-call)' : 'var(--calc-put)'};">
											{mispricingBadge(put)}
										</span>
									{/if}
								</td>
