...
export interface AITranslationState extends AIBlockState {
  sourceText: string;
...
export interface InteractiveState {
  accordion: Map<BlockId, Set<string>>;
  tabs: Map<BlockId, string>;
  toggles: Map<BlockId, boolean>;
...
export interface AIState {
  ...
  translation: Map<BlockId, AITranslationState>;
}
...
export class BlockStateManager {
  ...
  getAudioState(blockId: BlockId): AudioState {
    return this.state.media.audio.get(blockId) ?? {
      playing: false,
      progress: 0,
      volume: 1,
      muted: false,
  ...
  getNewsletterState(blockId: BlockId): NewsletterState {
    return this.state.forms.newsletter.get(blockId) ?? {
      email: '',
      submitting: false,
      success: false,
      error: null
  ...
  getAITranslationState(blockId: BlockId): AITranslationState {
    return this.state.ai.translation.get(blockId) ?? {
  ...
  setAITranslationState(blockId: BlockId, updates: Partial<AITranslationState>): void {
  ...
  getCountdownState(blockId: BlockId): CountdownState {
    return this.state.countdown.get(blockId) ?? {
      days: 0,
      hours: 0,
      minutes: 0,
      seconds: 0,
  ...
  getReusableState(blockId: BlockId): ReusableComponentState {
    return this.state.reusable.get(blockId) ?? {
      data: null,
      loading: false,
      error: null,
      version: 0,
