...
function createDashboardStore() {
  ...
  return {
    ...
    async updateWidgetLayout(widgetId: string, layout: { x: number; y: number; width: number; height: number }) {
      try {
...
export const visibleWidgets = derived(
  dashboardStore,
  $dashboard => $dashboard.widgets.filter(w => w.is_visible)
);

export const widgetsByType = derived(
  dashboardStore,
  $dashboard => {
