 ...
 * 
 * Usage:
 *   import { clickToSource } from './lib/dev/click-to-source';
 *   export default defineConfig({
 *     plugins: [clickToSource(), sveltekit()]
 *   });
 */
...
export interface ClickToSourceOptions {
  /** Modifier key to hold while clicking. Default: 'alt' */
  hotkey?: 'alt' | 'ctrl' | 'meta' | 'shift';
  
  /** Editor to open. Auto-detected if not specified */
  editor?: 'vscode' | 'cursor' | 'windsurf' | 'webstorm' | 'vim' | 'neovim';
  
  /** Show highlight on hover when holding hotkey. Default: true */
  highlight?: boolean;
  
  /** Highlight color. Default: '#3b82f6' (blue) */
  highlightColor?: string;
  
  /** Enable/disable the plugin. Default: true in dev */
  enabled?: boolean;
}
...
/**
 * Detects the editor from environment or running processes
 */
function detectEditor(): string {
  ...
  // Default to VS Code (most common)
  return 'vscode';
}

/**
 * Gets the URL protocol for opening files in the editor
 */
...
/**
 * Creates the Vite plugin for click-to-source functionality
 */
...
/**
 * Svelte preprocessor that adds data-source attributes to elements
 * Use this with svelte.config.js if you want source locations on elements
 */
