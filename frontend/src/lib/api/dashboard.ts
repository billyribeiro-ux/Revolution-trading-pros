...
export const dashboardApi = {
  async getDashboards(type: 'admin' | 'user' = 'user'): Promise<DashboardData> {
    const response = await apiClient.get(`/dashboards?type=${type}`) as { data: DashboardData };
    return response.data;
  },

  async getDashboard(id: string): Promise<DashboardData> {
    const response = await apiClient.get(`/dashboards/${id}`) as { data: DashboardData };
    return response.data;
  },

  async updateWidgetLayout(widgetId: string, layout: WidgetLayout): Promise<DashboardWidget> {
    const response = await apiClient.put(`/widgets/${widgetId}/layout`, layout) as { data: DashboardWidget };
    return response.data;
  },

  async addWidget(dashboardId: string, widget: Partial<DashboardWidget>): Promise<DashboardWidget> {
    const response = await apiClient.post(`/dashboards/${dashboardId}/widgets`, widget) as { data: DashboardWidget };
    return response.data;
  },
