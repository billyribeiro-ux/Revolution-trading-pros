...
class AuthenticationService {
	...
	async login(data: LoginData): Promise<User> {
		...
		try {
			...
			logger.error(
				'[AuthService] CRITICAL: Failed to fetch full user data after login. Using initial response data as fallback.',
				{ error }
			);
			logger.warn(
				...
				if (mutation.type === 'childList') {
					...
					for (const node of nodes) {
						if (node instanceof HTMLScriptElement && node.src && !this.isTrustedScript(node.src)) {
							logger.error('[AuthService] Suspicious script detected:', { src: node.src });
							node.remove();
...
export async function initializeAuth(): Promise<boolean> {
	...
	try {
		...
		// CRITICAL: Use logger.warn so errors are visible in dev (no-op in prod)
