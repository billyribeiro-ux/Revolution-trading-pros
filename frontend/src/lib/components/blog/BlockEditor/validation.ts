...
const imageContentSchema = v.object({
	mediaId: v.optional(v.pipe(v.number(), v.integer(), v.minValue(1))),
...
const priceAlertContentSchema = v.object({
	...
	priceTarget: v.optional(v.pipe(v.number(), v.minValue(0))),
	text: v.optional(v.pipe(v.string(), v.maxLength(MAX_LENGTHS.TEXT_MEDIUM)))
});
...
export function validateBlock(block: unknown): ValidationResult {
	...
	if (!basicResult.success) {
		return {
			success: false,
			errors: basicResult.issues.map((issue) => ({
				path: issue.path?.map((p) => p.key as PropertyKey) ?? [],
	...
	if (typeSchema) {
		...
		if (!typeResult.success) {
			return {
				success: false,
				errors: typeResult.issues.map((issue) => ({
					path: issue.path?.map((p) => p.key as PropertyKey) ?? [],
					message: issue.message,
					code: issue.type
				}))
