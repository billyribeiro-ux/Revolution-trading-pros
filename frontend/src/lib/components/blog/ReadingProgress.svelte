<script lang="ts">
	...
	let currentScrollY = $derived(scrollY.current ?? 0);
	let currentInnerHeight = $derived(innerHeight.current ?? 0);
	let isVisible = $derived(currentScrollY > 100);
	let progress = $derived.by(() => {
		if (typeof document === 'undefined') return 0;

		const contentElement = document.querySelector(contentSelector);
		if (!contentElement || currentInnerHeight <= 0) return 0;

		const rect = contentElement.getBoundingClientRect();
		const contentTop = rect.top + currentScrollY;
		const contentHeight = rect.height;
		if (contentHeight <= 0) return 0;

		const scrollPosition = currentScrollY + currentInnerHeight;
		const visibleContent = Math.max(0, scrollPosition - contentTop);
		const nextProgress = Math.min(100, Math.max(0, (visibleContent / contentHeight) * 100));

		return Math.round(nextProgress);
	});
</script>

{#if isVisible}
	<div
		class={['reading-progress', { bottom: position === 'bottom' }]}
		style="
			--progress: {progress}%;
			--height: {height}px;
			--color: {color};
