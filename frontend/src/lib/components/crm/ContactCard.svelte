<script lang="ts">
  ...
  import { IconMail, IconPhone, IconTrendingUp, IconAlertTriangle, IconStar } from '@tabler/icons-svelte';
  ...
  function getHealthBadge(score: number): { color: string; label: string } {
    if (score >= 75) return { color: 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30', label: 'Healthy' };
    if (score >= 50) return { color: 'bg-sky-500/10 text-sky-300 border-sky-500/30', label: 'Good' };
    if (score >= 25) return { color: 'bg-amber-500/10 text-amber-300 border-amber-500/30', label: 'At Risk' };
  ...
  <div class="flex items-start justify-between gap-3">
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2 mb-1">
        <h3 class="text-base font-semibold text-slate-50 truncate group-hover:text-indigo-300 transition-colors">
          {contact.full_name}
        </h3>
        {#if contact.is_vip}
    ...
    <div class="flex flex-col items-end gap-2 flex-shrink-0">
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-sm font-semibold {getScoreColor(contact.lead_score)}">{contact.lead_score}</span>
      </div>
