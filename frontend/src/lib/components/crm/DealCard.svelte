...
>
	<div class="mb-2 flex items-start justify-between gap-2">
		<h4
			class="flex-1 text-[13px] font-medium text-slate-100 group-hover:text-sky-300 transition-colors"
		>
			{deal.name}
		</h4>
		{#if deal.priority !== 'normal'}
			<span
				class="rounded-full border px-1.5 py-0.5 text-[9px] font-medium {getPriorityColor(
					deal.priority
				)}"
			>
				{deal.priority}
