...
>
	...
	<div class="language-bar">
		<select
			value={sourceLang}
			onchange={(e) =>
				updateContent({ translationSourceLang: (e.target as HTMLSelectElement).value })}
		...
		<select
			value={targetLang}
			onchange={(e) =>
				updateContent({ translationTargetLang: (e.target as HTMLSelectElement).value })}
		...
		<div class="panel source-panel">
			...
			{#if props.isEditing}
				<textarea
					...
					value={sourceText}
					oninput={(e) =>
						updateContent({ translationSource: (e.target as HTMLTextAreaElement).value })}
				></textarea>
			{:else}
...
<style>
	...
	:global(.dark) .language-bar select {
		background: #0f172a;
		border-color: #075985;
		color: #e2e8f0;
	}
	:global(.dark) .swap-btn {
		background: #0f172a;
		border-color: #075985;
		color: #38bdf8;
	}
	...
	:global(.dark) textarea.panel-content {
		background: #0f172a;
		color: #e2e8f0;
	}
	:global(.dark) .panel-content.readonly {
		background: #1e293b;
		color: #e2e8f0;
	}
	:global(.dark) .translation-actions {
		background: #1e293b;
		border-color: #374151;
	}
