		...
		<div class="trade-info">
			{#if props.isEditing}
				<input
					...
					value={symbol}
					oninput={(e) =>
						updateContent({ tradeSymbol: (e.target as HTMLInputElement).value.toUpperCase() })}
				/>
			{:else}
			...
			<div class="level-row entry-level">
				...
				{#if props.isEditing}
					<input
						...
						value={entry}
						oninput={(e) =>
							updateContent({ tradeEntry: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
			...
			<div class="level-row stop-level">
				...
				{#if props.isEditing}
					<input
						...
						value={stopLoss}
						oninput={(e) =>
							updateContent({ tradeStop: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
			...
			<div class="level-row target-level">
				...
				{#if props.isEditing}
					<input
						...
						value={target1}
						oninput={(e) =>
							updateContent({ tradeTarget1: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
			...
			<div class="level-row target-level">
				...
				{#if props.isEditing}
					<input
						...
						value={target2}
						oninput={(e) =>
							updateContent({ tradeTarget2: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
			...
			<div class="confidence-meter">
				...
				{#if props.isEditing}
					<input
						...
						value={confidence}
						oninput={(e) =>
							updateContent({ tradeConfidence: parseInt((e.target as HTMLInputElement).value) })}
					/>
				{/if}
			...
			<div class="timeframe">
				...
				{#if props.isEditing}
					<select
						value={timeframe}
						onchange={(e) =>
							updateContent({ tradeTimeframe: (e.target as HTMLSelectElement).value })}
					>
		...
		<div class="thesis-section">
			...
			{#if props.isEditing}
				<textarea
					...
					value={thesis}
					oninput={(e) => updateContent({ tradeThesis: (e.target as HTMLTextAreaElement).value })}
				></textarea>
			{:else if thesis}
...
<style>
	...
	.confidence-meter input[type='range'] {
		width: 100%;
		margin-bottom: 0.5rem;
	}
	.confidence-bar {
	...
	.long .confidence-fill {
		background: linear-gradient(90deg, #22c55e, #16a34a);
	}
	.short .confidence-fill {
	...
	:global(.dark) .trade-idea-block.long {
		background: linear-gradient(180deg, #052e16 0%, #0f172a 30%);
		border-color: #166534;
	}
	:global(.dark) .trade-idea-block.short {
		background: linear-gradient(180deg, #450a0a 0%, #0f172a 30%);
		border-color: #991b1b;
	}
	:global(.dark) .trade-symbol {
		color: #f8fafc;
	}
	:global(.dark) .trade-label {
		color: #94a3b8;
	}
	:global(.dark) .level-row {
		background: #1e293b;
	}
	:global(.dark) .level-label {
		color: #94a3b8;
	}
	:global(.dark) .level-value {
		color: #f1f5f9;
	}
	:global(.dark) .confidence-value {
		color: #f1f5f9;
	}
	:global(.dark) .confidence-bar {
		background: #334155;
	}
	:global(.dark) .timeframe-value {
		color: #f1f5f9;
	}
	:global(.dark) .thesis-text {
		color: #e2e8f0;
	}
	...
	:global(.dark) .symbol-input,
	:global(.dark) .level-row input,
	:global(.dark) .thesis-section textarea,
