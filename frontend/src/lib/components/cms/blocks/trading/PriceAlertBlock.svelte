...
<script lang="ts">
	...
	import { Icon, IconBell, IconTarget, IconTrendingDown, IconTrendingUp } from '$lib/icons';
		...
		<div class="alert-title">
			{#if isEditing}
				<input
					...
					value={symbol}
					oninput={(e) =>
						updateContent({ alertSymbol: (e.target as HTMLInputElement).value.toUpperCase() })}
					aria-label="Symbol"
				/>
			{:else}
			...
			<div class="level target">
				...
				{#if isEditing}
					<input
						...
						value={targetPrice}
						oninput={(e) =>
							updateContent({ alertTarget: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
			...
			<div class="level entry">
				...
				{#if isEditing}
					<input
						...
						value={entryPrice}
						oninput={(e) =>
							updateContent({ alertEntry: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
			...
			<div class="level stop">
				...
				{#if isEditing}
					<input
						...
						value={stopLoss}
						oninput={(e) =>
							updateContent({ alertStop: parseFloat((e.target as HTMLInputElement).value) })}
					/>
				{:else}
...
<style>
	...
	.price-alert-block.bullish {
		background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%);
		border-color: #86efac;
	}
	.price-alert-block.bearish {
	...
	.bullish .direction-badge {
		background: #22c55e;
		color: white;
	}
	.bearish .direction-badge {
		background: #ef4444;
		color: white;
	}
	...
	:global(.dark) .price-alert-block.bullish {
		background: linear-gradient(135deg, #052e16 0%, #14532d 100%);
		border-color: #166534;
	}
	:global(.dark) .price-alert-block.bearish {
	...
	:global(.dark) .rr-display {
		background: rgba(0, 0, 0, 0.3);
	}
	:global(.dark) .rr-label {
		color: #94a3b8;
	}
	:global(.dark) .alert-note p {
		color: #94a3b8;
	}
	:global(.dark) .symbol-input,
	:global(.dark) .level input,
	:global(.dark) .alert-note textarea {
		background: #0f172a;
		border-color: #475569;
		color: #e2e8f0;
	}
