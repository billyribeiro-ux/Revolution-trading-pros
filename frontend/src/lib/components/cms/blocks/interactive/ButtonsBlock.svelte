...
<script lang="ts">
	import { IconPlus, IconX, IconExternalLink } from '#lib/icons/index.js';
			...
			<div class="button-wrapper">
				{#if props.isEditing}
					<span
						...
						onpaste={handlePaste}>{button.text}</span
					>
					{#if buttons.length > 1}
						<button
							type="button"
				...
				<div class="button-settings">
					...
					<label class="setting-field">
						...
						<input
							type="url"
							value={button.url}
							oninput={(e) => updateButton(index, { url: (e.target as HTMLInputElement).value })}
						/>
					</label>
					<label class="setting-field">
						<span>Style:</span>
						<select
							...
							onchange={(e) =>
								updateButton(index, {
									style: (e.target as HTMLSelectElement).value as ButtonItem['style']
								})}
						>
					...
					<label class="setting-checkbox">
						<input
							...
							onchange={(e) =>
								updateButton(index, { newTab: (e.target as HTMLInputElement).checked })}
						/>
						<span>New tab</span>
					</label>
				</div>
...
<style>
	...
	.layout-row .buttons-container {
		flex-direction: row;
	}
	.layout-column .buttons-container {
		flex-direction: column;
		align-items: flex-start;
	}
	.layout-wrap .buttons-container {
		flex-wrap: wrap;
	}
	.align-left .buttons-container {
		justify-content: flex-start;
	}
	.align-center .buttons-container {
		justify-content: center;
	}
	.align-right .buttons-container {
	...
	.button-wrapper:hover .remove-btn {
		opacity: 1;
	}
	.add-btn {
		display: flex;
