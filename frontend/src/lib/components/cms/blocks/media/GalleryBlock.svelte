...
<script lang="ts">
	...
	let columns = $derived(
		Math.min(6, Math.max(1, props.block.settings.galleryColumns || props.block.content.columnCount || 3))
	);
	...
	function addImage(): void {
		...
		const updatedImages = [
			...images,
			{ url: newImage.url, alt: newImage.alt, id: newImage.id }
		];
	...
	function removeImage(index: number): void {
		...
		const updatedImages = images
			.filter((_, i) => i !== index)
			.map((img) => ({ url: img.url, alt: img.alt, id: img.id }));
	...
	function updateImageAlt(index: number, alt: string): void {
		...
		const updatedImages = images.map((img, i) =>
			i === index ? { url: img.url, alt, id: img.id } : { url: img.url, alt: img.alt, id: img.id }
		);
	...
	function updateImageUrl(index: number, url: string): void {
		...
		const updatedImages = images.map((img, i) =>
			i === index ? { url, alt: img.alt, id: img.id } : { url: img.url, alt: img.alt, id: img.id }
		);
