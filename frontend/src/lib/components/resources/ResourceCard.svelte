...
>
	...
	<div class="thumbnail-container relative aspect-video overflow-hidden bg-gray-100 dark:bg-gray-700">
		...
		<!-- Access level badge -->
		{#if showAccessLevel}
			<span class="absolute right-2 top-2 rounded-md px-2 py-1 text-xs font-medium {getAccessLevelColor(resource.access_level ?? 'premium')}">
		...
		{#if isVideo && resource.formatted_duration}
			<span class="absolute bottom-2 right-2 rounded bg-black/80 px-2 py-0.5 text-xs font-medium text-white">
	...
	<div class="content p-4">
		<!-- Title -->
		<h3 class="mb-1 line-clamp-2 font-semibold text-gray-900 dark:text-white" title={resource.title}>
			{resource.title}
		</h3>
	...
	{#if resource.is_featured || resource.is_pinned}
		<div class="absolute -right-8 top-3 rotate-45 bg-blue-600 px-8 py-0.5 text-xs font-medium text-white shadow-sm">
