<script lang="ts">
	...
	const depthRandom = mulberry32(4200);
	const marketDepth = Array(20)
		.fill(0)
		.map((_, i) => ({
			price: (4200 + i * 0.25).toFixed(2),
			size: Math.floor(depthRandom() * 500)
		}));
</script>
						...
						<div class="relative bg-[#050505] border border-white/10 p-1 shadow-2xl">
							...
							<div class="p-6 md:p-8 text-left">
								<div class="mb-6">
									<label
										for="email-access"
										class="block text-[10px] font-mono uppercase text-slate-500 tracking-widest mb-2"
									>
										Identity / Email
									</label>
									<div class="relative">
										...
										/>
										<div
											class="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-4 bg-amber-600 animate-pulse pointer-events-none"
										></div>
									</div>
								</div>
								...
								>
									<div class="flex flex-col">
										<span class="text-xs font-mono uppercase tracking-widest text-amber-200/80"
											>Action</span
										>
							...
							<div
								class="px-4 py-3 bg-[#0a0a0a] border-t border-white/5 flex justify-between items-center text-[10px] font-mono text-slate-600"
							>
