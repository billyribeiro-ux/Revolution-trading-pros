<script lang="ts">
	...
	// Ticker Tape Data
	const tickerRandom = mulberry32(8821);
	const tickerItems = Array(20)
		.fill(0)
		.map((_, i) => ({
			symbol: ['ES_F', 'NQ_F', 'SPX', 'VIX', 'AAPL', 'NVDA'][i % 6],
			price: (4000 + tickerRandom() * 1000).toFixed(2),
			change: (tickerRandom() * 2 - 0.5).toFixed(2) + '%'
		}));
</script>
...
>
	...
	>
		<div class="flex gap-12 whitespace-nowrap animate-marquee-left text-xs font-mono text-white">
			{#each [...tickerItems, ...tickerItems, ...tickerItems] as item, i (i)}
