    ...
    const reviews = [
        {
            ...
            location: 'London, UK',
            quote: "The risk protocols alone saved our desk six figures during the last CPI volatility. This isn't a course; it's an operating system for capital preservation.",
        ...
        },
        {
            ...
            quote: "I spent 3 years in retail 'discord' hell. One month in the institutional audit reset my entire view of liquidity. My execution drag is now virtually zero.",
        ...
        },
        {
            ...
            role: 'Swing Trader',
            location: 'Singapore',
            quote: "The dark pool logic is frighteningly accurate. We are front-running moves that I used to chase. The mental clarity of having a structured invalidated level is priceless.",
            metric: 'Win Rate',
            value: '68.2%',
            trend: 'positive'
        },
        {
            ...
            location: 'Chicago, IL',
            quote: "Strictly business. No hype, no noise. Just level 2 analysis and probability mapping. If you treat trading as a career, this is the only room that matters.",
        ...
        },
        {
            ...
            role: 'Options Desk',
            location: 'Toronto, CA',
            quote: "The 0DTE structure is surgical. We aren't gambling on gamma squeezes anymore; we are capturing defined volatility expansion. Professional grade.",
            metric: 'Sharpe Ratio',
            value: '1.92',
            trend: 'positive'
        },
        {
            ...
            role: 'Capital Growth',
            location: 'Sydney, AU',
            quote: "Scaled my account from mid-five to six figures in Q3 using the 'Compound Protocol.' The psychological sizing models removed the fear of pulling the trigger.",
            metric: 'YTD Return',
            value: '+41.8%',
            trend: 'positive'
        }
...
>
    <div class="absolute inset-0 pointer-events-none opacity-[0.04] overflow-hidden flex flex-col justify-center gap-24 transform -rotate-6 scale-110">
        <div class="flex gap-12 whitespace-nowrap animate-marquee-left text-xs font-mono text-white">
            {#each [...tickerItems, ...tickerItems, ...tickerItems] as item}
                <div class="flex gap-4">
                    ...
                    <span class={item.change.startsWith('-') ? 'text-red-500' : 'text-emerald-500'}>{item.change}</span>
                </div>
            {/each}
        </div>
    ...
    <div class="absolute inset-0 pointer-events-none bg-gradient-to-b from-[#020202] via-transparent to-[#020202]"></div>
    <div class="absolute inset-0 pointer-events-none opacity-40 transition-opacity duration-300"
    ...
    <div class="relative max-w-[1600px] mx-auto z-10">
        ...
        <div class="max-w-4xl mx-auto text-center mb-24">
            {#if isVisible}
                <div in:heavySlide={{ delay: 0, duration: 1000 }} class="inline-flex items-center gap-3 px-4 py-1.5 border border-emerald-900/30 bg-emerald-950/10 text-emerald-500 text-[10px] font-bold tracking-[0.3em] uppercase mb-8 rounded-sm">
                ...
                <p in:heavySlide={{ delay: 200 }} class="text-lg text-slate-400 font-light leading-relaxed max-w-2xl mx-auto">
                    We don't rely on marketing claims. We rely on the PnL of our desk. 
                    Verified feedback from funded traders, prop managers, and institutional clients.
                </p>
            {/if}
        </div>
                ...
                {#if isVisible}
                    ...
                    >
                        <div class="flex justify-between items-start mb-6 pb-6 border-b border-white/5">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-[10px] font-mono uppercase text-slate-500 tracking-wide">{review.role}</div>
                                </div>
                            </div>
                            <div class="text-[10px] font-mono text-emerald-500/80 border border-emerald-900/30 px-2 py-1 bg-emerald-950/10 rounded">
                                {review.id}
                            </div>
                        </div>

                        <div class="relative mb-8">
                            <IconQuote size={24} class="absolute -top-2 -left-2 text-white/5 transform -scale-x-100" />
                            <p class="relative z-10 text-slate-300 font-light leading-relaxed text-sm">
                                "{review.quote}"
                            </p>
                        </div>

                        <div class="flex items-center justify-between bg-white/[0.02] -mx-8 -mb-8 px-8 py-4 border-t border-white/5 group-hover:bg-emerald-950/[0.05] transition-colors duration-500">
                            ...
                            <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center gap-1.5 text-[10px] font-bold text-emerald-500 uppercase tracking-widest">
                        ...
                        <div class="absolute top-0 right-0 w-8 h-8 pointer-events-none overflow-hidden">
                            <div class="absolute top-0 right-0 w-2 h-2 bg-emerald-500/0 group-hover:bg-emerald-500/50 transition-colors duration-500"></div>
                            <svg class="absolute top-0 right-0 w-full h-full text-emerald-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500" viewBox="0 0 24 24">
        ...
        {#if isVisible}
            <div in:heavySlide={{ delay: 600 }} class="mt-20 pt-10 border-t border-white/5 flex flex-wrap justify-center gap-12 lg:gap-24 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
