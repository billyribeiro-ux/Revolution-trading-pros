					...
					{#if item.submenu}
						<div class="nav-dropdown">
							...
							>
								...
								<IconChevronDown
									size={16}
									class="nav-icon-chevron {activeDropdown === item.id ? 'rotate-180' : ''}"
								/>
							</button>
...
{#if !isDesktop && isMobileMenuOpen}
	...
	<nav class="nav-mobile-panel" bind:this={mobileNavRef} aria-label="Mobile Navigation">
				...
				{#if item.submenu}
					<div class="nav-mobile-group">
						...
						>
							...
							<IconChevronRight
								size={20}
								class="transition-transform duration-200 {activeMobileSubmenu === item.id
									? 'rotate-90'
									: ''}"
							/>
						</button>
			...
			<div class="nav-mobile-footer">
				{#if $isAuthenticated}
					<div class="p-4 bg-white/5 rounded-lg mb-4">
						...
						{/each}
						<button
							class="nav-mobile-sublink pl-0 text-red-400 w-full text-left"
				...
				{:else}
					<a
						href="/login"
						class="nav-btn-primary w-full justify-center py-3"
						onclick={closeMobileMenu}
					>
						Login
					</a>
