	...
	>
		...
		<div class="max-h-[300px] overflow-y-auto">
			{#each uploads as upload (upload.id)}
				<div
					class="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0"
					transition:slide
				>
					<!-- Thumbnail -->
					<div
					...
					<div class="flex-1 min-w-0">
						...
						{#if upload.status === 'uploading' || upload.status === 'processing'}
							<div class="h-1 bg-gray-200 dark:bg-gray-700 rounded-full mt-2 overflow-hidden">
								<div
									class="h-full bg-blue-500 transition-all duration-300 rounded-full {upload.status ===
									'processing'
										? 'animate-pulse'
										: ''}"
						...
						{#if showStats && upload.status === 'complete' && upload.result?.stats}
							<div class="flex items-center gap-1 text-xs mt-1" transition:fade>
								...
								>
								<Icon name="IconArrowRight" size={12} class="text-gray-400" />
								<span class="text-gray-600 dark:text-gray-300"
									>{formatBytes(upload.result.stats.optimizedSize)}</span
								>
								<span class="text-green-500 font-medium"
									>-{upload.result.stats.savingsPercent}%</span
								>
							</div>
						{/if}
					</div>
