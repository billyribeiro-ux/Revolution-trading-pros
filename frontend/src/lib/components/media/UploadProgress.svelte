			...
			{#each uploads as upload (upload.id)}
				<div class="up-item" transition:slide>
					...
					<div class="up-thumb" aria-hidden="true">
						...
						{:else}
							<svg
								class="up-icon-md up-icon-muted"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
					...
					<div class="up-info">
						...
						{#if showStats && upload.status === 'complete' && upload.result?.stats}
							<div class="up-opt-stats" transition:fade>
								...
								<svg
									class="up-icon-xs up-icon-muted"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
								>
...
<style>
	...
	.up-status-dot {
		...
		&[data-status='pending'] {
			background-color: oklch(0.75 0.005 265);
		}
	...
	.up-action-btn {
		...
		&[data-action='done']:hover {
			color: oklch(0.45 0.18 160);
			background-color: oklch(0.95 0.03 160);
		}
	}
</style>
