...
{#if compact}
  ...
  <div class="inline-flex items-center gap-2 text-sm {className}" transition:fade>
    ...
    <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 dark:bg-green-900/30 {getSavingsColor(stats.savingsPercent)}">
      -{Math.round($animatedSavings)}%
    </span>
  </div>
{:else}
  <!-- Full card version -->
  <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl overflow-hidden {className}" transition:slide>
    <!-- Success header -->
    <div class="flex items-center gap-3 p-4 bg-green-100/50 dark:bg-green-900/30 border-b border-green-200 dark:border-green-800">
      ...
      <div>
        <h3 class="text-lg font-semibold text-green-800 dark:text-green-200">Optimized Successfully</h3>
        {#if stats.processingTime}
          <p class="text-sm text-green-600 dark:text-green-400">Processed in {stats.processingTime}ms</p>
        {/if}
      </div>
    </div>
      ...
      <div class="flex flex-col items-center gap-2">
        <div class="flex items-center gap-2">
          ...
          {#if stats.originalFormat}
            <span class="px-1.5 py-0.5 text-[10px] font-medium rounded bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-400">{stats.originalFormat.toUpperCase()}</span>
          {/if}
        </div>
        <div class="py-1">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm text-green-600 dark:text-green-400 font-semibold">{formatBytes(stats.optimizedSize)}</span>
          {#if stats.optimizedFormat}
            <span class="px-1.5 py-0.5 text-[10px] font-medium rounded bg-green-200 text-green-700 dark:bg-green-800 dark:text-green-300">{stats.optimizedFormat.toUpperCase()}</span>
          {/if}
        </div>
      </div>
    </div>
    ...
    <!-- Additional details -->
    {#if showDetails}
      <div class="flex items-center justify-center gap-6 px-4 py-3 border-t border-green-200 dark:border-green-800 text-xs text-gray-600 dark:text-gray-400">
