<script lang="ts">
	...
	let compressionRatio = $derived(
		optimizedSize > 0 ? (originalSize / optimizedSize).toFixed(1) : '1'
	);
...
>
	...
	<div class="zoom-controls">
		<button
			onclick={() => (zoomLevel = Math.max(1, zoomLevel - 0.5))}
		...
		<button
			onclick={() => (zoomLevel = Math.min(5, zoomLevel + 0.5))}
		...
		{:else}
			...
			<div
				...
				onkeydown={(e: KeyboardEvent) =>
					e.key === ' ' && ((showOriginal = !showOriginal), e.preventDefault())}
				role="button"
				tabindex="0"
			>
