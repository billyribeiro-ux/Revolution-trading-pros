...
class GoogleAnalyticsAdapter implements AnalyticsAdapter {
	...
	identify(payload: IdentifyPayload): void {
		...
		if (this._config?.debug) {
			logger.debug('[GA4] User identified', { user_id: payload.user_id });
		}
	}
	...
	private _queueEvent(eventName: string, payload: Record<string, unknown>): void {
		...
		if (this._config?.debug) {
			logger.debug('[GA4] Event queued', { eventName, queueSize: this._eventQueue.length });
		}
	}
	...
	private _flushQueue(): void {
		...
		if (this._config?.debug) {
			logger.debug('[GA4] Queue flushed', { count: events.length });
		}
	}
}
