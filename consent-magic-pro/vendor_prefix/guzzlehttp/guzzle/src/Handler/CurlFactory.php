...
namespace ConsentMagicPro\GuzzleHttp\Handler;

use ConsentMagicPro\GuzzleHttp\Exception\ConnectException;
use ConsentMagicPro\GuzzleHttp\Exception\RequestException;
use ConsentMagicPro\GuzzleHttp\Promise as P;
use ConsentMagicPro\GuzzleHttp\Promise\FulfilledPromise;
use ConsentMagicPro\GuzzleHttp\Promise\PromiseInterface;
use ConsentMagicPro\GuzzleHttp\Psr7\LazyOpenStream;
use ConsentMagicPro\GuzzleHttp\TransferStats;
use ConsentMagicPro\GuzzleHttp\Utils;
use ConsentMagicPro\Psr\Http\Message\RequestInterface;
use ConsentMagicPro\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \ConsentMagicPro\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \ConsentMagicPro\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \ConsentMagicPro\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \ConsentMagicPro\GuzzleHttp\Psr7\Utils::streamFor($sink);
